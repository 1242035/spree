module Viauco
  module Api
    module V1
      class OptionValuesController < Viauco::Api::BaseController
        def index
        ...
        def create
          authorize! :create, Viauco::OptionValue
        ...
        def scope
          @scope ||= if params[:option_type_id]
                       Viauco::OptionType.find(params[:option_type_id]).option_values.accessible_by(current_ability, :show)
                     else
                       Viauco::OptionValue.accessible_by(current_ability, :show).load
                     end
        end
