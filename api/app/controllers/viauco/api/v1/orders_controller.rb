module Viauco
  module Api
    module V1
      class OrdersController < Viauco::Api::BaseController
        ...
        def create
          ...
          if can?(:admin, Viauco::Order)
            order_user = if @current_user_roles.include?('admin') && order_params[:user_id]
                           Viauco.user_class.find(order_params[:user_id])
                         else
                           current_api_user
                         end
            ...
            @order = Viauco::Core::Importer::Order.import(order_user, import_params)

            respond_with(@order, default_template: :show, status: 201)
          else
            @order = Viauco::Order.create!(user: current_api_user, store: current_store)
        ...
        def update
          ...
          if Cart::Update.call(order: @order, params: order_params).success?
            ...
            if current_api_user.has_viauco_role?('admin') && user_id
        ...
        def apply_coupon_code
          ...
          render 'viauco/api/v1/promotions/handler', status: status
        end

        def remove_coupon_code
          find_order(true)
          authorize! :update, @order, order_token
          @handler = Viauco::PromotionHandler::Coupon.new(@order).remove(params[:coupon_code])
