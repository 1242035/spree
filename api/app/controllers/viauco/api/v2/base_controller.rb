  ...
  module Api
    module V2
      class BaseController < ActionController::API
        ...
        def content_type
          Viauco::Api::Config[:api_v2_content_type]
        end
        ...
        def viauco_current_store
          @viauco_current_store ||= Viauco::Store.current(request.env['SERVER_NAME'])
        end

        def viauco_current_user
          @viauco_current_user ||= Viauco.user_class.find_by(id: doorkeeper_token.resource_owner_id) if doorkeeper_token
        end

        def viauco_authorize!(action, subject, *args)
          authorize!(action, subject, *args)
        end

        def require_viauco_current_user
          raise CanCan::AccessDenied if viauco_current_user.nil?
        end

        # Needs to be overriden so that we use Viauco's Ability rather than anyone else's.
        def current_ability
          @current_ability ||= Viauco::Dependencies.ability_class.constantize.new(viauco_current_user)
        end
        ...
        def current_currency
          viauco_current_store.default_currency || Viauco::Config[:currency]
        end

        def record_not_found
          render_error_payload(I18n.t(:resource_not_found, scope: 'viauco.api'), 404)
        end
