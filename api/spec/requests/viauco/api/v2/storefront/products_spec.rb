  ...
  describe 'products#index' do
    context 'with no params' do
      ...
      it 'returns all products' do
        expect(json_response['data'].count).to eq Viauco::Product.not_discontinued.count
      ...
      context 'sorting by price' do
        context 'ascending order' do
          ...
          it 'returns products sorted by price' do
            expect(json_response['data'].count).to      eq Viauco::Product.not_discontinued.count
            expect(json_response['data'].pluck(:id)).to eq Viauco::Product.not_discontinued.joins(master: :prices).select("#{Viauco::Product.table_name}.*, #{Viauco::Price.table_name}.amount").distinct.order("#{Viauco::Price.table_name}.amount").map(&:id).map(&:to_s)
          end
        end
        ...
        context 'descending order' do
          ...
          it 'returns products sorted by price with descending order' do
            expect(json_response['data'].count).to      eq Viauco::Product.not_discontinued.count
            expect(json_response['data'].pluck(:id)).to eq Viauco::Product.not_discontinued.joins(master: :prices).select("#{Viauco::Product.table_name}.*, #{Viauco::Price.table_name}.amount").distinct.order("#{Viauco::Price.table_name}.amount DESC").map(&:id).map(&:to_s)
          end
        end
      end
      ...
      context 'sorting by updated_at' do
        context 'ascending order' do
          ...
          it 'returns products sorted by updated_at' do
            expect(json_response['data'].count).to      eq Viauco::Product.not_discontinued.count
            expect(json_response['data'].pluck(:id)).to eq Viauco::Product.not_discontinued.order(:updated_at).pluck(:id).map(&:to_s)
          end
        end
        ...
        context 'descending order' do
          ...
          it 'returns products sorted by updated_at with descending order' do
            expect(json_response['data'].count).to      eq Viauco::Product.not_discontinued.count
            expect(json_response['data'].pluck(:id)).to eq Viauco::Product.not_discontinued.order(updated_at: :desc).pluck(:id).map(&:to_s)
          end
        end
      end
    end
    ...
    context 'paginate products' do
      context 'with specified pagination params' do
        ...
        it 'returns proper meta data' do
          ...
          expect(json_response['meta']['total_count']).to eq Viauco::Product.not_discontinued.count
        end
