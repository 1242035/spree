...
describe 'Taxons Spec', type: :request do
    ...
    context 'paginate taxons' do
      context 'with specified pagination params' do
        ...
        it 'returns proper meta data' do
          ...
          expect(json_response['meta']['total_count']).to eq Viauco::Taxon.count
        end
      ...
      context 'without specified pagination params' do
        ...
        it 'returns specified amount of taxons' do
          expect(json_response['data'].count).to eq Viauco::Taxon.count
        end
  ...
  describe 'taxons#show' do
    ...
    context 'by permalink' do
      before do
        get "/api/v2/storefront/taxons/#{Viauco::Taxon.first.permalink}"
      end

      it_behaves_like 'returns valid taxon resource JSON'

      it 'returns taxon by permalink' do
        expect(json_response['data']).to have_id(Viauco::Taxon.first.id.to_s)
        expect(json_response['data']).to have_attribute(:name).with_value(Viauco::Taxon.first.name)
      end
    end
  end
end
