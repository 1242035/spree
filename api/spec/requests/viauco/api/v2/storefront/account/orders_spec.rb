  ...
  describe 'orders#index' do
    context 'with option: include' do
      ...
      it 'returns included resource' do
        expect(json_response['included'].size).to eq Viauco::Order.count
    ...
    context 'with specified pagination params' do
      ...
      it 'returns proper meta data' do
        ...
        expect(json_response['meta']['total_count']).to eq Viauco::Order.count
      end
    ...
    context 'without specified pagination params' do
      ...
      it 'returns specified amount orders' do
        expect(json_response['data'].count).to eq Viauco::Order.count
      end

      it 'returns proper meta data' do
        expect(json_response['meta']['count']).to       eq Viauco::Order.count
        expect(json_response['meta']['total_count']).to eq Viauco::Order.count
      end
      ...
      context 'sorting by completed_at' do
        context 'ascending order' do
          ...
          it 'returns orders sorted by completed_at' do
            expect(json_response['data'].count).to eq Viauco::Order.count
            expect(json_response['data'].pluck(:id)).to eq Viauco::Order.select('*').order(completed_at: :desc).pluck(:id).map(&:to_s)
          end
        end
        ...
        context 'descending order' do
          ...
          it 'returns orders sorted by completed_at' do
            expect(json_response['data'].count).to eq Viauco::Order.count
            expect(json_response['data'].pluck(:id)).to eq Viauco::Order.select('*').order(completed_at: :asc).pluck(:id).map(&:to_s)
          end
        end
      end
    end
