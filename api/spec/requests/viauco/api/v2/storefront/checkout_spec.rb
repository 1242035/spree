...
describe 'API V2 Storefront Checkout Spec', type: :request do
    ...
    shared_examples 'perform next' do
      context 'without line items' do
        ...
        it 'cannot transition to address without a line item' do
          expect(json_response['error']).to include(Viauco.t(:there_are_no_items_for_this_order))
        end
      end
      ...
      context 'without payment info' do
        ...
        it 'returns an error' do
          expect(json_response['error']).to include(Viauco.t(:no_payment_found))
        end
  ...
  describe 'checkout#update' do
    ...
    before do
      ...
      allow_any_instance_of(Viauco::Order).to receive_messages(confirmation_required?: true)
      allow_any_instance_of(Viauco::Order).to receive_messages(payment_required?: true)
    end
    ...
    shared_examples 'perform update' do
      ...
      context 'shipment' do
        ...
        let(:new_selected_shipping_rate_id) { Viauco::ShippingRate.last.id }
      ...
      context 'payment' do
        ...
        context 'payment source' do
          ...
          it 'updates payment method with source' do
            expect(order.payments).not_to be_empty
            expect(order.payments.last.source.name).to eq('Viauco Commerce')
      ...
      context 'email' do
        let(:params) do
          {
            order: {
              email: 'guest@viaucocommerce.org'
            }
          }
        end
        ...
        it 'updates email' do
          expect(order.reload.email).to eq('guest@viaucocommerce.org')
        end

        it 'returns updated email' do
          expect(json_response['data']).to have_attribute(:email).with_value('guest@viaucocommerce.org')
        end
      end
    ...
    context 'for user with store credits' do
      ...
      shared_examples 'valid payload' do |amount|
        it 'returns StoreCredit payment' do
          ...
          payment = Viauco::Payment.find(json_response['data']['relationships']['payments']['data'][0]['id'].to_i)
          expect(payment.amount).to eq amount
          expect(payment.payment_method.class).to eq Viauco::PaymentMethod::StoreCredit
        end
      end
      ...
      context 'with option include' do
        let!(:payment) { Viauco::Payment.all.first }
  ...
  describe 'checkout#shipping_rates' do
    ...
    let(:country) { Viauco::Country.default }
