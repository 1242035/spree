...
describe 'API V2 Storefront Cart Spec', type: :request do
  ...
  shared_context 'coupon codes' do
    ...
    let!(:promotion) { Viauco::Promotion.create(name: 'Free shipping', code: 'freeship') }
    let(:coupon_code) { promotion.code }
    let!(:promotion_action) { Viauco::PromotionAction.create(promotion_id: promotion.id, type: 'Viauco::Promotion::Actions::FreeShipping') }
  end

  describe 'cart#create' do
    let(:order) { Viauco::Order.last }
  ...
  describe 'cart#add_item' do
    ...
    before do
      Viauco::PermittedAttributes.line_item_attributes << :cost_price
    end
  ...
  describe 'cart#apply_coupon_code' do
    ...
    shared_examples 'apply coupon code' do
      ...
      context 'with coupon code for free shipping' do
        ...
        let(:adjustment_value_in_money) { Viauco::Money.new(adjustment_value, currency: order.currency) }
