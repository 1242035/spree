  ...
  describe Api::V1::CheckoutsController, type: :controller do
      ...
      context 'ensure no double_render_error' do
        before do
          def controller.load_order(*)
            respond_with(@order, default_template: 'viauco/api/v1/orders/show', status: 200)
          end
        end
    ...
    context "PUT 'update'" do
      let(:order) do
        ...
        Viauco::ShippingRate.where(shipment_id: order.shipment_ids).delete_all
      ...
      context 'transitioning to delivery' do
        ...
        it 'can update addresses but not transition to delivery w/o shipping setup' do
          Viauco::ShippingMethod.destroy_all
          api_put :update,
                  id: order.to_param, order_token: order.token,
                  order: {
                    bill_address_attributes: address,
          ...
          expect(json_response['error']).to eq(I18n.t(:could_not_transition, scope: 'viauco.api.order'))
          expect(response.status).to eq(422)
        end
      ...
      it 'can transition from confirm to complete' do
        ...
        allow_any_instance_of(Viauco::Order).to receive_messages(payment_required?: false)
        api_put :update, id: order.to_param, order_token: order.token
      ...
      context 'as an admin' do
        ...
        it 'can assign a user to the order' do
          ...
          api_put :update, id: order.to_param, order_token: order.token,
                           order: { user_id: user.id, email: 'guest@viaucocommerce.org' }
          expect(response.status).to eq(200)
      ...
      it 'can assign an email to the order' do
        api_put :update, id: order.to_param, order_token: order.token,
                         order: { email: 'guest@viaucocommerce.org' }
    ...
    context "PUT 'next'" do
      ...
      it 'cannot transition to address without a line item' do
        ...
        order.update_column(:email, 'viauco@example.com')
        api_put :next, id: order.to_param, order_token: order.token
        expect(response.status).to eq(422)
        expect(json_response['errors']['base']).to include(Viauco.t(:there_are_no_items_for_this_order))
      end
      ...
      it 'cannot transition if any line_item becomes unavailable' do
        ...
        api_put :next, id: order.to_param, order_token: order.token
        expect(response.status).to eq(422)
        expect(json_response['error']).to match(Viauco.t(:insufficient_quantity, scope: [:api, :order]))
      end

      it 'doesnt advance payment state if order has no payment' do
        order.update_column(:state, 'payment')
        api_put :next, id: order.to_param, order_token: order.token, order: {}
        expect(json_response['errors']['base']).to include(Viauco.t(:no_payment_found))
      end
    ...
    context "PUT 'advance'" do
      ...
      it 'continues to advance advances an order while it can move forward' do
        expect_any_instance_of(Viauco::Order).to receive(:next).exactly(3).times.and_return(true, true, false)
