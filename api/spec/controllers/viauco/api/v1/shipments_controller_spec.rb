...
describe Viauco::Api::V1::ShipmentsController, type: :controller do
  render_views
  ...
  context 'as an admin' do
    ...
    describe 'POST #transfer_to_shipment' do
      ...
      context 'wrong quantity and shipment target' do
        ...
        it 'displays wrong target and negative quantity errors' do
          ...
          expect(json_response['exception']).to eq("#{Viauco.t(:shipment_transfer_errors_occured, scope: 'api')} \n#{Viauco.t(:negative_quantity, scope: 'api')}, \n#{Viauco.t(:wrong_shipment_target, scope: 'api')}")
        end
      end
    ...
    it 'can make a shipment ready' do
      allow_any_instance_of(Viauco::Order).to receive_messages(paid?: true, complete?: true)
      api_put :ready
      expect(json_response).to have_attributes(attributes)
    ...
    it 'cannot make a shipment ready if the order is unpaid' do
      allow_any_instance_of(Viauco::Order).to receive_messages(paid?: false)
      api_put :ready
    ...
    context 'for completed shipments' do
      ...
      it 'removes a variant from a shipment' do
        Viauco::Cart::AddItem.call(order: order, variant: variant, quantity: 2)
    ...
    context 'can transition a shipment from ready to ship' do
      before do
        ...
        allow_any_instance_of(Viauco::ShippingRate).to receive_messages(cost: 5)
      end
