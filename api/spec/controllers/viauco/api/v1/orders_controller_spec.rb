...
module Viauco
  describe Api::V1::OrdersController, type: :controller do
    ...
    let(:current_api_user) do
      user = Viauco.user_class.new(email: 'viauco@example.com')
    ...
    context 'the current api user is not persisted' do
      let(:current_api_user) { Viauco.user_class.new }
    ...
    describe 'current' do
      ...
      context 'multiple incomplete orders exist' do
        it 'returns the latest incomplete order' do
          Timecop.scale(3600) do
            new_order = Viauco::Order.create! user: order.user
    ...
    it "can not view someone else's order" do
      allow_any_instance_of(Order).to receive_messages user: stub_model(Viauco::LegacyUser)
      api_get :show, id: order.to_param
      assert_unauthorized!
    end
    ...
    it 'can view an order if the token is passed in header' do
      request.headers['X-Viauco-Order-Token'] = order.token
      api_get :show, id: order.to_param
      expect(response.status).to eq(200)
    end

    context 'with BarAbility registered' do
      before { Viauco::Ability.register_ability(::BarAbility) }

      after  { Viauco::Ability.remove_ability(::BarAbility) }

      it 'can view an order' do
        user = mock_model(Viauco::LegacyUser)
        allow(user).to receive_message_chain(:viauco_roles, :pluck).and_return(['bar'])
        allow(user).to receive(:has_viauco_role?).with('bar').and_return(true)
        allow(user).to receive(:has_viauco_role?).with('admin').and_return(false)
        allow(Viauco.user_class).to receive_messages find_by: user
        api_get :show, id: order.to_param
        expect(response.status).to eq(200)
      end
    end
    ...
    it 'assigns email when creating a new order' do
      api_post :create, order: { email: 'guest@viaucocommerce.org' }
      expect(json_response['email']).not_to eq controller.current_api_user
      expect(json_response['email']).to eq 'guest@viaucocommerce.org'
    end
    ...
    context 'admin user imports order' do
      before do
        allow(current_api_user).to receive_messages has_viauco_role?: true
        allow(current_api_user).to receive_message_chain :viauco_roles, pluck: ['admin']
      end
      ...
      context 'line_items hash not present in request' do
        it 'responds successfully' do
          api_put :update, id: order.to_param, order: {
            email: 'hublock@viaucocommerce.com'
          }
    ...
    context 'as an admin' do
      ...
      context 'with no orders' do
        before { Viauco::Order.delete_all }
      ...
      context 'search' do
        before do
          create(:order)
          Viauco::Order.last.update_attribute(:email, 'viauco@viaucocommerce.com')
        end

        let(:expected_result) { Viauco::Order.last }

        it 'can query the results through a parameter' do
          api_get :index, q: { email_cont: 'viauco' }
          expect(json_response['orders'].count).to eq(1)
          expect(json_response['orders'].first).to have_attributes(attributes)
    ...
    context 'PUT remove_coupon_code' do
      ...
      context 'order with discount promotion' do
        ...
        it 'removes all order adjustments from order and return status 200' do
          ...
          expect(response.status).to eq 200
          expect(json_response['success']).to eq Viauco.t('adjustments_deleted')
