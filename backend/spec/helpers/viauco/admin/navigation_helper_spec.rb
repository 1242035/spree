...
describe Viauco::Admin::NavigationHelper, type: :helper do
  before do
    # `viauco` route helper is not accessible in `type: :helper` hence extending it explicitly
    # https://github.com/rspec/rspec-rails/issues/1626
    helper.extend Viauco::TestingSupport::UrlHelpers
  end
  ...
  describe '#klass_for' do
    it 'returns correct klass for Viauco model' do
      expect(klass_for(:products)).to eq(Viauco::Product)
      expect(klass_for(:product_properties)).to eq(Viauco::ProductProperty)
    end

    it 'returns correct klass for non-viauco model' do
      class MyUser
      end
    ...
    it 'returns correct namespaced klass for non-viauco model' do
      module My
        class User
        end
      end
