    ...
    describe PaymentsController, type: :controller do
      ...
      context 'with a valid credit card' do
        ...
        it 'processes payment correctly' do
          ...
          expect(response).to redirect_to(viauco.admin_order_payments_path(order))
          expect(order.reload.state).to eq('complete')
        end

        # Regression for #4768
        it 'doesnt process the same payment twice' do
          expect(Viauco::LogEntry.where(source: order.payments.first).count).to eq(1)
        end
      end
      ...
      context 'order has billing address' do
        ...
        context 'order does not have payments' do
          it 'redirect to new payments page' do
            ...
            expect(response).to redirect_to(viauco.new_admin_order_payment_path(order))
          end
        end
      ...
      context 'order does not have a billing address' do
        ...
        it 'redirects to the customer details page' do
          get :index, params: { amount: 100, order_id: order.number }
          expect(response).to redirect_to(viauco.edit_admin_order_customer_path(order))
        end
      end
    end
  end
