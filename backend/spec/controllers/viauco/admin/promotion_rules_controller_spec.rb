...
describe Viauco::Admin::PromotionRulesController, type: :controller do
  stub_authorization!

  let!(:promotion) { create(:promotion) }

  it 'can create a promotion rule of a valid type' do
    post :create, params: { promotion_id: promotion.id, promotion_rule: { type: 'Viauco::Promotion::Rules::Product' } }
  ...
  it 'can not create a promotion rule of an invalid type' do
    post :create, params: { promotion_id: promotion.id, promotion_rule: { type: 'Viauco::InvalidType' } }
    expect(response).to be_redirect
    expect(response).to redirect_to viauco.edit_admin_promotion_path(promotion)
