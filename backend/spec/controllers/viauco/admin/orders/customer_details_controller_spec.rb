...
describe Viauco::Admin::Orders::CustomerDetailsController, type: :controller do
  context 'with authorization' do
    stub_authorization!

    let(:user) { mock_model(Viauco.user_class) }
    ...
    describe '#update' do
          ...
          context 'with correct method flow' do
            ...
            it 'does refresh the shipment rates with all shipping methods' do
              expect(order).to receive(:refresh_shipment_rates).
                with(Viauco::ShippingMethod::DISPLAY_ON_BACK_END)
            end
      ...
      context 'without using guest checkout' do
        ...
        context 'having invalid parameters' do
          before do
            allow(Viauco.user_class).to receive(:find_by).and_return(false)
            allow(controller).to receive(:guest_checkout?).and_return(false)
          end
        ...
        describe '#load_user' do
          context 'having valid parameters' do
            ...
            context 'with correct method flow' do
              ...
              it { expect(Viauco.user_class).to receive(:find_by).with(id: user.id.to_s).and_return(user) }
            end
          end
          ...
          context 'with invalid parameters' do
            ...
            context 'with correct method flow' do
              ...
              it { expect(Viauco.user_class).to receive(:find_by).with(id: user.id.to_s).and_return(nil) }
              it 'expects user class to receive find_by with email' do
                expect(Viauco.user_class).to receive(:find_by).
