...
describe Viauco::Admin::ProductsController, type: :controller do
  stub_authorization!

  context '#index' do
    let(:ability_user) { stub_model(Viauco::LegacyUser, has_viauco_role?: true) }
  ...
  describe '#destroy' do
    let(:product) { mock_model(Viauco::Product) }
    ...
    context 'will successfully destroy product' do
      before do
        allow(Viauco::Product).to receive(:friendly).and_return(products)
      ...
      describe 'expects to receive' do
        ...
        it { expect(Viauco::Product).to receive(:friendly).and_return(products) }
      ...
      describe 'response' do
        ...
        it { expect(flash[:success]).to eq(Viauco.t('notice_messages.product_deleted')) }
      end
    end
    ...
    context 'will not successfully destroy product' do
      ...
      describe 'response' do
        ...
        it 'set flash error' do
          expected_error = Viauco.t('notice_messages.product_not_deleted', error: error_msg)
  ...
  describe '#clone' do
    ...
    context 'will successfully clone product' do
      ...
      describe 'response' do
        ...
        it { expect(flash[:success]).to eq(Viauco.t('notice_messages.product_cloned')) }
      end
    end
    ...
    context 'will not successfully clone product' do
      ...
      describe 'response' do
        ...
        it 'set flash error' do
          expected_error = Viauco.t('notice_messages.product_not_cloned', error: 'Validation failed: Sku has already been taken')
          expect(flash[:error]).to eq(expected_error)
        end
      end
    end
  ...
  context 'stock' do
    ...
    it 'restricts stock location based on accessible attributes' do
      expect(Viauco::StockLocation).to receive(:accessible_by).and_return([])
