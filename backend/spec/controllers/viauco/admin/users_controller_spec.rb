...
describe Viauco::Admin::UsersController, type: :controller do
  let(:user) { create(:user) }
  let(:mock_user) { mock_model Viauco.user_class }

  before do
    allow(controller).to receive_messages viauco_current_user: user
    user.viauco_roles.clear
    stub_const('Viauco::User', user.class)
  end
  ...
  context '#show' do
    ...
    it 'redirects to edit' do
      ...
      expect(response).to redirect_to viauco.edit_admin_user_path(user)
    end
  end
  ...
  context '#authorize_admin' do
    ...
    it "allows admins to update a user's API key" do
      user.viauco_roles << Viauco::Role.find_or_create_by(name: 'admin')
      expect(mock_user).to receive(:generate_viauco_api_key!).and_return(true)
      put :generate_api_key, params: { id: mock_user.id }
      expect(response).to redirect_to(viauco.edit_admin_user_path(mock_user))
    end

    it "allows admins to clear a user's API key" do
      user.viauco_roles << Viauco::Role.find_or_create_by(name: 'admin')
      expect(mock_user).to receive(:clear_viauco_api_key!).and_return(true)
  ...
  describe '#create' do
    ...
    it 'can create a shipping_address' do
      expect(Viauco.user_class).to receive(:new).with(ActionController::Parameters.new(
    ...
    it 'redirects to user edit page' do
      ...
      expect(response).to redirect_to(viauco.edit_admin_user_path(assigns[:user]))
    end
  end
  ...
  describe '#update' do
    ...
    it 'allows updating without password resetting' do
      ...
      put :update, params: { id: mock_user.id, user: { password: '', password_confirmation: '', email: 'viauco@example.com' } }
    end

    it 'redirects to user edit page' do
      expect(mock_user).to receive(:update).with(hash_not_including(email: '')).and_return(true)
      put :update, params: { id: mock_user.id, user: { email: 'viauco@example.com' } }
      expect(response).to redirect_to(viauco.edit_admin_user_path(mock_user))
    end
  ...
  describe '#orders' do
    ...
    it 'assigns a ransack search for Viauco::Order' do
      get :orders, params: { id: user.id }
      expect(assigns[:search]).to be_a Ransack::Search
      expect(assigns[:search].klass).to eq Viauco::Order
    end
  end
...
def use_mock_user
  ...
  allow(Viauco.user_class).to receive(:find).with(mock_user.id.to_s).and_return(mock_user)
  allow(Viauco.user_class).to receive(:new).and_return(mock_user)
end
