...
module Viauco
  ...
  describe Admin::PaymentMethodsController, type: :controller do
    ...
    it 'does not clear password on update' do
      ...
      expect(response).to redirect_to(viauco.edit_admin_payment_method_path(payment_method))
    ...
    context 'tries to save invalid payment' do
      it "doesn't break, responds nicely" do
        expect do
          post :create, params: { payment_method: { name: '', type: 'Viauco::Gateway::Bogus' } }
    ...
    it 'can create a payment method of a valid type' do
      expect do
        post :create, params: { payment_method: { name: 'Test Method', type: 'Viauco::Gateway::Bogus' } }
      end.to change(Viauco::PaymentMethod, :count).by(1)

      expect(response).to be_redirect
      expect(response).to redirect_to viauco.edit_admin_payment_method_path(assigns(:payment_method))
    end

    it 'can not create a payment method of an invalid type' do
      expect do
        post :create, params: { payment_method: { name: 'Invalid Payment Method', type: 'Viauco::InvalidType' } }
      end.to change(Viauco::PaymentMethod, :count).by(0)

      expect(response).to be_redirect
      expect(response).to redirect_to viauco.new_admin_payment_method_path
    end
  end
end
