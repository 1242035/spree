...
describe Viauco::Admin::PromotionsController, type: :controller do
  stub_authorization!
  ...
  context '#clone' do
    context 'cloning valid promotion' do
      ...
      it 'creates a copy of promotion' do
        expect { subject }.to change { Viauco::Promotion.count }.by(1)
      end

      it 'creates a copy of promotion with changed fields' do
        subject
        new_promo = Viauco::Promotion.last
    ...
    context 'cloning invalid promotion' do
      ...
      it 'doesnt create a copy of promotion' do
        expect { subject }.not_to(change { Viauco::Promotion.count })
      end

      it 'returns error' do
        subject
        expected_error = Viauco.t('promotion_not_cloned', error: assigns(:new_promo).errors.full_messages.to_sentence)
