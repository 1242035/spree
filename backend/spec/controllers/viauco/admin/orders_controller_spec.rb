...
class OrderSpecificAbility
  ...
  def initialize(_user)
    can [:admin, :manage], Viauco::Order, number: 'R987654321'
  end
end

describe Viauco::Admin::OrdersController, type: :controller do
  context 'with authorization' do
    ...
    let(:order) do
      mock_model(
        ...
        billing_address: mock_model(Viauco::Address)
      )
    end

    let(:adjustments) { double('adjustments') }
    let(:display_value) { Viauco::ShippingMethod::DISPLAY_ON_BACK_END }

    before do
      request.env['HTTP_REFERER'] = 'http://localhost:3000'
      # ensure no respond_overrides are in effect
      allow(Viauco::Order).to receive_message_chain(:includes, find_by!: order)
    end

    context '#approve' do
      it 'approves an order' do
        expect(order).to receive(:approved_by).with(controller.try_viauco_current_user)
        put :approve, params: { id: order.number }
        expect(flash[:success]).to eq Viauco.t(:order_approved)
      end
    end

    context '#cancel' do
      it 'cancels an order' do
        expect(order).to receive(:canceled_by).with(controller.try_viauco_current_user)
        put :cancel, params: { id: order.number }
        expect(flash[:success]).to eq Viauco.t(:order_canceled)
      end
    end
    ...
    context '#resume' do
      it 'resumes an order' do
        ...
        expect(flash[:success]).to eq Viauco.t(:order_resumed)
      end
    end
    ...
    context '#new' do
      it 'a new order has the current user assigned as a creator' do
        get :new
        expect(assigns[:order].created_by).to eq(controller.try_viauco_current_user)
      end
    end
    ...
    context 'search' do
      ...
      def send_request
        get :index, params: {
          q: {
                line_items_variant_id_in: Viauco::Order.first.variants.map(&:id)
            }
          }
      end
      ...
      it 'preloads users' do
        expect(Viauco::Order).to receive(:preload).with(:user).and_return(Viauco::Order.all)
        send_request
      end
    end
  ...
  context '#authorize_admin' do
    ...
    before do
      allow(Viauco::Order).to receive_messages find: order
      allow(controller).to receive_messages viauco_current_user: user
    end
    ...
    it 'denies access to users with an bar role' do
      with_ability(BarAbility) do
        ...
        allow(order).to receive(:user).and_return Viauco.user_class.new
        allow(order).to receive(:token).and_return nil
        user.viauco_roles.clear
        user.viauco_roles << Viauco::Role.find_or_create_by(name: 'bar')
    ...
    it 'denies access to users without an admin role' do
      allow(user).to receive_messages has_viauco_role?: false
      post :index
      expect(response).to redirect_to(viauco.forbidden_path)
    end

    it 'denies access to not signed in users' do
      allow(controller).to receive_messages viauco_current_user: nil
      get :index
      expect(response).to redirect_to('/')
    end
    ...
    it 'restricts returned order(s) on index when using OrderSpecificAbility' do
      ...
      expect(Viauco::Order.complete.count).to eq 4
      ...
      with_ability(OrderSpecificAbility) do
        ...
        expect(Viauco::Order.accessible_by(Viauco::Ability.new(user), :index).pluck(:number)).to eq [number]
      end
    end
  end
