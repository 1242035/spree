...
describe 'Store credits admin', type: :feature do
  ...
  before do
    allow(Viauco.user_class).to receive(:find_by).and_return(store_credit.user)
  end
  ...
  describe 'visiting the store credits page' do
    ...
    it 'is on the store credits page' do
      ...
      expect(page).to have_current_path(viauco.admin_user_store_credits_path(store_credit.user))

      store_credit_table = page.find('table', match: :first)
      expect(store_credit_table).to have_css('tr').once
      expect(store_credit_table).to have_content(Viauco::Money.new(store_credit.amount, currency: store_credit.currency).to_s)
      expect(store_credit_table).to have_content(Viauco::Money.new(store_credit.amount_used, currency: store_credit.currency).to_s)
  ...
  describe 'creating store credit' do
    before do
      ...
      click_link store_credit.user.email
      click_link 'Store Credits'
      allow_any_instance_of(Viauco::Admin::StoreCreditsController).to receive(:try_viauco_current_user).and_return(admin_user)
    end
    ...
    describe 'with default currency' do
      it 'creates store credit and associate it with the user' do
        ...
        expect(Viauco::StoreCredit.count).to eq 2
      end
    end
    ...
    describe 'with selected currency' do
      it 'creates store credit and associate it with the user' do
        ...
        expect(store_credit_table).to have_css('tr').twice
        expect(Viauco::StoreCredit.count).to eq 2
        expect(Viauco::StoreCredit.last.currency).to eq 'EUR'
  ...
  describe 'updating store credit' do
    ...
    it 'creates store credit and associate it with the user' do
      ...
      expect(store_credit_table).to have_content(Viauco::Money.new(updated_amount, currency: store_credit.currency).to_s)
  ...
  describe 'deleting store credit', js: true do
    ...
    it 'updates store credit in lifetime stats' do
      ...
      expect(page).to have_css('#user-lifetime-stats #store_credit', text: Viauco::Money.new(0).to_s)
    end
  end
  ...
  describe 'non-existent user' do
    before do
      ...
      allow(Viauco.user_class).to receive(:find_by).and_return(nil)
      click_link 'Store Credits'
      allow_any_instance_of(Viauco::Admin::StoreCreditsController).to receive(:try_viauco_current_user).and_return(admin_user)
    end

    it 'displays flash withe error' do
      expect(page).to have_content(Viauco.t(:user_not_found))
    end
  end
end
