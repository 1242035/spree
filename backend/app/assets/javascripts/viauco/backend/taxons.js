...
$(function () {
  ...
  taxonProducts.on('sortstop', function (event, ui) {
    return $.ajax({
      url: Viauco.routes.classifications_api,
      method: 'PUT',
      dataType: 'json',
      data: {
  ...
  if (taxonId.length > 0) {
    taxonId.select2({
      ...
      placeholder: Viauco.translations.find_a_taxon,
      ajax: {
        url: Viauco.routes.taxons_api,
        datatype: 'json',
  ...
  taxonId.on('change', function (e) {
    ...
    $.ajax({
      url: Viauco.routes.taxon_products_api,
      data: {
        id: e.val,
        token: Viauco.api_key
      }
      ...
      if (data.products.length === 0) {
        return $('#taxon_products').html('<div class="alert alert-info">' + Viauco.translations.no_results + '</div>')
      } else {
        ref = data.products
        results = []
  ...
  taxonProducts.on('click', '.js-delete-product', function (e) {
    ...
    $.ajax({
      url: Viauco.routes.products_api + '/' + productId,
      data: {
        product: {
          taxon_ids: taxonIds
        },
