...
$(document).ready(function () {
  ...
  $('a.delete-item').click(function (event) {
    if (confirm(Viauco.translations.are_you_sure_delete)) {
      ...
      var url = Viauco.routes.shipments_api + '/' + shipmentNumber + '/remove'
  ...
  $('[data-hook=admin_shipment_form] a.ship').on('click', function () {
    ...
    var url = Viauco.url(Viauco.routes.shipments_api + '/' + shipmentNumber + '/ship.json')
    $.ajax({
      type: 'PUT',
      url: url,
      data: {
  ...
  $('[data-hook=admin_shipment_form] a.save-method').on('click', function (event) {
    ...
    var url = Viauco.url(Viauco.routes.shipments_api + '/' + shipmentNumber + '.json')
  ...
  $('[data-hook=admin_shipment_form] a.save-tracking').on('click', function (event) {
    ...
    $.ajax({
      ...
      if (data.tracking) {
        show.find('.tracking-value').html($('<strong>').html(Viauco.translations.tracking + ': ')).append(createTrackingValueContent(data))
      } else {
        show.find('.tracking-value').html(Viauco.translations.no_tracking_present)
      }
    })
  })
})
...
function adjustShipmentItems(shipmentNumber, variantId, quantity) {
  ...
  var url = Viauco.routes.shipments_api + '/' + shipmentNumber
...
function startItemSplit(event) {
  ...
  $.ajax({
    type: 'GET',
    async: false,
    url: Viauco.url(Viauco.routes.variants_api),
    data: {
      q: {
        'id_eq': variantId
      },
  ...
  $('#item_stock_location').select2({ width: 'resolve', placeholder: Viauco.translations.item_stock_placeholder })
}
  ...
  if (stockLocationId != 'new_shipment') {
    if (newShipment !== undefined) {
      ...
      $.ajax({
        type: 'POST',
        async: false,
        url: Viauco.url(Viauco.routes.shipments_api + '/transfer_to_location'),
        data: {
      ...
      $.ajax({
        type: 'POST',
        async: false,
        url: Viauco.url(Viauco.routes.shipments_api + '/transfer_to_shipment'),
        data: {
          original_shipment_number: originalShipmentNumber,
...
function addVariantFromStockLocation(event) {
  ...
  if (shipment === undefined) {
    $.ajax({
      ...
      url: Viauco.url(Viauco.routes.shipments_api + '?shipment[order_id]=' + order_number),
      data: {
        variant_id: variantId,
        quantity: quantity,
        stock_location_id: stockLocationId,
