...
function handleAjaxError (lastRollback) {
  ...
  show_flash('error', '<strong>' + Viauco.translations.server_error + '</strong><br />' + Viauco.translations.taxonomy_tree_error)
}
...
function handleMove (e, data) {
  ...
  var url = Viauco.url(base_url).clone()
...
root.setup_taxonomy_tree = function (taxonomyId) {
  ...
  if (taxonomyId !== void 0) {
    ...
    root.base_url = Viauco.url(Viauco.routes.taxonomy_taxons_path)
    $.ajax({
      url: Viauco.url(base_url.path().replace('/taxons', '/jstree')).toString(),
      data: {
        token: Viauco.api_key
      }
      ...
      var conf = {
        json_data: {
          ...
          ajax: {
            url: function (e) {
              return Viauco.url(base_url.path() + '/' + e.prop('id') + '/jstree' + '?token=' + Viauco.api_key).toString()
            }
          }
        },
        themes: {
          theme: 'viauco',
          url: Viauco.url(Viauco.routes.jstree_theme_path)
        },
        strings: {
          new_node: Viauco.translations.new_taxon,
          loading: Viauco.translations.loading + '...'
        },
        crrm: {
          move: {
