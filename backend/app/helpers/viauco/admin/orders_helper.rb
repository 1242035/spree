    ...
    module OrdersHelper
      ...
      def event_links(order, events)
        ...
        events.each do |event|
          ...
          label = Viauco.t(event, scope: 'admin.order.events', default: Viauco.t(event))
          links << button_link_to(
            label.capitalize,
            [event, :admin, order],
            method: :put,
            icon: event.to_s,
            data: { confirm: Viauco.t(:order_sure_want_to, event: label) }
          )
        end
      ...
      def line_item_shipment_price(line_item, quantity)
        Viauco::Money.new(line_item.price * quantity, currency: line_item.currency)
      end
