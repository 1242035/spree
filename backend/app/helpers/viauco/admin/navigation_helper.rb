  ...
  module Admin
    module NavigationHelper
      ...
      def tab(*args)
        ...
        destination_url = options[:url] || viauco.send("#{options[:route]}_path")
        titleized_label = Viauco.t(options[:label], default: options[:label], scope: [:admin, :tab]).titleize
        ...
        selected = if options[:match_path].is_a? Regexp
                     ...
                     request.fullpath.starts_with?("#{viauco.admin_path}#{options[:match_path]}")
                   else
      ...
      def main_menu_tree(text, icon: nil, sub_menu: nil, url: '#')
        content_tag :li, class: 'sidebar-menu-item d-block w-100' do
          main_menu_item(text, url: url, icon: icon) +
            render(partial: "viauco/admin/shared/sub_menu/#{sub_menu}")
        end
      end
      ...
      def per_page_dropdown
        ...
        if @products && per_page_default = Viauco::Config.admin_products_per_page
          per_page_options = []
          5.times do |amount|
            per_page_options << (amount + 1) * Viauco::Config.admin_products_per_page
          end
        else
          per_page_default = Viauco::Config.admin_orders_per_page
      ...
      # fixes https://github.com/viauco/viauco/issues/6888
      ...
      #
      # Example :
      # :products returns Viauco::Product
      ...
      def klass_for(name)
        ...
        ["Viauco::#{model_name.classify}", model_name.classify, model_name.tr('_', '/').classify].find(&:safe_constantize).try(:safe_constantize)
      end

      def link_to_clone(resource, options = {})
        options[:data] = { action: 'clone', 'original-title': Viauco.t(:clone) }
      ...
      def link_to_edit(resource, options = {})
        ...
        link_to_with_icon('edit', Viauco.t(:edit), url, options)
      end
      ...
      def link_to_delete(resource, options = {})
        ...
        name = options[:name] || Viauco.t(:delete)
        options[:class] = 'btn btn-danger btn-sm delete-resource'
        options[:data] = { confirm: Viauco.t(:are_you_sure), action: 'remove' }
      ...
      def button(text, icon_name = nil, button_type = 'submit', options = {})
        ...
        button_tag(
          ...
          options.merge(
            ...
            'data-disable-with' => "#{Viauco.t(:saving)}..."
          )
        )
      end
