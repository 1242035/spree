  ...
  module Admin
    module BaseHelper
      def flash_alert(flash)
        if flash.present?
          close_button = button_tag(class: 'close', 'data-dismiss' => 'alert', 'aria-label' => Viauco.t(:close)) do
      ...
      def datepicker_field_value(date)
        unless date.blank?
          l(date, format: Viauco.t('date_picker.format', default: '%Y/%m/%d'))
        end
      end
      ...
      def preference_fields(object, form)
        ...
        fields = object.preferences.keys.map do |key|
          if object.has_preference?(key)
            form.label("preferred_#{key}", Viauco.t(key) + ': ') +
      ...
      def link_to_icon_remove_fields(form)
        ...
        link_to_with_icon('delete', '', url,
                          class: 'viauco_remove_fields btn btn-sm btn-danger',
                          data: {
                            action: 'remove'
                          },
                          title: Viauco.t(:remove),
                          no_text: true
                         ) + form.hidden_field(:_destroy)
      end

      def viauco_dom_id(record)
        dom_id(record, 'viauco')
      end
