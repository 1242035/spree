class Viauco::Admin::PromotionRulesController < Viauco::Admin::BaseController
  ...
  def create
    ...
    if @promotion_rule.save
      flash[:success] = Viauco.t(:successfully_created, resource: Viauco.t(:promotion_rule))
    end
  ...
  def destroy
    ...
    if @promotion_rule.destroy
      flash[:success] = Viauco.t(:successfully_removed, resource: Viauco.t(:promotion_rule))
    end
    respond_to do |format|
      format.html { redirect_to viauco.edit_admin_promotion_path(@promotion) }
      format.js   { render layout: false }
  ...
  def validate_promotion_rule_type
    ...
    promotion_rule_types = Rails.application.config.viauco.promotions.rules
    @promotion_rule_type = promotion_rule_types.detect do |klass|
      klass.name == requested_type
    end
    unless @promotion_rule_type
      flash[:error] = Viauco.t(:invalid_promotion_rule)
      respond_to do |format|
        format.html { redirect_to viauco.edit_admin_promotion_path(@promotion) }
        format.js   { render layout: false }
      end
