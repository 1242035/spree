module Viauco
  module Admin
    class CustomerReturnsController < ResourceController
      ...
      def build_resource
        Viauco::CustomerReturn.new
      end

      def find_resource
        Viauco::CustomerReturn.accessible_by(current_ability, :show).find(params[:id])
      end

      def collection
        parent # trigger loading the order
        @collection ||= Viauco::ReturnItem.
      ...
      def build_return_items_from_params
        ...
        @customer_return.return_items = return_items_params.map do |item_params|
          ...
          return_item = item_params[:id] ? Viauco::ReturnItem.find(item_params[:id]) : Viauco::ReturnItem.new
