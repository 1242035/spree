module Viauco
  module Admin
    ...
    class StoreCreditsController < Viauco::Admin::BaseController
      ...
      def create
        @store_credit = @user.store_credits.build(
          permitted_store_credit_params.merge(
            created_by: try_viauco_current_user,
            action_originator: try_viauco_current_user
          )
        )
        ...
        if @store_credit.save
          ...
          load_categories
          flash[:error] = Viauco.t('store_credit.errors.unable_to_create')
          render :new
        end
      end
      ...
      def update
        ...
        @store_credit.created_by = try_viauco_current_user
        ...
        if @store_credit.save
          ...
          load_categories
          flash[:error] = Viauco.t('store_credit.errors.unable_to_update')
          render :edit
        end
      end
      ...
      def destroy
        ...
        if @store_credit.destroy
          ...
          render plain: Viauco.t('store_credit.errors.unable_to_delete'), status: :unprocessable_entity
        end
      end
      ...
      def load_user
        @user = Viauco.user_class.find_by(id: params[:user_id])

        unless @user
          flash[:error] = Viauco.t(:user_not_found)
          redirect_to admin_path
        end
      end

      def load_categories
        @credit_categories = Viauco::StoreCreditCategory.order(:name)
      end

      def load_store_credit
        @store_credit = Viauco::StoreCredit.find_by(id: params[:id]) || Viauco::StoreCredit.new
      end

      def ensure_unused_store_credit
        unless @store_credit.amount_used.zero?
          raise StoreCreditError, Viauco.t('store_credit.errors.cannot_change_used_store_credit')
        end
      end
    end
  end
