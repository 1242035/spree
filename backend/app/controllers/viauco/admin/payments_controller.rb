module Viauco
  module Admin
    class PaymentsController < Viauco::Admin::BaseController
      ...
      def create
        ...
        begin
          if @payment_method.store_credit?
            Viauco::Dependencies.checkout_add_store_credit_service.constantize.call(order: @order)
          ...
          if payments && (saved_payments = payments.select &:persisted?).any?
            ...
            flash[:error] = Viauco.t(:payment_could_not_be_created)
            render :new
          end
        rescue Viauco::Core::GatewayError => e
          invoke_callbacks(:create, :fails)
      ...
      def fire
        ...
        if @payment.send("#{event}!")
          flash[:success] = Viauco.t(:payment_updated)
        else
          flash[:error] = Viauco.t(:cannot_perform_operation)
        end
      rescue Viauco::Core::GatewayError => ge
