...
describe 'Checkout', type: :feature, inaccessible: true, js: true do
  ...
  context 'visitor makes checkout as guest without registration' do
    ...
    context 'defaults to use billing address' do
      before do
        add_mug_to_cart
        Viauco::Order.last.update_column(:email, 'test@example.com')
    ...
    context 'persists state when on address page' do
      ...
      specify do
        expect(Viauco::Order.count).to eq 1
        expect(Viauco::Order.last.state).to eq 'address'
      end
    end
  ...
  context "doesn't allow bad credit card numbers" do
    before do
      ...
      allow_any_instance_of(Viauco::CheckoutController).to receive_messages(current_order: order)
    ...
    it 'redirects to payment page', inaccessible: true, js: true do
      visit viauco.checkout_state_path(:payment)
      click_button 'Save and Continue'
      choose 'Credit Card'
  ...
  context 'when Viauco::Config[:always_include_confirm_step] is true' do
    before do
  ...
  context 'and likes to double click buttons' do
    ...
    it 'prevents double clicking the confirm button on checkout', js: true do
      ...
      visit viauco.checkout_state_path(:confirm)
  ...
  context 'when several payment methods are available', js: true do
    ...
    before do
      ...
      allow_any_instance_of(Viauco::CheckoutController).to receive_messages(current_order: order)
      allow_any_instance_of(Viauco::CheckoutController).to receive_messages(try_viauco_current_user: order.user)
  ...
  context 'user has payment sources', js: true do
    ...
    it 'selects first source available and customer moves on' do
      ...
      expect { click_on 'Save and Continue' }.not_to change { Viauco::CreditCard.count }

      click_on 'Place Order'
      expect(page).to have_current_path(viauco.order_path(Viauco::Order.last))
    end

    it 'allows user to enter a new source' do
      choose 'use_existing_card_no'

      delayed_fill_in 'Name on card', 'Viauco Commerce'
      delayed_fill_in 'Card Number',  '4111111111111111'
      delayed_fill_in 'card_expiry',  '04 / 20'
      delayed_fill_in 'Card Code',    '123'

      expect { click_on 'Save and Continue' }.to change { Viauco::CreditCard.count }.by 1

      click_on 'Place Order'

      expect(page).to have_content(Viauco.t(:thank_you_for_your_order).gsub(/[[:space:]]+/, ' '))
      expect(page).to have_current_path(viauco.order_path(Viauco::Order.last))
    end
  end
  ...
  context 'goes back from payment to add another item', js: true do
    ...
    it 'transit nicely through checkout steps again' do
      ...
      click_on 'Save and Continue'
      expect(page).to have_current_path(viauco.checkout_state_path('payment'))
  ...
  context 'from payment step customer goes back to cart', js: true do
    ...
    context 'and updates line item quantity and try to reach payment page' do
      ...
      it 'updates shipments properly through step address -> delivery transitions' do
        ...
        expect(Viauco::InventoryUnit.count).to eq 1
        expect(Viauco::InventoryUnit.first.quantity).to eq cart_quantity
      end
    end
    ...
    context 'and adds new product to cart and try to reach payment page' do
      ...
      it 'updates shipments properly through step address -> delivery transitions' do
        ...
        expect(Viauco::InventoryUnit.count).to eq 2
      end
    end
  end
  ...
  context 'if multiple coupon promotions applied' do
    let(:promotion) { Viauco::Promotion.create(name: 'Order Promotion', code: 'o_promotion') }
    let(:calculator) { Viauco::Calculator::FlatPercentItemTotal.create(preferred_flat_percent: '90') }
    let(:action) { Viauco::Promotion::Actions::CreateAdjustment.create(calculator: calculator) }

    let(:promotion_2) { Viauco::Promotion.create(name: 'Line Item Promotion', code: 'li_promotion') }
    let(:calculator_2) { Viauco::Calculator::FlatRate.create(preferred_amount: '1000') }
    let(:action_2) { Viauco::Promotion::Actions::CreateItemAdjustments.create(calculator: calculator_2) }
    ...
    it "totals aren't negative" do
      ...
      expect(Viauco::Order.last.total.to_f).to eq 0.0
    end
  end

  context 'if coupon promotion, submits coupon along with payment', js: true do
    let!(:promotion) { Viauco::Promotion.create(name: 'Huhuhu', code: 'huhu') }
    let!(:calculator) { Viauco::Calculator::FlatPercentItemTotal.create(preferred_flat_percent: '10') }
    let!(:action) { Viauco::Promotion::Actions::CreateItemAdjustments.create(calculator: calculator) }
    ...
    it 'makes sure payment reflects order total with discounts' do
      ...
      expect(page).to have_content(promotion.name)
      expect(Viauco::Payment.first.amount.to_f).to eq Viauco::Order.last.total.to_f
    end
    ...
    context 'invalid coupon' do
      it 'doesnt create a payment record' do
        ...
        expect(Viauco::Payment.count).to eq 0
    ...
    context 'the promotion makes order free (downgrade it total to 0.0)' do
      let(:promotion2) { Viauco::Promotion.create(name: 'test-7450', code: 'test-7450') }
      let(:calculator2) do
        Viauco::Calculator::FlatRate.create(preferences: { currency: 'USD', amount: BigDecimal('99999') })
      end
      let(:action2) { Viauco::Promotion::Actions::CreateItemAdjustments.create(calculator: calculator2) }
      ...
      context 'user choose to pay by check' do
        it 'move user to complete checkout step' do
          ...
          expect(Viauco::Order.last.total.to_f).to eq(0)
          expect(page).to have_current_path(viauco.order_path(Viauco::Order.last))
        end
      end
      ...
      context 'user choose to pay by card' do
        ...
        it 'move user to confirmation checkout step' do
          fill_in 'Name on card', with: 'Viauco Commerce'
          ...
          expect(page).to have_content(promotion2.name)
          expect(Viauco::Order.last.total.to_f).to eq(0)
          expect(page).to have_current_path(viauco.checkout_state_path('confirm'))
        end
      end
    end
  end
  ...
  context 'order has only payment step' do
    before do
      ...
      @old_checkout_flow = Viauco::Order.checkout_flow
      ...
      allow_any_instance_of(Viauco::Order).to receive_messages email: 'viauco@commerce.com'
    ...
    after do
      Viauco::Order.checkout_flow(&@old_checkout_flow)
    end
  ...
  context 'save my address' do
    ...
    context 'as a User' do
      before do
        user = create(:user)
        Viauco::Order.last.update_column :user_id, user.id
        allow_any_instance_of(Viauco::OrdersController).to receive_messages(try_viauco_current_user: user)
        allow_any_instance_of(Viauco::CheckoutController).to receive_messages(try_viauco_current_user: user)
        click_button 'Checkout'
      end
  ...
  context 'when order is completed' do
    ...
    it 'does not display a thank you message on that order future visits' do
      ...
      expect(page).not_to have_content(Viauco.t(:thank_you_for_your_order))
    end
  end
  ...
  context "order's address is outside the default included tax zone" do
    context 'so that no taxation applies to its product' do
      before do
        usa = Viauco::Country.find_by(name: 'United States of America')
        ...
        create(:product, name: 'Viauco Bag', price: 100, tax_category: default_tax_category)
        create(:product, name: 'Viauco T-Shirt', price: 100, tax_category: default_tax_category)
      end

      it 'correctly displays other product taxless price which has been added to cart later' do
        visit viauco.root_path

        click_link 'Viauco Bag'
  ...
  context 'user has store credits', js: true do
    ...
    context 'when not all Store Credits are used' do
      ...
      it 'page has data for (multiple) Store Credits' do
        ...
        amount = Viauco::Money.new(store_credit.amount_remaining + additional_store_credit.amount_remaining)
        expect(page).to have_content(Viauco.t('store_credit.available_amount', amount: amount))
      end
      ...
      it 'apply store credits button should move checkout to next step if amount is sufficient' do
        ...
        expect(page).to have_current_path(viauco.order_path(order))
        expect(page).to have_content(Viauco.t('order_processed_successfully'))
      end
      ...
      it 'apply store credits button should wait on payment step for other payment' do
        ...
        expect(page).to have_current_path(viauco.checkout_state_path(:payment))
        amount = Viauco::Money.new(store_credit.amount_remaining + additional_store_credit.amount_remaining)
        remaining_amount = Viauco::Money.new(order.total - amount.money.to_f)
        expect(page).to have_content(Viauco.t('store_credit.applicable_amount', amount: amount))
        expect(page).to have_content(Viauco.t('store_credit.additional_payment_needed', amount: remaining_amount))
        expect(page).to have_content(Viauco.t('store_credit.remove'))
      end
      ...
      context 'remove store credits payments' do
        ...
        it 'remove store credits button should remove store_credits' do
          ...
          expect(page).to have_current_path(viauco.checkout_state_path(:payment))
          expect(page).to have_content(Viauco.t('store_credit.available_amount', amount: order.display_total_available_store_credit))
          expect(page).to have_selector('button[name="apply_store_credit"]')
        end
      end
    end
