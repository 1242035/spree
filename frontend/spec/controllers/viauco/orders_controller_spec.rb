...
describe Viauco::OrdersController, type: :controller do
  ...
  context 'Order model mock' do
    ...
    context '#update' do
      before do
        allow(user).to receive :last_incomplete_viauco_order
        allow(controller).to receive :set_current_order
      end

      it 'cannot update a blank order' do
        put :update, params: { order: { email: 'foo' } }
        expect(flash[:error]).to eq(Viauco.t(:order_not_found))
        expect(response).to redirect_to(viauco.root_path)
      end
    end
  end
