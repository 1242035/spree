...
describe Viauco::CheckoutController, type: :controller do
  let(:token) { 'some_token' }
  let(:user) { stub_model(Viauco::LegacyUser) }
  ...
  context '#edit' do
    ...
    it 'redirects to the cart path unless checkout_allowed?' do
      ...
      expect(response).to redirect_to(viauco.cart_path)
    end
    ...
    it 'redirects to current step trying to access a future step' do
      ...
      get :edit, params: { state: 'delivery' }
      expect(response).to redirect_to viauco.checkout_state_path('address')
    end
  ...
  context '#update' do
    ...
    context 'save successful' do
      ...
      before do
        ...
        allow(order).to receive(:available_shipping_methods).
          and_return [stub_model(Viauco::ShippingMethod)]
        allow(order).to receive(:available_payment_methods).
          and_return [stub_model(Viauco::PaymentMethod)]
      ...
      context 'with the order in the cart state' do
        ...
        it 'redirects the next state' do
          viauco_post_address
          expect(response).to redirect_to viauco.checkout_state_path('delivery')
        end
      end
      ...
      context 'when in the confirm state' do
        ...
        it 'redirects to the order view' do
          ...
          expect(response).to redirect_to viauco.order_path(order)
        end

        it 'populates the flash message' do
          post :update, params: { state: 'confirm' }
          expect(flash.notice).to eq(Viauco.t(:order_processed_successfully))
        end
    ...
    context 'Viauco::Core::GatewayError' do
      before do
        allow(order).to receive_messages user: user
        allow(order).to receive(:update).and_raise(Viauco::Core::GatewayError.new('Invalid something or other.'))
        post :update, params: { state: 'address' }
      end

      it 'renders the edit template and display exception message' do
        expect(response).to render_template :edit
        expect(flash.now[:error]).to eq(Viauco.t(:viauco_gateway_error_flash_for_checkout))
    ...
    context 'fails to transition from address' do
      ...
      context 'when the country is not a shippable country' do
        ...
        it 'due to no available shipping rates for any of the shipments' do
          ...
          expect(flash[:error]).to eq(Viauco.t(:items_cannot_be_shipped))
          expect(response).to redirect_to(viauco.checkout_state_path('address'))
        end
      end
    ...
    context 'fails to transition from payment to complete' do
      ...
      it 'when GatewayError is raised' do
        allow_any_instance_of(Viauco::Payment).to receive(:process!).and_raise(Viauco::Core::GatewayError.new(Viauco.t(:payment_processing_failed)))
        put :update, params: { state: order.state, order: {} }
        expect(flash[:error]).to eq(Viauco.t(:payment_processing_failed))
      end
    end
  end

  context 'When last inventory item has been purchased' do
    let(:product) { mock_model(Viauco::Product, name: 'Amazing Object') }
    let(:variant) { mock_model(Viauco::Variant) }
    let(:line_item) { mock_model Viauco::LineItem, insufficient_stock?: true, amount: 0 }
    ...
    context 'and back orders are not allowed' do
      ...
      it 'sets flash message for no inventory' do
        expect(flash[:error]).to eq(
          Viauco.t(:inventory_error_flash_for_insufficient_quantity, names: "'#{product.name}'")
        )
      end
    end
  end
  ...
  context 'in the payment step' do
    ...
    let(:payment_method_id) { Viauco::PaymentMethod.first.id }
    ...
    context 'with store credits payment' do
      ...
      def expect_one_store_credit_payment(order, amount)
        ...
        expect(order.payments.first.source).to be_a Viauco::StoreCredit
      ...
      it 'can fully pay with store credits while removing an existing card' do
        credit_card = create(:credit_card, user: user, payment_method: Viauco::PaymentMethod.first)
    ...
    context 'remove store credits payment' do
      ...
      before do
        create(:store_credit_payment_method)
        create(:store_credit, user: user, amount: credit_amount)
        Viauco::Checkout::AddStoreCredit.call(order: order)
      end

      def expect_invalid_store_credit_payment(order)
        expect(order.payments.store_credits.with_state(:invalid).count).to eq 1
        expect(order.payments.store_credits.with_state(:invalid).first.source).to be_a Viauco::StoreCredit
      end
  ...
  context 'Address Book' do
    ...
    before do
      Viauco::Cart::AddItem.call(order: order, variant: variant, quantity: 1)
      allow(controller).to receive(:viauco_current_user).and_return(user)
      allow(controller).to receive(:current_store).and_return(order.store)
      allow(order).to receive_messages user: user
      allow(order).to receive(:available_shipping_methods).and_return [stub_model(Viauco::ShippingMethod)]
      allow(order).to receive(:available_payment_methods).and_return [stub_model(Viauco::PaymentMethod)]
