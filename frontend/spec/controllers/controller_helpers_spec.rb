...
# So we need to use one of the controllers inside Viauco.
# ProductsController is good.
describe Viauco::ProductsController, type: :controller do
  ...
  context 'when session locale not set' do
    ...
    context 'when Viauco::Frontend::Config[:locale] not present' do
      before do
        Viauco::Frontend::Config[:locale] = nil
      end
    ...
    context 'when Viauco::Frontend::Config[:locale] is present' do
      context 'and not in available_locales' do
        before do
          Viauco::Frontend::Config[:locale] = unavailable_locale
        end
      ...
      context 'and not in available_locales' do
        before do
          Viauco::Frontend::Config[:locale] = available_locale
        end

        it 'sets the default locale based on Viauco::Frontend::Config[:locale]' do
          expect(I18n.locale).to eq(:en)
          get :index
          expect(I18n.locale).to eq(available_locale)
        end
