module Viauco
  ...
  # is waranted.
  class CheckoutController < Viauco::StoreController
    include Viauco::Checkout::AddressBook
    ...
    helper 'viauco/orders'

    rescue_from Viauco::Core::GatewayError, with: :rescue_from_viauco_gateway_error
    ...
    def update
      if @order.update_from_params(params, permitted_checkout_attributes, request.headers.env)
        ...
        if @order.completed?
          @current_order = nil
          flash.notice = Viauco.t(:order_processed_successfully)
    ...
    def load_order_with_lock
      ...
      redirect_to(viauco.cart_path) && return unless @order
    end
      ...
      if params[:order] && params[:order][:state_lock_version]
        ...
        @order.reload.with_lock do
          unless @order.state_lock_version == params[:order].delete(:state_lock_version).to_i
            flash[:error] = Viauco.t(:order_already_updated)
            redirect_to(checkout_state_path(@order.state)) && return
          end
    ...
    def ensure_checkout_allowed
      redirect_to viauco.cart_path unless @order.checkout_allowed?
    end

    def ensure_order_not_completed
      redirect_to viauco.cart_path if @order.completed?
    end

    def ensure_sufficient_stock_lines
      if @order.insufficient_stock_lines.present?
        flash[:error] = Viauco.t(:inventory_error_flash_for_insufficient_quantity)
    ...
    def completion_route(custom_params = nil)
      viauco.order_path(@order, custom_params)
    end
    ...
    def before_delivery
      ...
      @differentiator = Viauco::Stock::Differentiator.new(@order, packages)
    end
    ...
    def before_payment
      if @order.checkout_steps.include? 'delivery'
        ...
        @differentiator.missing.each do |variant, quantity|
          Viauco::Dependencies.cart_remove_item_service.constantize.call(order: @order, variant: variant, quantity: quantity)
        end
      end

      @payment_sources = try_viauco_current_user.payment_sources if try_viauco_current_user&.respond_to?(:payment_sources)
    end
    ...
    def rescue_from_viauco_gateway_error(exception)
      flash.now[:error] = Viauco.t(:viauco_gateway_error_flash_for_checkout)
    ...
    def add_store_credit_service
      Viauco::Dependencies.checkout_add_store_credit_service.constantize
    end

    def remove_store_credit_service
      Viauco::Dependencies.checkout_remove_store_credit_service.constantize
    end
  end
end
