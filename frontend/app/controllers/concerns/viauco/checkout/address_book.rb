# https://github.com/viauco-contrib/viauco_address_book/blob/master/app/controllers/viauco/checkout_controller_decorator.rb
module Viauco
  module Checkout
    module AddressBook
      ...
      def set_addresses
        ...
        if params[:order][:ship_address_id].to_i > 0
          ...
          Viauco::Address.find(params[:order][:ship_address_id]).user_id != try_viauco_current_user&.id && raise('Frontend address forging')
        else
        ...
        if params[:order][:bill_address_id].to_i > 0
          ...
          Viauco::Address.find(params[:order][:bill_address_id]).user_id != try_viauco_current_user&.id && raise('Frontend address forging')
        else
      ...
      def normalize_addresses
        ...
        if @order.bill_address_id != @order.ship_address_id && bill_address == ship_address
          ...
          bill_address.update_attribute(:user_id, try_viauco_current_user&.id)
        end

        ship_address.update_attribute(:user_id, try_viauco_current_user&.id)
      end
    end
  end
end
