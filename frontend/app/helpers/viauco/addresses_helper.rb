# https://github.com/viauco-contrib/viauco_address_book/blob/master/app/helpers/viauco/addresses_helper.rb
module Viauco
  module AddressesHelper
    def address_field(form, method, address_id = 'b', &handler)
      content_tag :div, id: [address_id, method].join, class: 'form-group' do
        if handler
          ...
          is_required = Viauco::Address.required_fields.include?(method)
    ...
    def address_state(form, country, _address_id = 'b')
      country ||= Viauco::Country.find(Viauco::Config[:default_country_id])
      ...
      form.label(:state, Viauco.t(:state)) + '<span class="req">*</span><br />'.html_safe +
