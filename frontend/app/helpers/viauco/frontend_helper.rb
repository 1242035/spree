module Viauco
  module FrontendHelper
    ...
    def viauco_breadcrumbs(taxon, separator = '')
      return '' if current_page?('/') || taxon.nil?

      separator = raw(separator)
      crumbs = [content_tag(:li, content_tag(:span, link_to(content_tag(:span, Viauco.t(:home), itemprop: 'name'), viauco.root_path, itemprop: 'url') + separator, itemprop: 'item'), itemscope: 'itemscope', itemtype: 'https://schema.org/ListItem', itemprop: 'itemListElement', class: 'breadcrumb-item')]
      if taxon
        crumbs << content_tag(:li, content_tag(:span, link_to(content_tag(:span, Viauco.t(:products), itemprop: 'name'), viauco.products_path, itemprop: 'url') + separator, itemprop: 'item'), itemscope: 'itemscope', itemtype: 'https://schema.org/ListItem', itemprop: 'itemListElement', class: 'breadcrumb-item')
        ...
        crumbs << content_tag(:li, content_tag(:span, Viauco.t(:products), itemprop: 'item'), class: 'active', itemscope: 'itemscope', itemtype: 'https://schema.org/ListItem', itemprop: 'itemListElement')
      end
    ...
    def checkout_progress(numbers: false)
      ...
      items = states.each_with_index.map do |state, i|
        text = Viauco.t("order_state.#{state}").titleize
    ...
    def link_to_cart(text = nil)
      text = text ? h(text) : Viauco.t('cart')
      css_class = nil

      if simple_current_order.nil? || simple_current_order.item_count.zero?
        text = "<span class='glyphicon glyphicon-shopping-cart'></span> #{text}: (#{Viauco.t('empty')})"
        css_class = 'empty'
      else
      ...
      link_to text.html_safe, viauco.cart_path, class: "cart-info nav-link #{css_class}"
    end
