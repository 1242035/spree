Viauco.ready(function ($) {
  Viauco.onAddress = function () {
    if ($('#checkout_form_address').length) {
      Viauco.updateState = function (region) {
        var countryId = getCountryId(region)
        if (countryId != null) {
          if (Viauco.Checkout[countryId] == null) {
            $.ajax({
              ...
              Viauco.Checkout[countryId] = {
                states: xStates,
                states_required: data.data.attributes.states_required,
                zipcode_required: data.data.attributes.zipcode_required
              }
              Viauco.fillStates(Viauco.Checkout[countryId], region)
              Viauco.toggleZipcode(Viauco.Checkout[countryId], region)
            })
          } else {
            Viauco.fillStates(Viauco.Checkout[countryId], region)
            Viauco.toggleZipcode(Viauco.Checkout[countryId], region)
      ...
      Viauco.toggleZipcode = function (data, region) {
      ...
      Viauco.fillStates = function (data, region) {
      ...
      $('#bcountry select').change(function () {
        Viauco.updateState('b')
      })
      $('#scountry select').change(function () {
        Viauco.updateState('s')
      })
      Viauco.updateState('b')
