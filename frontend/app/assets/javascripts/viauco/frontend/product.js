//= require viauco/api/storefront/cart
//= require viauco/frontend/cart

Viauco.ready(function ($) {
  Viauco.addImageHandlers = function () {
  ...
  Viauco.showVariantImages = function (variantId) {
    ($('li.vtmb')).hide();
  ...
  Viauco.updateVariantPrice = function (variant) {
  ...
  Viauco.disableCartForm = function (variant) {
  ...
  if (radios.length > 0) {
    ...
    Viauco.showVariantImages(selectedRadio.attr('value'));
    Viauco.updateVariantPrice(selectedRadio);
    Viauco.disableCartForm(selectedRadio);

    radios.click(function (event) {
      $("#product-variants").find('li.active').removeClass("active");
      $(this).closest("li").addClass("active");
      Viauco.showVariantImages(this.value);
      Viauco.updateVariantPrice($(this));
      return Viauco.disableCartForm($(this));
    })
  }

  return Viauco.addImageHandlers();
})
...
Viauco.ready(function () {
  ...
  if (addToCartForm) {
    ...
    addToCartForm.addEventListener('submit', function (event) {
      ...
      Viauco.ensureCart(
        function () {
          ViaucoAPI.Storefront.addToCart(
            variantId,
            quantity,
            {}, // options hash - you can pass additional parameters here, your backend
            // needs to be aware of those, see API docs:
            // https://github.com/viauco/viauco/blob/master/api/docs/v2/storefront/index.yaml#L42
            function () {
              // redirect with `variant_id` is crucial for analytics tracking
              // provided by `viauco_analytics_trackers` extension
              window.location = Viauco.routes.cart + '?variant_id=' + variantId.toString()
            },
