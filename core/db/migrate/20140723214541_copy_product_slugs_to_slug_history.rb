class CopyProductSlugsToSlugHistory < ActiveRecord::Migration[4.2]
  def change
	...
	Viauco::Product.connection.execute <<-SQL
INSERT INTO #{FriendlyId::Slug.table_name} (slug, sluggable_id, sluggable_type, created_at)
  SELECT slug, id, '#{Viauco::Product.to_s}', #{ApplicationRecord.send(:sanitize_sql_array, ['?', Time.current])} 
  FROM #{Viauco::Product.table_name}
