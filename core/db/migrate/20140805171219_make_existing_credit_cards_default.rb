class MakeExistingCreditCardsDefault < ActiveRecord::Migration[4.2]
  def up
    ...
    Viauco::CreditCard.where.not(user_id: nil).joins("LEFT OUTER JOIN viauco_credit_cards cc2 ON cc2.user_id = viauco_credit_cards.user_id AND viauco_credit_cards.created_at < cc2.created_at").where("cc2.user_id IS NULL").update_all(default: true)
  end
  def down
    # do nothing
  end
