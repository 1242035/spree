class IncreaseScaleOnPreTaxAmounts < ActiveRecord::Migration[4.2]
  def change
    ...
    #
    execute(<<-SQL)
      UPDATE viauco_shipments
      SET pre_tax_amount = (cost + promo_total) - included_tax_total
      WHERE pre_tax_amount IS NULL;
    SQL

    change_column :viauco_line_items, :pre_tax_amount, :decimal, precision: 12, scale: 4, default: 0.0, null: false
    change_column :viauco_shipments, :pre_tax_amount, :decimal, precision: 12, scale: 4, default: 0.0, null: false
  end
end
