class MigrateOldShippingCalculators < ActiveRecord::Migration[4.2]
  def up
    Viauco::ShippingMethod.all.each do |shipping_method|
      old_calculator = shipping_method.calculator
      next if old_calculator.class < Viauco::ShippingCalculator # We don't want to mess with new shipping calculators
