class RemoveStockItemAndVariantLock < ActiveRecord::Migration[4.2]
  def up
    ...
    remove_column :viauco_stock_items, :lock_version

    # variants no longer manage their count_on_hand so we are removing their lock
    remove_column :viauco_variants, :lock_version
  end

  def down
    add_column :viauco_stock_items, :lock_version, :integer
    add_column :viauco_variants, :lock_version, :integer
  end
end
