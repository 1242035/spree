...
describe Viauco::Core::ControllerHelpers::Order, type: :controller do
  ...
  describe '#simple_current_order' do
    before { allow(controller).to receive_messages(try_viauco_current_user: user) }

    it 'returns an empty order' do
      expect(controller.simple_current_order.item_count).to eq 0
    end
    it 'returns Viauco::Order instance' do
  ...
  describe '#current_order' do
    ...
    context 'create_order_if_necessary option is true' do
      it 'creates new order' do
        ...
        end.to change(Viauco::Order, :count).to(1)
      end

      it 'assigns the current_store id' do
        controller.current_order(create_order_if_necessary: true)
        expect(Viauco::Order.last.store_id).to eq store.id
      end
    end
    ...
    context 'gets using the token' do
      ...
      before do
        expect(controller).to receive(:current_order_params).and_return(
          currency: Viauco::Config[:currency], token: 'token', store_id: guest_order.store_id, user_id: user.id
        )
      end
  ...
  describe '#associate_user' do
    before do
      allow(controller).to receive_messages(current_order: order, try_viauco_current_user: user)
    end

    context "user's email is blank" do
      let(:user) { create(:user, email: '') }

      it 'calls Viauco::Order#associate_user! method' do
        expect_any_instance_of(Viauco::Order).to receive(:associate_user!)
        controller.associate_user
      end
    end

    context "user isn't blank" do
      it 'does not calls Viauco::Order#associate_user! method' do
        expect_any_instance_of(Viauco::Order).not_to receive(:associate_user!)
        controller.associate_user
      end
    end
  end
  ...
  describe '#set_current_order' do
    ...
    context 'when current order not equal to users incomplete orders' do
      ...
      it 'calls Viauco::Order#merge! method' do
