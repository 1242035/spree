...
module Viauco
  module Core
    describe QueryFilters::Text do
      let(:subject) do
        described_class.new(attribute: Viauco::DummyModel.arel_table[:name])
      end

      let!(:dummy1) { Viauco::DummyModel.create(name: 'TestName', position: 3) }
      let!(:dummy2) { Viauco::DummyModel.create(name: 'Test', position: 10) }
      let!(:dummy3) { Viauco::DummyModel.create(name: 'Something', position: 4) }
