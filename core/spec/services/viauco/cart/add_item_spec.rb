  ...
  describe Cart::AddItem do
    ...
    context 'with given shipment' do
      ...
      it 'ensure shipment calls update_amounts instead of order calling ensure_updated_shipments' do
        expect(order).to receive(:refresh_shipment_rates).with(Viauco::ShippingMethod::DISPLAY_ON_BACK_END)
    ...
    context 'running promotions' do
      ...
      let(:calculator) { Viauco::Calculator::FlatRate.new(preferred_amount: 10) }

      context 'one active order promotion' do
        let!(:action) { Viauco::Promotion::Actions::CreateAdjustment.create(promotion: promotion, calculator: calculator) }
      ...
      context 'one active line item promotion' do
        let!(:action) { Viauco::Promotion::Actions::CreateItemAdjustments.create(promotion: promotion, calculator: calculator) }
      ...
      context 'VAT for variant with percent promotion' do
        let!(:category) { Viauco::TaxCategory.create name: 'Taxable Foo' }
        let!(:rate) do
        ...
        let(:calculator) { Viauco::Calculator::PercentOnLineItem.new(preferred_percent: 50) }
        let!(:action) { Viauco::Promotion::Actions::CreateItemAdjustments.create(promotion: promotion, calculator: calculator) }
    ...
    context 'pass invalid arguments' do
      ...
      context 'pass non-existing variant' do
        ...
        before { Viauco::Variant.find(variant_2.id).destroy }
