...
describe Viauco::Country, type: :model do
  let(:america) { create :country }
  let(:canada)  { create :country, name: 'Canada', iso_name: 'CANADA', iso: 'CA', iso3: 'CAN', numcode: '124' }

  it 'validates uniqueness' do
    canada.touch
    expect(Viauco::Country.new(name: 'Canada', iso: 'CA', iso3: 'CAN', iso_name: 'CANADA')).not_to be_valid
  end
  ...
  describe '.default' do
    context 'when default_country_id config is set' do
      before { Viauco::Config[:default_country_id] = canada.id }
  ...
  describe 'ensure proper country deletion' do
    context 'when deleting default country' do
      before { Viauco::Config[:default_country_id] = america.id }
      ...
      it 'sets correct error message' do
        ...
        expect(america.errors[:base]).to include(Viauco.t(:default_country_cannot_be_deleted))
      end
    end
