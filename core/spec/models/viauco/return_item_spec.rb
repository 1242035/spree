...
describe Viauco::ReturnItem, type: :model do
  all_reception_statuses = Viauco::ReturnItem.state_machines[:reception_status].states.map(&:name).map(&:to_s)
  all_acceptance_statuses = Viauco::ReturnItem.state_machines[:acceptance_status].states.map(&:name).map(&:to_s)

  before do
    allow_any_instance_of(Viauco::Order).to receive_messages(return!: true)
  end
  ...
  describe '#receive!' do
    ...
    context 'with a stock location' do
      ...
      context 'when the restock_inventory preference is false' do
        before do
          Viauco::Config[:restock_inventory] = false
        end
  ...
  describe '#display_pre_tax_amount' do
    ...
    it 'returns a Viauco::Money' do
      expect(return_item.display_pre_tax_amount).to eq(Viauco::Money.new(pre_tax_amount))
    end
  end

  describe '.default_refund_amount_calculator' do
    it 'defaults to the default refund amount calculator' do
      expect(Viauco::ReturnItem.refund_amount_calculator).to eq Viauco::Calculator::Returns::DefaultRefundAmount
    end
  end
  ...
  describe 'pre_tax_amount calculations on create' do
    ...
    context 'pre tax amount is not specified' do
      ...
      context 'not an exchange' do
        it { expect(subject.pre_tax_amount).to eq Viauco::Calculator::Returns::DefaultRefundAmount.new.compute(subject) }
      end
  ...
  describe '.from_inventory_unit' do
    subject { Viauco::ReturnItem.from_inventory_unit(inventory_unit) }
  ...
  describe 'validity for reimbursements' do
    ...
    context 'when acceptance_status is accepted' do
      ...
      it 'is valid' do
        ...
        expect(subject.errors.messages).to eq(reimbursement: [I18n.t(:cannot_be_associated_unless_accepted, scope: 'activerecord.errors.models.viauco/return_item.attributes.reimbursement')])
      end
    end
  end
  ...
  describe '#exchange_requested?' do
    context 'exchange variant exists' do
      before { allow(subject).to receive(:exchange_variant) { mock_model(Viauco::Variant) } }
  ...
  describe '#exchange_processed?' do
    context 'exchange inventory unit exists' do
      before { allow(subject).to receive(:exchange_inventory_units) { [mock_model(Viauco::InventoryUnit)] } }
  ...
  describe '#eligible_exchange_variants' do
    it 'uses the exchange variant calculator to compute possible variants to exchange for' do
      ...
      expect(Viauco::ReturnItem.exchange_variant_engine).to receive(:eligible_variants).with(return_item.variant)
  ...
  describe '.exchange_variant_engine' do
    it 'defaults to the same product calculator' do
      expect(Viauco::ReturnItem.exchange_variant_engine).to eq Viauco::ReturnItem::ExchangeVariantEligibility::SameProduct
    end
  end
  ...
  describe '#build_default_exchange_inventory_unit' do
    ...
    context 'the return item is intended to be exchanged' do
      before { allow(return_item).to receive(:exchange_variant).and_return(mock_model(Viauco::Variant)) }

      context 'an exchange inventory unit already exists' do
        before do
          allow(return_item).to receive(:exchange_inventory_units).and_return([mock_model(Viauco::InventoryUnit)])
        end
  ...
  describe '#process_inventory_unit!' do
    ...
    context 'stock should restock' do
      ...
      it { expect(subject).to eq(Viauco::StockMovement.find_by(stock_movement_attributes)) }
    end
    ...
    context 'stock should not restock' do
      ...
      context 'when restock inventory preference false' do
        before { Viauco::Config[:restock_inventory] = false }
