...
describe Viauco::Store, type: :model do
  ...
  describe '.current' do
    ...
    let!(:store_1) { Viauco::Store.first || create(:store) }
    ...
    it 'returns store for domain' do
      expect(subject.class.current('viaucocommerce.com')).to eql(store_1)
  ...
  describe '.default' do
    context 'when a default store is already present' do
      ...
      it 'returns the already existing default store' do
        expect(Viauco::Store.default).to eq(store_2)
      end
      ...
      it 'ensures there is only one default' do
        ...
        expect(Viauco::Store.where(default: true).count).to eq(1)
        expect(store_2.default).to be true
    ...
    context 'when a default store is not present' do
      it 'builds a new default store' do
        expect(Viauco::Store.default.class).to eq(Viauco::Store)
        expect(Viauco::Store.default.persisted?).to eq(false)
        expect(Viauco::Store.default.default).to be(true)
      end
    end
  end
end
