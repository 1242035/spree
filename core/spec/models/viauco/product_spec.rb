...
module ThirdParty
  class Extension < Viauco::Base
    # nasty hack so we don't have to create a table to back this fake model
    self.table_name = 'viauco_products'
  end
end

describe Viauco::Product, type: :model do
  context 'product instance' do
    ...
    %w[purchasable backorderable in_stock].each do |method_name|
      describe "#{method_name}?" do
        ...
        it "returns false if no variant is #{method_name.humanize.downcase}" do
          allow_any_instance_of(Viauco::Variant).to receive("#{method_name}?").and_return(false)
    ...
    context '#duplicate' do
      ...
      it 'calls #duplicate_extra' do
        expect_any_instance_of(Viauco::Product).to receive(:duplicate_extra).
          with(product)
    ...
    context 'has stock movements' do
      ...
      it 'doesnt raise ReadOnlyRecord error' do
        Viauco::StockMovement.create!(stock_item: stock_item, quantity: 1)
    ...
    describe '#discontinue_on_must_be_later_than_available_on' do
      ...
      context 'available_on is a date earlier than discontinue_on' do
        ...
        context 'is not valid' do
          ...
          it { expect(product.errors[:discontinue_on]).to include(I18n.t(:invalid_date_range, scope: 'activerecord.errors.models.viauco/product.attributes.discontinue_on')) }
        end
      end
  ...
  context 'properties' do
    ...
    context 'optional property_presentation' do
      subject { Viauco::Property.where(name: 'foo').first.presentation }
    ...
    it "does not overwrite properties' presentation names" do
      Viauco::Property.where(name: 'foo').first_or_create!(presentation: "Foo's Presentation Name")
      product.set_property('foo', 'value1')
      product.set_property('bar', 'value2')
      expect(Viauco::Property.where(name: 'foo').first.presentation).to eq("Foo's Presentation Name")
      expect(Viauco::Property.where(name: 'bar').first.presentation).to eq('bar')
    end
  ...
  context '#create' do
    ...
    let!(:product) { Viauco::Product.new(name: 'Foo', price: 1.99, shipping_category_id: create(:shipping_category).id) }
  ...
  context '#images' do
    ...
    let(:params) { { viewable_id: product.master.id, viewable_type: 'Viauco::Variant', attachment: file, alt: 'position 2', position: 2 } }

    before do
      images = [
        Viauco::Image.new(params),
        Viauco::Image.new(params.merge(alt: 'position 1', position: 1)),
        Viauco::Image.new(params.merge(viewable_type: 'ThirdParty::Extension', alt: 'position 1', position: 2))
      ]
  ...
  context 'classifications and taxons' do
    it 'is joined through classifications' do
      reflection = Viauco::Product.reflect_on_association(:taxons)
      expect(reflection.options[:through]).to eq(:classifications)
    end

    it 'will delete all classifications' do
      reflection = Viauco::Product.reflect_on_association(:classifications)
  ...
  # Regression spec for https://github.com/viauco/viauco/issues/5588
  ...
  context '#brand' do
    let(:taxonomy) { create(:taxonomy, name: I18n.t('viauco.taxonomy_brands_name')) }
  ...
  context '#category' do
    let(:taxonomy) { create(:taxonomy, name: I18n.t('viauco.taxonomy_categories_name')) }
    let(:product) { create(:product, taxons: [taxonomy.taxons.first]) }
  ...
  describe '#ensure_no_line_items' do
    ...
    it 'adds error on product destroy' do
      ...
      expect(product.errors[:base]).to include I18n.t('activerecord.errors.models.viauco/product.attributes.base.cannot_destroy_if_attached_to_line_items')
    end
  end
