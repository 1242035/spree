      ...
      context 'status messages' do
        ...
        describe '#set_success_code' do
          ...
          it 'has success message' do
            subject
            expect(coupon.success).to eq(Viauco.t(status))
          end
        end
        ...
        describe '#set_error_code' do
          ...
          it 'has error message' do
            subject
            expect(coupon.error).to eq(Viauco.t(status))
          end
        end
      end
      ...
      context 'coupon code promotion doesnt exist' do
        ...
        context 'with no actions defined' do
          ...
          it 'populates error message' do
            subject.apply
            expect(subject.error).to eq Viauco.t(:coupon_code_not_found)
          end
        end
      end
        ...
        context 'with a per-item adjustment action' do
          ...
          context 'right coupon given' do
            context 'with correct coupon code casing' do
              ...
              it 'coupon already applied to the order' do
                ...
                expect(subject.success).to be_present
                subject.apply
                expect(subject.error).to eq Viauco.t(:coupon_code_already_applied)
              end
            end
          ...
          context 'coexists with a non coupon code promo' do
            ...
            before do
              ...
              Viauco::Cart::AddItem.call(order: order, variant: create(:variant))
            end
        ...
        context 'with a whole-order adjustment action' do
          ...
          context 'right coupon given' do
            ...
            it 'coupon fails to activate' do
              allow_any_instance_of(Viauco::Promotion).to receive(:activate).and_return false
              subject.apply
              expect(subject.error).to eq Viauco.t(:coupon_code_unknown_error)
            end
            ...
            it 'coupon code hit max usage' do
              ...
              expect(coupon.error).to eq Viauco.t(:coupon_code_max_usage)
            end
            ...
            context 'when the a new coupon is less good' do
              ...
              it 'notifies of better deal' do
                ...
                expect(coupon.error).to eq Viauco.t(:coupon_code_better_exists)
              end
            end
          end
        end
          ...
          context 'and the product price is less than promo discount' do
            before do
              3.times do |_i|
                ...
                Viauco::Cart::AddItem.call(order: @order, variant: taxable.master)
              end
            end
          ...
          context 'and the product price is greater than promo discount' do
            before do
              3.times do |_i|
                ...
                Viauco::Cart::AddItem.call(order: @order, variant: taxable.master, quantity: 2)
              end
            end
