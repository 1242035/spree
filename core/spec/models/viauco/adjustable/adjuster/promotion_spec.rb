...
describe Viauco::Adjustable::Adjuster::Promotion, type: :model do
  let(:order) { create :order_with_line_items, line_items_count: 1 }
  let(:line_item) { order.line_items.first }
  let(:subject) { Viauco::Adjustable::AdjustmentsUpdater.new(line_item) }
  let(:order_subject) { Viauco::Adjustable::AdjustmentsUpdater.new(order) }

  context 'best promotion is always applied' do
    let(:calculator) { Viauco::Calculator::FlatRate.new(preferred_amount: 10) }
    let(:source) { Viauco::Promotion::Actions::CreateItemAdjustments.create calculator: calculator }
    ...
    context 'when previously ineligible promotions become available' do
      ...
      promo_sequences.each do |promo_sequence|
        it 'picks the best order-level promo according to current eligibility' do
          ...
          Viauco::Cart::AddItem.call(order: order, variant: create(:variant, price: 10))
          order.save
