...
describe Viauco::CreditCard, type: :model do
  ...
  let(:credit_card) { Viauco::CreditCard.new }

  def self.payment_states
    Viauco::Payment.state_machine.states.keys
  end

  before do
    @order = create(:order)
    @payment = Viauco::Payment.create(amount: 100, order: @order)
    ...
    @payment_gateway = mock_model(Viauco::PaymentMethod,
  ...
  context '#can_capture?' do
    it 'is true if payment is pending' do
      payment = mock_model(Viauco::Payment, pending?: true, created_at: Time.current)
      expect(credit_card.can_capture?(payment)).to be true
    end

    it 'is true if payment is checkout' do
      payment = mock_model(Viauco::Payment, pending?: false, checkout?: true, created_at: Time.current)
      expect(credit_card.can_capture?(payment)).to be true
    end
  end
  ...
  context '#can_void?' do
    it 'is true if payment is not void' do
      payment = mock_model(Viauco::Payment, failed?: false, void?: false)
  ...
  context '#can_credit?' do
    it 'is false if payment is not completed' do
      payment = mock_model(Viauco::Payment, completed?: false)
      expect(credit_card.can_credit?(payment)).to be false
    end

    it 'is false when credit_allowed is zero' do
      payment = mock_model(Viauco::Payment, completed?: true, credit_allowed: 0, order: mock_model(Viauco::Order, payment_state: 'credit_owed'))
      expect(credit_card.can_credit?(payment)).to be false
    end
  end
  ...
  context '#save' do
    ...
    let!(:persisted_card) { Viauco::CreditCard.find(credit_card.id) }
