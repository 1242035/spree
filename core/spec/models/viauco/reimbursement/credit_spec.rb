  ...
  describe Reimbursement::Credit, type: :model do
    context 'class methods' do
      describe '.total_amount_reimbursed_for' do
        subject { Viauco::Reimbursement::Credit.total_amount_reimbursed_for(reimbursement) }
    ...
    describe '#description' do
      let(:credit) { Viauco::Reimbursement::Credit.new(amount: 100, creditable: mock_model(Viauco::PaymentMethod::Check)) }
    ...
    describe '#display_amount' do
      let(:credit) { Viauco::Reimbursement::Credit.new(amount: 100) }

      it 'is a money object' do
        expect(credit.display_amount).to eq Viauco::Money.new(100, currency: 'USD')
      end
    end
  end
end
