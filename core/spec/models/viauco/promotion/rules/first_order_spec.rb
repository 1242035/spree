...
describe Viauco::Promotion::Rules::FirstOrder, type: :model do
  let(:rule) { Viauco::Promotion::Rules::FirstOrder.new }
  let(:order) { mock_model(Viauco::Order, user: nil, email: nil) }
  let(:user) { mock_model(Viauco::LegacyUser) }
  ...
  context 'first order' do
    context 'for a signed user' do
      ...
      context 'with completed orders' do
        ...
        context 'with another order' do
          before { allow(user).to receive_message_chain(:orders, complete: [mock_model(Viauco::Order)]) }
    ...
    context 'for a guest user' do
      let(:email) { 'user@viaucocommerce.org' }

      before { allow(order).to receive_messages email: 'user@viaucocommerce.org' }
      ...
      context 'with another order' do
        before { allow(rule).to receive_messages(orders_by_email: [mock_model(Viauco::Order)]) }
