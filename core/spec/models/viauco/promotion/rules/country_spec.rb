...
describe Viauco::Promotion::Rules::Country, type: :model do
  let(:rule) { Viauco::Promotion::Rules::Country.new }
  let(:order) { create(:order) }
  let(:country) { create(:country) }
  let(:other_country) { create(:country) }

  before { allow(Viauco::Country).to receive(:default) { other_country.id } }
