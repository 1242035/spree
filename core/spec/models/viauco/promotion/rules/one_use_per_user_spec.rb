...
describe Viauco::Promotion::Rules::OneUsePerUser, type: :model do
  let(:rule) { described_class.new }

  describe '#eligible?(order)' do
    subject { rule.eligible?(order) }

    let(:order) { double Viauco::Order, user: user }
    let(:user) { double Viauco::LegacyUser }
    let(:promotion) { stub_model Viauco::Promotion, used_by?: used_by }
    let(:used_by) { false }
