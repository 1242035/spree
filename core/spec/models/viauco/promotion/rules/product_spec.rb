...
describe Viauco::Promotion::Rules::Product, type: :model do
  let(:rule) { Viauco::Promotion::Rules::Product.new(rule_options) }
  let(:rule_options) { {} }
  ...
  context '#eligible?(order)' do
    ...
    before do
      3.times { |i| instance_variable_set("@product#{i}", mock_model(Viauco::Product)) }
    end
  ...
  describe '#actionable?' do
    ...
    let(:rule_line_item) { Viauco::LineItem.new(product: rule_product) }
    let(:other_line_item) { Viauco::LineItem.new(product: other_product) }

    let(:rule_options) { super().merge(products: [rule_product]) }
    let(:rule_product) { mock_model(Viauco::Product) }
    let(:other_product) { mock_model(Viauco::Product) }
