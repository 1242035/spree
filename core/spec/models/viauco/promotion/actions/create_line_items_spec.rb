...
describe Viauco::Promotion::Actions::CreateLineItems, type: :model do
  let(:order) { create(:order) }
  let(:action) { Viauco::Promotion::Actions::CreateLineItems.create }
  let(:promotion) { stub_model(Viauco::Promotion) }
  ...
  context '#perform' do
    ...
    context 'order is eligible' do
      ...
      it 'only adds the delta of quantity to an order' do
        Viauco::Cart::AddItem.call(order: order, variant: shirt)
        action.perform(payload)
      ...
      it "doesn't add if the quantity is greater" do
        Viauco::Cart::AddItem.call(order: order, variant: shirt, quantity: 3)
        action.perform(payload)
        line_item = order.line_items.find_by(variant_id: shirt.id)
        expect(line_item).not_to be_nil
