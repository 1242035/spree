...
describe Viauco::Promotion::Actions::CreateAdjustment, type: :model do
  let(:order) { create(:order_with_line_items, line_items_count: 1) }
  let(:promotion) { create(:promotion) }
  let(:action) { Viauco::Promotion::Actions::CreateAdjustment.new }
  ...
  context '#perform' do
    before do
      action.calculator = Viauco::Calculator::FlatRate.new(preferred_amount: 10)
