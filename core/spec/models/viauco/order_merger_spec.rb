...
module Viauco
  describe OrderMerger, type: :model do
    ...
    let(:order_1) { Viauco::Order.create }
    let(:order_2) { Viauco::Order.create }
    let(:user) { stub_model(Viauco::LegacyUser, email: 'viauco@example.com') }
    let(:subject) { Viauco::OrderMerger.new(order_1) }
    ...
    context 'merging together two orders with line items for the same variant' do
      before do
        Viauco::Cart::AddItem.call order: order_1, variant: variant, quantity: 1
        Viauco::Cart::AddItem.call order: order_2, variant: variant, quantity: 1
      end
    ...
    context 'merging using extension-specific line_item_comparison_hooks' do
      before do
        ...
        allow(Viauco::Variant).to receive(:price_modifier_amount).and_return(0.00)
      end
      ...
      context '2 equal line items' do
        before do
          @line_item_1 = Viauco::Cart::AddItem.call(order: order_1, variant: variant, quantity: 1, options: {foos: {}}).value
          @line_item_2 = Viauco::Cart::AddItem.call(order: order_2, variant: variant, quantity: 1, options: {foos: {}}).value
        end
      ...
      context '2 different line items' do
        before do
          ...
          Viauco::Cart::AddItem.call order: order_1, variant: variant, quantity: 1, options: {foos: {}}
          Viauco::Cart::AddItem.call order: order_2, variant: variant, quantity: 1, options: {foos: {}}
        end
    ...
    context 'merging together two orders with different line items' do
      ...
      before do
        Viauco::Cart::AddItem.call order: order_1, variant: variant, quantity: 1
        Viauco::Cart::AddItem.call order: order_2, variant: variant_2, quantity: 1
      end
