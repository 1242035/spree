...
describe Viauco::Order, type: :model do
  ...
  context 'with default state machine' do
    ...
    it 'has the following transitions' do
      transitions.each do |transition|
        transition = Viauco::Order.find_transition(from: transition.keys.first, to: transition.values.first)
        expect(transition).not_to be_nil
      end
    end

    it 'does not have a transition from delivery to confirm' do
      transition = Viauco::Order.find_transition(from: :delivery, to: :confirm)
      expect(transition).to be_nil
    end

    it '.find_transition when contract was broken' do
      expect(Viauco::Order.find_transition(foo: :bar, baz: :dog)).to be_falsey
    end

    it '.remove_transition' do
      options = { from: transitions.first.keys.first, to: transitions.first.values.first }
      expect(Viauco::Order).to receive_messages(
        removed_transitions: [],
        next_event_transitions: transitions.dup
      )
      expect(Viauco::Order.remove_transition(options)).to be_truthy
      expect(Viauco::Order.removed_transitions).to eql([options])
      expect(Viauco::Order.next_event_transitions).not_to include(transitions.first)
    end

    it '.remove_transition when contract was broken' do
      expect(Viauco::Order.remove_transition(nil)).to be_falsey
    end
    ...
    context 'to address' do
      ...
      context 'with a line item' do
        ...
        it "doesn't raise an error if the default address is invalid" do
          order.user = mock_model(Viauco::LegacyUser, ship_address: Viauco::Address.new, bill_address: Viauco::Address.new)
      ...
      it 'cannot transition to address without any line items' do
        ...
        expect { order.next! }.to raise_error(StateMachines::InvalidTransition, /#{Viauco.t(:there_are_no_items_for_this_order)}/)
      end
    end
    ...
    context 'from address' do
      ...
      it 'updates totals' do
        ...
        allow(Viauco::TaxRate).to receive_messages match: [tax_rate]
        ...
        context 'with an existing shipment' do
          ...
          context 'if there are no shipping rates for any shipment' do
            it 'raises an InvalidTransitionError' do
              ...
              expect(transition).to raise_error(StateMachines::InvalidTransition, /#{Viauco.t(:items_cannot_be_shipped)}/)
            end
    ...
    context 'to delivery' do
      context 'when order has default selected_shipping_rate_id' do
        ...
        let(:shipping_rate) do
          [
            Viauco::ShippingRate.create!(shipping_method: shipping_method, cost: 10.00, shipment: shipment)
          ]
        end
    ...
    context 'from payment' do
      ...
      context 'without confirmation required' do
        before do
          order.email = 'viauco@example.com'
          allow(order).to receive_messages confirmation_required?: false
          allow(order).to receive_messages payment_required?: true
        ...
        context 'when there is only an invalid payment' do
          ...
          it 'raises a StateMachine::InvalidTransition' do
            expect do
              order.next!
            end.to raise_error(StateMachines::InvalidTransition, /#{Viauco.t(:no_payment_found)}/)

            expect(order.errors[:base]).to include(Viauco.t(:no_payment_found))
          end
        end
      end
  ...
  context 'subclassed order' do
    ...
    #   the other test pass
    class SubclassedOrder < Viauco::Order
      checkout_flow do
        go_to_state :payment
  ...
  context 're-define checkout flow' do
    ...
    it 'does not keep old event transitions when checkout_flow is redefined' do
      expect(Viauco::Order.next_event_transitions).to eq([{ cart: :payment }, { payment: :complete }])
    end

    it 'does not keep old events when checkout_flow is redefined' do
      state_machine = Viauco::Order.state_machine
  ...
  describe 'update_from_params' do
    ...
    context 'passing a credit card' do
      let(:permitted_params) do
        Viauco::PermittedAttributes.checkout_attributes +
          [payments_attributes: Viauco::PermittedAttributes.payment_attributes]
      end
      ...
      it 'sets confirmation value when its available via :cvc_confirm' do
        allow(Viauco::CreditCard).to receive_messages find: credit_card
      ...
      it 'sets existing card as source for new payment' do
        expect do
          order.update_from_params(params, permitted_params)
        end.to change { Viauco::Payment.count }.by(1)

        expect(Viauco::Payment.last.source).to eq credit_card
      end
      ...
      it 'dont let users mess with others users cards' do
        ...
        expect do
          order.update_from_params(params, permitted_params)
        end.to raise_error(Viauco.t(:invalid_credit_card))
      end
    end
