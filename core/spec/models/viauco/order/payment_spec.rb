  ...
  describe Viauco::Order, type: :model do
    ...
    context 'processing payments' do
      before do
        ...
        Viauco::Config[:auto_capture] = true
    ...
    context 'ensure source attributes stick around' do
      # For the reason of this test, please see viauco/viauco_gateway#132
      it 'does not have inverse_of defined' do
        expect(Viauco::Order.reflections['payments'].options[:inverse_of]).to be_nil
      end

      it 'keeps source attributes after updating' do
        persisted_order = Viauco::Order.create
    ...
    context '#process_payments!' do
      ...
      # Regression spec for https://github.com/viauco/viauco/issues/5436
      ...
      context 'when a payment raises a GatewayError' do
        before { expect(payment).to receive(:process!).and_raise(Viauco::Core::GatewayError) }

        it 'returns true when configured to allow checkout on gateway failures' do
          Viauco::Config.set allow_checkout_on_gateway_error: true
          expect(order.process_payments!).to be true
        end

        it 'returns false when not configured to allow checkout on gateway failures' do
          Viauco::Config.set allow_checkout_on_gateway_error: false
          expect(order.process_payments!).to be false
        end
      end

      # Regression spec for https://github.com/viauco/viauco/issues/8148
    ...
    context '#authorize_payments!' do
      ...
      let(:payment) { stub_model(Viauco::Payment) }
