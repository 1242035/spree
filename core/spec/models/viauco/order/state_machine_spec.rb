...
describe Viauco::Order, type: :model do
  ...
  before do
    ...
    Viauco::Order.define_state_machine!
  ...
  context '#next!' do
    ...
    context 'when current state is delivery' do
      ...
      it 'adjusts tax rates when transitioning to delivery' do
        ...
        expect(Viauco::TaxRate).to receive(:adjust).once
      ...
      it 'adjusts tax rates twice if there are any shipments' do
        ...
        expect(Viauco::TaxRate).to receive(:adjust).twice
        allow(order).to receive :set_shipments_cost
        order.next!
      end
    end
  ...
  context '#can_cancel?' do
    ...
    (Viauco::Shipment.state_machine.states.keys - [:pending, :backorder, :ready]).each do |shipment_state|
  ...
  context '#cancel' do
    ...
    let!(:inventory_units) do
      [stub_model(Viauco::InventoryUnit, variant: variant),
       stub_model(Viauco::InventoryUnit, variant: variant)]
    end
    let!(:shipment) do
    ...
    before do
      ...
      allow_any_instance_of(Viauco::OrderUpdater).to receive(:update_adjustment_total).and_return(10)
    end
    ...
    it 'sends a cancel email' do
      ...
      order_id = nil
      expect(Viauco::OrderMailer).to receive(:cancel_email) { |*args|
        order_id = args[0]
        mail_message
      }
    ...
    context 'resets payment state' do
      ...
      before do
        ...
        allow(Viauco::OrderMailer).to receive(:cancel_email).and_return(mail_message = double)
