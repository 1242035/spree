...
describe Viauco::InventoryUnit, type: :model do
  ...
  describe 'scopes' do
    ...
    describe '.backordered' do
      it { expect(Viauco::InventoryUnit.backordered).to eq([inventory_unit_2]) }
    end

    describe '.on_hand' do
      it { expect(Viauco::InventoryUnit.on_hand).to eq([inventory_unit_1]) }
    end

    describe '.on_hand_or_backordered' do
      it { expect(Viauco::InventoryUnit.on_hand_or_backordered).to match_array([inventory_unit_1, inventory_unit_2]) }
    end

    describe '.shipped' do
      it { expect(Viauco::InventoryUnit.shipped).to eq([inventory_unit_3]) }
    end

    describe '.returned' do
      it { expect(Viauco::InventoryUnit.returned).to eq([inventory_unit_4]) }
    end
  end
  ...
  context '#backordered_for_stock_item' do
    ...
    it 'returns modifiable objects' do
      units = Viauco::InventoryUnit.backordered_for_stock_item(stock_item)
    ...
    it "finds inventory units from its stock location when the unit's variant matches the stock item's variant" do
      expect(Viauco::InventoryUnit.backordered_for_stock_item(stock_item)).to match_array([unit])
    end
    ...
    it "does not find inventory units that aren't backordered" do
      ...
      expect(Viauco::InventoryUnit.backordered_for_stock_item(stock_item)).not_to include(on_hand_unit)
    end
    ...
    it "does not find inventory units that don't match the stock item's variant" do
      ...
      expect(Viauco::InventoryUnit.backordered_for_stock_item(stock_item)).not_to include(other_variant_unit)
    end
    ...
    context 'other shipments' do
      ...
      it 'does not find inventory units belonging to incomplete orders' do
        expect(Viauco::InventoryUnit.backordered_for_stock_item(stock_item)).not_to include(other_unit)
      end
    end
  end
