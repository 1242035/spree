...
class DummyShippingCalculator < Viauco::ShippingCalculator
end

describe Viauco::ShippingMethod, type: :model do
  ...
  context 'calculators' do
    it "rejects calculators that don't inherit from Viauco::ShippingCalculator" do
      allow(Viauco::ShippingMethod).to receive_message_chain(:viauco_calculators, :shipping_methods).and_return([
                                                                                                                Viauco::Calculator::Shipping::FlatPercentItemTotal,
                                                                                                                Viauco::Calculator::Shipping::PriceSack,
                                                                                                                Viauco::Calculator::DefaultTax,
                                                                                                                DummyShippingCalculator # included as regression test for https://github.com/viauco/viauco/issues/3109
                                                                                                              ])

      expect(Viauco::ShippingMethod.calculators).to eq([Viauco::Calculator::Shipping::FlatPercentItemTotal, Viauco::Calculator::Shipping::PriceSack, DummyShippingCalculator])
      expect(Viauco::ShippingMethod.calculators).not_to eq([Viauco::Calculator::DefaultTax])
    end
  end
        ...
        it 'adds error to base' do
          expect(subject.errors.messages[:base]).to include(I18n.t(:required_shipping_category,
                                                            scope: [
                                                              ...
                                                              'viauco/shipping_method', :attributes, :base
                                                            ]))
        end
      end
  ...
  describe '#available_to_display?' do
    context 'when available on frontend' do
      it { expect(frontend_shipping_method.available_to_display?(Viauco::ShippingMethod::DISPLAY_ON_FRONT_END)).to be true }
      it { expect(backend_shipping_method.available_to_display?(Viauco::ShippingMethod::DISPLAY_ON_FRONT_END)).to be false }
      it { expect(front_and_back_end_shipping_method.available_to_display?(Viauco::ShippingMethod::DISPLAY_ON_FRONT_END)).to be true }
    end

    context 'when available on backend' do
      it { expect(frontend_shipping_method.available_to_display?(Viauco::ShippingMethod::DISPLAY_ON_BACK_END)).to be false }
      it { expect(backend_shipping_method.available_to_display?(Viauco::ShippingMethod::DISPLAY_ON_BACK_END)).to be true }
      it { expect(front_and_back_end_shipping_method.available_to_display?(Viauco::ShippingMethod::DISPLAY_ON_BACK_END)).to be true }
    end
  end
