...
describe Viauco::Payment, type: :model do
  let(:order) { Viauco::Order.create }
  let(:refund_reason) { create(:refund_reason) }

  let(:gateway) do
    gateway = Viauco::Gateway::Bogus.new(active: true)
  ...
  describe 'Constants' do
    it { expect(Viauco::Payment::INVALID_STATES).to eq(%w(failed invalid)) }
  end

  describe 'scopes' do
    describe '.valid' do
      subject { Viauco::Payment.valid }
  ...
  context 'validations' do
    context 'when payment source is not required' do
      it 'do not validate source presence' do
        allow_any_instance_of(Viauco::PaymentMethod).to receive(:source_required?).and_return(false)
    ...
    it 'returns useful error messages when source is invalid' do
      payment.source = Viauco::CreditCard.new
      expect(payment).not_to be_valid
  ...
  # Regression test for https://github.com/viauco/viauco/pull/2224
  context 'failure' do
  ...
  context 'processing' do
    describe '#process!' do
      ...
      it 'invalidates if payment method doesnt support source' do
        ...
        expect { payment.process! }.to raise_error(Viauco::Core::GatewayError)
        expect(payment.state).to eq('invalid')
      end
    ...
    describe '#authorize!' do
      ...
      context 'if unsuccessful' do
        it 'marks payment as failed' do
          ...
          expect do
            payment.authorize!
          end.to raise_error(Viauco::Core::GatewayError)
        end
      end
    end
    ...
    describe '#purchase!' do
      ...
      context 'if unsuccessful' do
        ...
        it 'makes payment failed' do
          expect { payment.purchase! }.to raise_error(Viauco::Core::GatewayError)
        end
      ...
      context 'when payment is pending' do
        ...
        context 'if unsuccessful' do
          it 'does not make payment complete' do
            ...
            expect { payment.capture! }.to raise_error(Viauco::Core::GatewayError)
          end
        end
      end
    ...
    describe '#void_transaction!' do
      ...
      context 'if unsuccessful' do
        it 'does not void the payment' do
          ...
          expect { payment.void_transaction! }.to raise_error(Viauco::Core::GatewayError)
        end
      end
  ...
  context 'with source required' do
    context 'raises an error if no source is specified' do
      ...
      specify do
        expect { payment.process! }.to raise_error(Viauco::Core::GatewayError, Viauco.t(:payment_processing_failed))
      end
    end
  end
  ...
  describe '#save' do
    ...
    context 'not completed payments' do
      it "doesn't update order payment total" do
        expect do
          Viauco::Payment.create(amount: 100, order: order)
        end.not_to change(order, :payment_total)
      end

      it 'requires a payment method' do
        expect(Viauco::Payment.create(amount: 100, order: order).errors).not_to be_empty
        expect(Viauco::Payment.create(amount: 100, order: order).errors.messages[:payment_method]).to be_present
      end
    end

    context 'when the payment was completed but now void' do
      let(:payment) do
        Viauco::Payment.create(
          amount: 100,
          order: order,
          state: 'completed'
        )
    ...
    context 'completed orders' do
      ...
      it 'updates payment_state and shipments' do
        ...
        Viauco::Payment.create(amount: 100, order: order, payment_method: gateway, source: card)
      end
    end
    ...
    context 'when profiles are supported' do
      ...
      context 'with multiple payment attempts' do
        ...
        it 'does not try to create profiles on old failed payment attempts' do
          allow_any_instance_of(Viauco::Payment).to receive(:payment_method) { gateway }
    ...
    let(:params) do
      {
        ...
        source_attributes: {
          ...
          name: 'Viauco Commerce'
        }
      }
    end

    it "builds the payment's source" do
      payment = Viauco::Payment.new(params)
      expect(payment).to be_valid
  ...
  describe '#display_amount' do
    it 'returns a Viauco::Money for this amount' do
      expect(payment.display_amount).to eq(Viauco::Money.new(payment.amount))
    end
  end
  ...
  describe '#gateway_options' do
    ...
    it 'contains the email address from a persisted order' do
      ...
      payment.order = Viauco::Order.find(payment.order_id)
