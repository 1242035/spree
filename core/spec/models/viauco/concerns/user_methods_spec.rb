...
describe Viauco::UserMethods do
  let(:test_user) { create :user }
  let(:current_store) { create :store }

  describe '#has_viauco_role?' do
    subject { test_user.has_viauco_role? name }

    let(:role) { Viauco::Role.create(name: name) }
    let(:name) { 'test' }

    context 'with a role' do
      before { test_user.viauco_roles << role }
  ...
  describe '#last_incomplete_viauco_order' do
    subject { test_user.last_incomplete_viauco_order(current_store) }
  ...
  context '#check_completed_orders' do
    ...
    context 'rstrict t delete dependent destroyed' do
      ...
      it 'does not destroy dependent destroy items' do
        expect { test_user.destroy }.to raise_error(Viauco::Core::DestroyWithOrdersError)
