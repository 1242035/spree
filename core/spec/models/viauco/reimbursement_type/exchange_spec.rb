...
module Viauco
  describe ReimbursementType::Exchange, type: :model do
    describe '.reimburse' do
      subject { Viauco::ReimbursementType::Exchange.reimburse(reimbursement, return_items, simulate) }
      ...
      context 'return items are supplied' do
        before do
          expect(Viauco::Exchange).to receive(:new).with(reimbursement.order, return_items).and_return(new_exchange)
        end
