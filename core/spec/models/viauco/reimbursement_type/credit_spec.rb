...
module Viauco
  describe ReimbursementType::Credit, type: :model do
    subject { Viauco::ReimbursementType::Credit.reimburse(reimbursement, [return_item], simulate) }
    ...
    class DummyCreditable < Viauco::Base
      attr_accessor :amount
      self.table_name = 'viauco_payments' # Your creditable class should not use this table
    end

    before do
      reimbursement.update!(total: reimbursement.calculated_total)
      allow(Viauco::StoreCredit).to receive(:new).and_return(creditable)
    end
