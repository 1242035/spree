...
module Viauco
  describe ReimbursementType::OriginalPayment, type: :model do
    subject { Viauco::ReimbursementType::OriginalPayment.reimburse(reimbursement, [return_item], simulate) }
    ...
    describe '.reimburse' do
      ...
      context 'when no credit is allowed on the payment' do
        before do
          expect_any_instance_of(Viauco::Payment).to receive(:credit_allowed).and_return 0
        end
      ...
      context 'when a payment is negative' do
        before do
          expect_any_instance_of(Viauco::Payment).to receive(:amount).and_return(-100)
        end
