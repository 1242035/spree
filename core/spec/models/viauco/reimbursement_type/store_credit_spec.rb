...
module Viauco
  describe ReimbursementType::StoreCredit do
    subject do
      Viauco::ReimbursementType::StoreCredit.reimburse(reimbursement, [return_item, return_item2],
                                                      simulate)
    end
    ...
    let!(:default_refund_reason) do
      Viauco::RefundReason.find_or_create_by!(name: Viauco::RefundReason::RETURN_PROCESSING_REASON,
                                             mutable: false)
    end

    let!(:primary_credit_type) { create(:primary_credit_type) }
    let!(:created_by_user) { create(:user, email: Viauco::StoreCredit::DEFAULT_CREATED_BY_EMAIL) }
    ...
    describe '.reimburse' do
      context 'simulate is true' do
        ...
        context 'for store credits that the customer used' do
          ...
          it 'creates readonly refunds for all store credit payments' do
            expect(subject.map(&:class)).to eq [Viauco::Refund]
        ...
        context 'for return items that were not paid for with store credit' do
          context 'creates one readonly lump credit for all outstanding balance payable to the customer' do
            it 'creates a credit that is read only' do
              expect(subject.map(&:class)).to eq [Viauco::Reimbursement::Credit]
              expect(subject.map(&:readonly?)).to eq [true]
            end
          ...
          it 'does not save to the database' do
            expect { subject }.not_to change { Viauco::Reimbursement::Credit.count }
          end
        end
      end
      ...
      context 'simulate is false' do
        ...
        context 'for return items that were not paid for with store credit' do
          before do
            allow(Viauco::ReimbursementType::StoreCredit).to receive(:store_credit_payments).and_return([])
          end

          it 'creates one lump credit for all outstanding balance payable to the customer' do
            expect { subject }.to change { Viauco::Reimbursement::Credit.count }.by(1)
          ...
          it "creates a store credit with the same currency as the reimbursement's order" do
            ...
            expect(Viauco::StoreCredit.last.currency).to eq reimbursement.order.currency
          end
        end
      end
    end
