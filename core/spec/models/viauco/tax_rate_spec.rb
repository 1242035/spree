...
describe Viauco::TaxRate, type: :model do
  context 'match' do
    let(:order) { create(:order) }
    let(:country) { create(:country) }
    let(:tax_category) { create(:tax_category) }
    let(:calculator) { Viauco::Calculator::FlatRate.new }

    it 'returns an empty array when tax_zone is nil' do
      allow(order).to receive_messages tax_zone: nil
      expect(Viauco::TaxRate.match(order.tax_zone)).to eq([])
    end

    context 'when no rate zones match the tax zone' do
      before do
        Viauco::TaxRate.create(amount: 1, zone: create(:zone))
      end
      ...
      context 'when there is no default tax zone' do
        ...
        it 'returns the rate that matches the rate zone' do
          ...
          allow(order).to receive_messages tax_zone: @zone
          expect(Viauco::TaxRate.match(order.tax_zone)).to eq([rate])
        end

        it 'returns all rates that match the rate zone' do
          rate1 = Viauco::TaxRate.create(
            amount: 1,
            zone: @zone,
            tax_category: tax_category,
            calculator: calculator
          ...
          allow(order).to receive_messages tax_zone: @zone
          expect(Viauco::TaxRate.match(order.tax_zone)).to match_array([rate1, rate2])
        end
      ...
      context 'when there is a default tax zone' do
        subject { Viauco::TaxRate.match(order.tax_zone) }
        ...
        let!(:rate) do
          Viauco::TaxRate.create(amount: 1,
                                zone: @zone,
                                tax_category: tax_category,
                                calculator: calculator,
  ...
  describe '.adjust' do
    let(:order) { stub_model(Viauco::Order) }
    let(:tax_category_1) { stub_model(Viauco::TaxCategory) }
    let(:tax_category_2) { stub_model(Viauco::TaxCategory) }
    let(:rate_1) { stub_model(Viauco::TaxRate, tax_category: tax_category_1) }
    let(:rate_2) { stub_model(Viauco::TaxRate, tax_category: tax_category_2) }

    context 'with line items' do
      let(:line_item) do
        stub_model(Viauco::LineItem,
                   price: 10.0,
                   quantity: 1,
                   tax_category: tax_category_1,
                   variant: stub_model(Viauco::Variant))
      end

      let(:line_items) { [line_item] }

      before do
        allow(Viauco::TaxRate).to receive_messages match: [rate_1, rate_2]
      end
      ...
      it 'applies adjustments for two tax rates to the order' do
        ...
        Viauco::TaxRate.adjust(order, line_items)
      end
    end
    ...
    context 'with shipments' do
      let(:shipments) { [stub_model(Viauco::Shipment, cost: 10.0, tax_category: tax_category_1)] }
      ...
      it 'applies adjustments for two tax rates to the order' do
        expect(rate_1).to receive(:adjust)
        expect(rate_2).not_to receive(:adjust)
        Viauco::TaxRate.adjust(order, shipments)
      end
    end
    ...
    context 'for MOSS taxation in Europe' do
      ...
      let(:moss_category) { Viauco::TaxCategory.create(name: 'Digital Goods') }
      let(:normal_category) { Viauco::TaxCategory.create(name: 'Analogue Goods') }
      let(:eu_zone) { create(:zone, name: 'EU') }

      let!(:german_vat) do
        Viauco::TaxRate.create(
          name: 'German VAT',
          amount: 0.19,
          calculator: Viauco::Calculator::DefaultTax.create,
      ...
      context 'a download' do
        before do
          Viauco::Cart::AddItem.call(order: order, variant: download.master)
        end

        it 'without an adress costs 100 euros including tax' do
          Viauco::TaxRate.adjust(order, order.line_items)
          order.update_with_updater!
          expect(order.display_total).to eq(Viauco::Money.new(100))
        ...
        it 'to france costs more including tax' do
          ...
          Viauco::TaxRate.adjust(order, order.line_items)
          order.update_with_updater!
          expect(order.display_total).to eq(Viauco::Money.new(105.04))
        ...
        it 'to somewhere else costs the net amount' do
          ...
          expect(order.included_tax_total).to eq(0)
          expect(order.display_total).to eq(Viauco::Money.new(84.03))
        end
      end
      ...
      context 'a t-shirt' do
        it 'to germany costs 100 euros including tax' do
          allow(order).to receive(:tax_zone).and_return(germany_zone)
          Viauco::Cart::AddItem.call(order: order, variant: tshirt.master)
          Viauco::TaxRate.adjust(order, order.line_items)
          order.update_with_updater!
          expect(order.display_total).to eq(Viauco::Money.new(100))
          expect(order.included_tax_total).to eq(15.97)
        ...
        it 'to france costs 100 euros including tax' do
          ...
          Viauco::TaxRate.adjust(order, order.line_items)
          order.update_with_updater!
          expect(order.display_total).to eq(Viauco::Money.new(100.00))
          expect(order.included_tax_total).to eq(15.97)
          expect(order.additional_tax_total).to eq(0)
        end
  ...
  describe '.included_tax_amount_for' do
    subject(:included_tax_amount) { Viauco::TaxRate.included_tax_amount_for(price_options) }
  ...
  describe '#adjust' do
    before do
      ...
      @zone = create(:zone, name: 'Country Zone', default_tax: true, zone_members: [])
      @zone.zone_members.create(zoneable: @country)
      @category    = Viauco::TaxCategory.create name: 'Taxable Foo'
      @category2   = Viauco::TaxCategory.create(name: 'Non Taxable')
      @rate1 = Viauco::TaxRate.create(
        amount: 0.10,
        calculator: Viauco::Calculator::DefaultTax.create,
    ...
    context 'not taxable line item ' do
      let!(:line_item) { Viauco::Cart::AddItem.call(order: @order, variant: @nontaxable.master).value }
    ...
    context 'taxable line item' do
      let!(:line_item) { Viauco::Cart::AddItem.call(order: @order, variant: @taxable.master).value }

      context 'when price includes tax' do
        before do
          @rate1.update_column(:included_in_price, true)
          @rate2.update_column(:included_in_price, true)
          Viauco::TaxRate.store_pre_tax_amount(line_item, [@rate1, @rate2])
        end
        ...
        context "when order's zone is neither the default zone, or included in the default zone, but matches the rate's zone" do
          before do
            new_rate = Viauco::TaxRate.create(
              amount: 0.2,
              included_in_price: true,
              calculator: Viauco::Calculator::DefaultTax.create,
              tax_category: @category,
        ...
        context 'when price does not include tax' do
          ...
          describe 'tax adjustments' do
            before { Viauco::TaxRate.adjust(@order, @order.line_items) }
