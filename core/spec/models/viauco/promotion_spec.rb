...
describe Viauco::Promotion, type: :model do
  let(:promotion) { Viauco::Promotion.new }

  describe 'validations' do
    before do
      @valid_promotion = Viauco::Promotion.new name: 'A promotion'
    end
    ...
    describe 'expires_at_must_be_later_than_starts_at' do
      ...
      context 'starts_at is a date earlier than expires_at' do
        ...
        context 'is not valid' do
          ...
          it { expect(@valid_promotion.errors[:expires_at]).to include(I18n.t(:invalid_date_range, scope: 'activerecord.errors.models.viauco/promotion.attributes.expires_at')) }
        end
      end
  ...
  describe 'scopes' do
    describe '.coupons' do
      subject { Viauco::Promotion.coupons }

      let!(:promotion_without_code) { Viauco::Promotion.create! name: 'test', code: '' }
      let!(:promotion_with_code) { Viauco::Promotion.create! name: 'test1', code: 'code' }
    ...
    describe '.applied' do
      subject { Viauco::Promotion.applied }

      let!(:promotion_not_applied) { Viauco::Promotion.create! name: 'test', code: '' }
      let(:order) { create(:order) }
      let!(:promotion_applied) do
        promotion = Viauco::Promotion.create!(name: 'test1', code: '')
    ...
    describe '.advertised' do
      subject { Viauco::Promotion.advertised }

      let!(:promotion_not_advertised) { Viauco::Promotion.create! name: 'test', advertise: false }
      let!(:promotion_advertised) { Viauco::Promotion.create! name: 'test1', advertise: true }
  ...
  describe '#destroy' do
    let(:promotion) { Viauco::Promotion.create(name: 'delete me') }

    before do
      promotion.actions << Viauco::Promotion::Actions::CreateAdjustment.new
      promotion.rules << Viauco::Promotion::Rules::FirstOrder.new
      promotion.save!
      promotion.destroy
    end

    it 'deletes actions' do
      expect(Viauco::PromotionAction.count).to eq(0)
    end

    it 'deletes rules' do
      expect(Viauco::PromotionRule.count).to eq(0)
    end
  end
  ...
  describe '#save' do
    ...
    it 'deeply autosaves records and preferences' do
      ...
      promotion.save!
      expect(Viauco::Calculator.first.preferred_flat_percent).to eq(10)
    end
  end

  describe '#activate' do
    before do
      @action1 = Viauco::Promotion::Actions::CreateAdjustment.create!
      @action2 = Viauco::Promotion::Actions::CreateAdjustment.create!
      ...
      @user = stub_model(Viauco::LegacyUser, email: 'viauco@example.com')
      @order = Viauco::Order.create user: @user
  ...
  context '#credits_count' do
    ...
    let!(:action) do
      ...
      action = Viauco::Promotion::Actions::CreateAdjustment.create(action_params)
  ...
  context '#adjusted_credits_count' do
    ...
    let(:promotion) { Viauco::Promotion.create name: 'promo', code: '10off' }
    let(:order_action) do
      action = Viauco::Promotion::Actions::CreateAdjustment.create(calculator: Viauco::Calculator::FlatPercentItemTotal.new)
      promotion.actions << action
      action
    end
    let(:item_action) do
      action = Viauco::Promotion::Actions::CreateItemAdjustments.create(calculator: Viauco::Calculator::FlatPercentItemTotal.new)
      promotion.actions << action
      action
    end
  ...
  context '#products' do
    ...
    context 'when it has product rules with products associated' do
      let(:promotion_rule) { create(:promotion_rule, promotion: promotion, type: 'Viauco::Promotion::Rules::Product') }
  ...
  context '#eligible?' do
    ...
    context 'when promotable is a Viauco::LineItem' do
    ...
    context 'when promotable is a Viauco::Order' do
      let(:promotable) { create :order }
  ...
  context '#eligible_rules' do
    ...
    it 'true if there are no applicable rules' do
      promotion.promotion_rules = [stub_model(Viauco::PromotionRule, eligible?: true, applicable?: false)]
      allow(promotion.promotion_rules).to receive(:for).and_return([])
      expect(promotion.eligible_rules(promotable)).to eq []
    end

    context "with 'all' match policy" do
      let(:promo1) { Viauco::PromotionRule.create! }
      let(:promo2) { Viauco::PromotionRule.create! }
    ...
    context "with 'any' match policy" do
      let(:promotion) { Viauco::Promotion.create(name: 'Promo', match_policy: 'any') }
      let(:promotable) { double('Promotable') }

      it 'has eligible rules if any of the rules are eligible' do
        allow_any_instance_of(Viauco::PromotionRule).to receive_messages(applicable?: true)
        true_rule = Viauco::PromotionRule.create(promotion: promotion)
      ...
      context 'when none of the rules are eligible' do
        let(:promo) { Viauco::PromotionRule.create! }
        let(:errors) { double ActiveModel::Errors, empty?: false }
  ...
  describe '#line_item_actionable?' do
    ...
    let(:order) { double Viauco::Order }
    let(:line_item) { double Viauco::LineItem }
    let(:true_rule) { double Viauco::PromotionRule, eligible?: true, applicable?: true, actionable?: true }
    let(:false_rule) { double Viauco::PromotionRule, eligible?: true, applicable?: true, actionable?: false }
    let(:rules) { [] }
  ...
  describe 'normalize blank values for code & path' do
    it 'will save blank value as nil value instead' do
      promotion = Viauco::Promotion.create(name: 'A promotion', code: '', path: '')
  ...
  describe '#with_coupon_code' do
    context 'and code stored in uppercase' do
      ...
      it 'finds the code with lowercase' do
        expect(Viauco::Promotion.with_coupon_code('my-coupon-123')).to eql promotion
      end
    end
    ...
    context 'when promotion has no actions' do
      ...
      it 'then returns the one with an action' do
        expect(Viauco::Promotion.with_coupon_code('MY-COUPON-123')).to be_nil
      end
    end
  end
  ...
  describe 'adding items to the cart' do
    ...
    it 'updates the promotions for new line items' do
      ...
      other_line_item = Viauco::Cart::AddItem.call(order: order, variant: variant, options: { currency: order.currency }).value
  ...
  describe '#generate_code' do
    let(:promotion) { create(:promotion, code: 'viauco123') }

    context 'with generate_code' do
      it 'has a generated code' do
        promotion.generate_code = true
        expect(promotion.code).not_to eq 'viauco123'
      end
    end

    context 'without generate_code' do
      it 'has a generated code' do
        expect(promotion.code).to eq 'viauco123'
      end
    end
  end
end
