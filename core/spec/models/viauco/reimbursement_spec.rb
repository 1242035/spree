...
describe Viauco::Reimbursement, type: :model do
  describe '#display_total' do
    subject { reimbursement.display_total }

    let(:total)         { 100.50 }
    let(:currency)      { 'USD' }
    let(:order)         { Viauco::Order.new(currency: currency) }
    let(:reimbursement) { Viauco::Reimbursement.new(total: total, order: order) }

    it 'returns the value as a Viauco::Money instance' do
      expect(subject).to eq Viauco::Money.new(total)
    end
  ...
  describe '#perform!' do
    ...
    let!(:default_refund_reason) { Viauco::RefundReason.find_or_create_by!(name: Viauco::RefundReason::RETURN_PROCESSING_REASON, mutable: false) }

    let(:reimbursement) { create(:reimbursement, customer_return: customer_return, order: order, return_items: [return_item]) }

    let(:store_credit_reimbursement_type) { create(:reimbursement_type, name: 'StoreCredit', type: 'Viauco::ReimbursementType::StoreCredit') }
    ...
    it 'creates a refund' do
      expect do
        subject
      end.to change { Viauco::Refund.count }.by(1)
      expect(Viauco::Refund.last.amount).to eq order.total
    end
    ...
    context 'with additional tax' do
      ...
      it 'saves the additional tax and refunds the total' do
        ...
        expect(Viauco::Refund.last.amount).to eq line_item.pre_tax_amount + line_item.additional_tax_total
      end
    end
    ...
    context 'with included tax' do
      ...
      it 'saves the included tax and refunds the total' do
        ...
        expect(Viauco::Refund.last.amount).to eq (line_item.pre_tax_amount + line_item.included_tax_total).round(2)
      end
    end
    ...
    context 'when reimbursement cannot be fully performed' do
      ...
      it 'raises IncompleteReimbursement error' do
        expect { subject }.to raise_error(Viauco::Reimbursement::IncompleteReimbursementError)
      end
    end
    ...
    it 'triggers the reimbursement mailer to be sent' do
      expect(Viauco::ReimbursementMailer).to receive(:reimbursement_email).with(reimbursement.id) { double(deliver_later: true) }
      subject
    end
  end
  ...
  describe '#calculated_total' do
    context 'with return item amounts that would round up if added' do
      ...
      let(:reimbursement) { Viauco::Reimbursement.new }

      before do
        reimbursement.return_items << Viauco::ReturnItem.new(pre_tax_amount: 10.003)
        reimbursement.return_items << Viauco::ReturnItem.new(pre_tax_amount: 10.003)
      end
    ...
    context 'with a return item amount that should round up' do
      ...
      before do
        reimbursement.return_items << Viauco::ReturnItem.new(pre_tax_amount: 19.998)
      end
  ...
  describe '.build_from_customer_return' do
    subject { Viauco::Reimbursement.build_from_customer_return(customer_return) }
