...
RSpec.describe Viauco::Payment::GatewayOptions, type: :model do
  let(:options) { Viauco::Payment::GatewayOptions.new(payment) }
  ...
  let(:payment_method) do
    double(
      ...
      exchange_multiplier: Viauco::Gateway::FROM_DOLLAR_TO_CENT_RATE
    )
  end
  ...
  let(:bill_address) do
    double Viauco::Address, active_merchant_hash: { bill: :address }
  end

  let(:ship_address) do
    double Viauco::Address, active_merchant_hash: { ship: :address }
  end
