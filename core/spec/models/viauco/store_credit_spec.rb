...
describe 'StoreCredit' do
  ...
  describe 'callbacks' do
    ...
    context 'amount used is greater than zero' do
      ...
      let(:validation_message) { I18n.t('activerecord.errors.models.viauco/store_credit.attributes.amount_used.greater_than_zero_restrict_delete') }
  ...
  describe 'validations' do
    describe 'used amount should not be greater than the credited amount' do
      context 'the used amount is defined' do
        ...
        it 'sets the correct error message' do
          ...
          attribute_name = I18n.t('activerecord.attributes.viauco/store_credit.amount_used')
          validation_message = I18n.t('activerecord.errors.models.viauco/store_credit.attributes.amount_used.cannot_be_greater_than_amount')
    ...
    describe 'amount used less than or equal to amount' do
      ...
      it 'adds an error message about the invalid amount used' do
        subject.valid?
        text = I18n.t('activerecord.errors.models.viauco/store_credit.attributes.amount_used.cannot_be_greater_than_amount')
    ...
    describe 'amount authorized less than or equal to amount' do
      ...
      it 'adds an error message about the invalid authorized amount' do
        subject.valid?
        text = I18n.t('activerecord.errors.models.viauco/store_credit.attributes.amount_authorized.exceeds_total_credits')
  ...
  describe '#display_amount' do
    it 'returns a Viauco::Money instance' do
      expect(store_credit.display_amount).to be_instance_of(Viauco::Money)
    end
  end

  describe '#display_amount_used' do
    it 'returns a Viauco::Money instance' do
      expect(store_credit.display_amount_used).to be_instance_of(Viauco::Money)
    end
  end
      ...
      context 'amount has not been authorized yet' do
        ...
        context 'originator is present' do
          ...
          it 'records the originator' do
            expect { subject }.to change { Viauco::StoreCreditEvent.count }.by(1)
            expect(Viauco::StoreCreditEvent.last.originator).to eq originator
          end
        end
      end
  ...
  describe '#validate_authorization' do
    context 'insufficient funds' do
      ...
      it 'adds an error to the model' do
        subject
        text = Viauco.t('store_credit_payment_method.insufficient_funds')
    ...
    context 'currency mismatch' do
      ...
      it 'adds an error to the model' do
        subject
        text = Viauco.t('store_credit_payment_method.currency_mismatch')
        expect(store_credit.errors.full_messages).to include(text)
      end
    end
  ...
  describe '#capture' do
    ...
    context 'insufficient funds' do
      ...
      it 'adds an error to the model' do
        subject
        text = Viauco.t('store_credit_payment_method.insufficient_authorized_amount')
        expect(store_credit.errors.full_messages).to include(text)
      end
  ...
  describe '#void' do
    ...
    context 'no event found for auth_code' do
      ...
      it 'adds an error to the model' do
        subject
        text = Viauco.t('store_credit_payment_method.unable_to_void', auth_code: auth_code)
        expect(store_credit.errors.full_messages).to include(text)
      end
    end
    ...
    context 'capture event found for auth_code' do
      ...
      let!(:capture_event) do
        create(:store_credit_auth_event,
               action: Viauco::StoreCredit::CAPTURE_ACTION,
  ...
  describe '#credit' do
    ...
    context 'unable to find capture event' do
      ...
      it 'adds an error message about the currency mismatch' do
        subject
        text = Viauco.t('store_credit_payment_method.unable_to_credit', auth_code: auth_code)
        expect(store_credit.errors.full_messages).to include(text)
      end
    end
    ...
    context 'amount is successfully credited' do
      ...
      context 'credit_to_new_allocation is set' do
        before { Viauco::Config[:credit_to_new_allocation] = true }
        ...
        it 'creates a new store credit record' do
          expect { subject }.to change { Viauco::StoreCredit.count }.by(1)
        end
        ...
        context 'credits the passed amount to a new store credit record' do
          before do
            subject
            @new_store_credit = Viauco::StoreCredit.last
          end
  ...
  describe '#amount_used' do
    context 'amount used is not defined' do
      subject { Viauco::StoreCredit.new }
  ...
  describe '#can_capture?' do
    ...
    context 'void payment' do
      let(:payment_state) { Viauco::StoreCredit::VOID_ACTION }
      ...
      context 'create' do
        context 'user has one store credit' do
          ...
          it 'makes the store credit event an allocation event' do
            expect(subject.store_credit_events.first.action).to eq Viauco::StoreCredit::ALLOCATION_ACTION
          end
        ...
        context 'an action is specified' do
          it 'creates an event with the set action' do
            ...
            store_credit.action = Viauco::StoreCredit::VOID_ACTION
            store_credit.action_authorization_code = '1-SC-TEST'

            expect { store_credit.save! }.to change {
              Viauco::StoreCreditEvent.where(action: Viauco::StoreCredit::VOID_ACTION).count
            }.by(1)
          end
        end
      end
