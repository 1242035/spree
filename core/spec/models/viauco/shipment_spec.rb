...
describe Viauco::Shipment, type: :model do
  let(:order) do
    mock_model Viauco::Order, backordered?: false,
                             canceled?: false,
                             can_ship?: true,
                             currency: 'USD',
                             number: 'S12345',
  ...
  let(:shipment) do
    shipment = Viauco::Shipment.new(cost: 1, state: 'pending', stock_location: create(:stock_location))
  ...
  let(:variant) { mock_model(Viauco::Variant) }
  let(:line_item) { mock_model(Viauco::LineItem, variant: variant) }
  ...
  describe 'precision of pre_tax_amount' do
    ...
    it 'keeps four digits of precision even when reloading' do
      ...
      allow_any_instance_of(Viauco::LineItem).to receive(:update_tax_charge)
  ...
  it 'is backordered if one if its inventory_units is backordered' do
    allow(shipment).to receive_messages(inventory_units: [
                                          mock_model(Viauco::InventoryUnit, backordered?: false),
                                          mock_model(Viauco::InventoryUnit, backordered?: true)
                                        ])
  ...
  context '#determine_state' do
    ...
    it 'returns pending if backordered' do
      allow(shipment).to receive_messages inventory_units: [mock_model(Viauco::InventoryUnit, backordered?: true)]
      expect(shipment.determine_state(order)).to eq 'pending'
    end
    ...
    it 'returns ready when Config.auto_capture_on_dispatch' do
      Viauco::Config.auto_capture_on_dispatch = true
      expect(shipment.determine_state(order)).to eq 'ready'
    end
  end

  context 'display_amount' do
    it 'retuns a Viauco::Money' do
      allow(shipment).to receive(:cost).and_return(21.22)
      expect(shipment.display_amount).to eq(Viauco::Money.new(21.22))
    end
  end
  ...
  context 'display_final_price' do
    it 'retuns a Viauco::Money' do
      ...
      expect(shipment.display_final_price).to eq(Viauco::Money.new(21.22))
    end
  end
  ...
  context 'display_item_cost' do
    it 'retuns a Viauco::Money' do
      ...
      expect(shipment.display_item_cost).to eq(Viauco::Money.new(21.22))
    end
  end
  ...
  context '#free?' do
    ...
    it 'returns when Free Shipping promotion is applied' do
      ...
      Viauco::PromotionHandler::Coupon.new(order).apply
  ...
  context 'manifest' do
    ...
    let!(:line_item) { Viauco::Cart::AddItem.call(order: order, variant: variant).value }
  ...
  context 'shipping_rates' do
    ...
    let(:shipping_rates) do
      [
        Viauco::ShippingRate.new(shipping_method: shipping_method1, cost: 10.00, selected: true),
        Viauco::ShippingRate.new(shipping_method: shipping_method2, cost: 20.00)
      ]
    end
    ...
    context 'refresh_rates' do
      ...
      it 'requests new rates, and maintain shipping_method selection' do
        expect(Viauco::Stock::Estimator).to receive(:new).with(shipment.order).and_return(mock_estimator)
      ...
      it 'does not refresh if shipment is shipped' do
        expect(Viauco::Stock::Estimator).not_to receive(:new)
        shipment.shipping_rates.delete_all
  ...
  context '#update!' do
    ...
    context 'when shipment state changes to shipped' do
      before do
        allow_any_instance_of(Viauco::ShipmentHandler).to receive(:send_shipped_email)
        allow_any_instance_of(Viauco::ShipmentHandler).to receive(:update_order_shipment_state)
      end
      ...
      context 'when using the default shipment handler' do
        it "calls the 'perform' method" do
          shipment.state = 'pending'
          allow(shipment).to receive_messages determine_state: 'shipped'
          expect_any_instance_of(Viauco::ShipmentHandler).to receive(:perform)
          shipment.update!(order)
        end
      end
      ...
      context 'when using a custom shipment handler' do
        before do
          Viauco::ShipmentHandler::UPS = Class.new do
        ...
        after do
          Viauco::ShipmentHandler.send(:remove_const, :UPS)
        end

        it "calls the custom handler's 'perform' method" do
          shipment.state = 'pending'
          allow(shipment).to receive_messages determine_state: 'shipped'
          expect_any_instance_of(Viauco::ShipmentHandler::UPS).to receive(:perform)
          shipment.update!(order)
        end
      end
      ...
      context 'with adjustments' do
        before do
          shipment.adjustments << Viauco::Adjustment.create(order: order, label: 'Label', amount: 5)
        end
  ...
  context 'when order is completed' do
    ...
    context 'with inventory tracking' do
      before { Viauco::Config.set track_inventory_levels: true }
    ...
    context 'without inventory tracking' do
      before { Viauco::Config.set track_inventory_levels: false }
  ...
  context '#cancel' do
    ...
    it 'restocks the items' do
      inventory_unit = mock_model(Viauco::InventoryUnit, state: 'on_hand', line_item: line_item, variant: variant, quantity: 1)
      allow(shipment).to receive(:inventory_units).and_return([inventory_unit])
      shipment.stock_location = mock_model(Viauco::StockLocation)
    ...
    context 'with backordered inventory units' do
      ...
      before do
        Viauco::Cart::AddItem.call(order: order, variant: variant)
        order.create_proposed_shipments

        Viauco::Cart::AddItem.call(order: other_order, variant: variant)
  ...
  context '#resume' do
    ...
    it 'unstocks them items' do
      inventory_unit = mock_model(Viauco::InventoryUnit, quantity: 1, line_item: line_item, variant: variant)
      allow(shipment).to receive(:inventory_units).and_return([inventory_unit])
      shipment.stock_location = mock_model(Viauco::StockLocation)
    ...
    ['ready', 'canceled'].each do |state|
      context "from #{state}" do
        ...
        it 'sends a shipment email' do
          ...
          shipment_id = nil
          expect(Viauco::ShipmentMailer).to receive(:shipped_email) { |*args|
            shipment_id = args[0]
            mail_message
          }
  ...
  context 'changes shipping rate via general update' do
    ...
    let(:shipment) { Viauco::Shipment.create order_id: order.id, stock_location: create(:stock_location) }

    let(:shipping_rate) do
      Viauco::ShippingRate.create shipment_id: shipment.id, cost: 10
    end
  ...
  context '#destroy' do
    it 'destroys linked shipping_rates' do
      reflection = Viauco::Shipment.reflect_on_association(:shipping_rates)
