...
describe 'StoreCreditEvent' do
  describe '#display_amount' do
    ...
    it 'returns a Viauco::Money instance' do
      expect(subject.display_amount).to be_instance_of(Viauco::Money)
    end

    it 'uses the events amount attribute' do
      expect(subject.display_amount).to eq Viauco::Money.new(event_amount, currency: subject.currency)
    end
  end
  ...
  describe '#display_user_total_amount' do
    ...
    it 'returns a Viauco::Money instance' do
      expect(subject.display_user_total_amount).to be_instance_of(Viauco::Money)
    end

    it 'uses the events user_total_amount attribute' do
      amount = Viauco::Money.new(user_total_amount, currency: subject.currency)
  ...
  describe '#display_action' do
    ...
    context 'capture event' do
      let(:action) { Viauco::StoreCredit::CAPTURE_ACTION }

      it 'returns used' do
        expect(subject.display_action).to eq Viauco.t('store_credit.captured')
      end
    end

    context 'authorize event' do
      let(:action) { Viauco::StoreCredit::AUTHORIZE_ACTION }

      it 'returns authorized' do
        expect(subject.display_action).to eq Viauco.t('store_credit.authorized')
      end
    end

    context 'allocation event' do
      let(:action) { Viauco::StoreCredit::ALLOCATION_ACTION }

      it 'returns added' do
        expect(subject.display_action).to eq Viauco.t('store_credit.allocated')
      end
    end

    context 'void event' do
      let(:action) { Viauco::StoreCredit::VOID_ACTION }

      it 'returns credit' do
        expect(subject.display_action).to eq Viauco.t('store_credit.credit')
      end
    end

    context 'credit event' do
      let(:action) { Viauco::StoreCredit::CREDIT_ACTION }
  ...
  describe '#order' do
    ...
    context 'there is an associated payment with the event' do
      subject do
        create(:store_credit_auth_event, action: Viauco::StoreCredit::CAPTURE_ACTION,
