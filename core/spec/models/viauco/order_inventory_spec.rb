...
describe Viauco::OrderInventory, type: :model do
  ...
  context '#add_to_shipment' do
    ...
    context 'store doesnt track inventory' do
      ...
      before { Viauco::Config.track_inventory_levels = false }

      it 'creates only on hand inventory units' do
        variant.stock_items.destroy_all

        # The before_save callback in LineItem would verify inventory
        line_item = Viauco::Cart::AddItem.call(order: order, variant: variant, options: { shipment: shipment }).value
    ...
    context 'variant doesnt track inventory' do
      ...
      it 'creates only on hand inventory units' do
        ...
        line_item = Viauco::Cart::AddItem.call(order: order, variant: variant).value
  ...
  context 'when order has too many inventory units' do
    ...
    context '#remove_from_shipment' do
      ...
      it 'destroys backordered units first' do
        allow(shipment).to receive_messages(
          inventory_units_for_item: [
            mock_model(
              Viauco::InventoryUnit, variant_id: variant.id, quantity: 2, state: 'backordered', shipped?: false, backordered?: true
            ),
            mock_model(
              Viauco::InventoryUnit, variant_id: variant.id, quantity: 1, state: 'on_hand', shipped?: false, backordered?: false
            )
          ]
        )
      ...
      it 'destroys unshipped units first' do
        allow(shipment).to receive_messages(
          inventory_units_for_item: [
            mock_model(Viauco::InventoryUnit, variant_id: variant.id, quantity: 1, state: 'shipped', shipped?: true, backordered?: false),
            mock_model(Viauco::InventoryUnit, variant_id: variant.id, quantity: 1, state: 'on_hand', shipped?: false, backordered?: false)
          ]
        )
