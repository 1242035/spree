...
describe Viauco::Preferences::Store, type: :model do
  before do
    @store = Viauco::Preferences::StoreInstance.new
  end
  ...
  it 'will return db value when cache is emtpy and cache the db value' do
    preference = Viauco::Preference.where(key: 'test').first_or_initialize
