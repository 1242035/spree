...
describe Viauco::ReturnItem::EligibilityValidator::InventoryShipped do
  ...
  describe '#eligible_for_return?' do
    ...
    context 'the associated inventory unit is not shipped' do
      ...
      it 'sets an error' do
        subject
        expect(validator.errors[:inventory_unit_shipped]).to eq Viauco.t('return_item_inventory_unit_ineligible')
      end
    end
  end
