...
describe Viauco::ReturnItem::EligibilityValidator::OrderCompleted do
  ...
  describe '#eligible_for_return?' do
    ...
    context 'the order is not completed' do
      ...
      it 'sets an error' do
        subject
        expect(validator.errors[:order_not_completed]).to eq Viauco.t('return_item_order_not_completed')
      end
    end
  end
end
