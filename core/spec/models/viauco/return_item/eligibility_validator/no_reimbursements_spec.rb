...
describe Viauco::ReturnItem::EligibilityValidator::NoReimbursements do
  ...
  describe '#eligible_for_return?' do
    ...
    context 'inventory unit has already been reimbursed' do
      ...
      it 'sets an error' do
        subject
        expect(validator.errors[:inventory_unit_reimbursed]).to eq Viauco.t('return_item_inventory_unit_reimbursed')
      end
    end
