...
describe 'Product scopes', type: :model do
  ...
  describe '#available' do
    context 'when discontinued' do
      ...
      it { expect(Viauco::Product.available).not_to include(discontinued_product) }
    end

    context 'when not discontinued' do
      let!(:product_2) { create(:product, discontinue_on: Time.current + 1.day) }

      it { expect(Viauco::Product.available).to include(product_2) }
    end
    ...
    context 'when not available' do
      ...
      it { expect(Viauco::Product.available).not_to include(unavailable_product) }
    end
  end
  ...
  context 'A product assigned to parent and child taxons' do
    ...
    it 'calling Product.in_taxon returns products in child taxons' do
      ...
      expect(Viauco::Product.in_taxon(@parent_taxon)).to include(product)
    end

    it 'calling Product.in_taxon should not return duplicate records' do
      expect(Viauco::Product.in_taxon(@parent_taxon).to_a.size).to eq(1)
    end
    ...
    context 'orders products based on their ordering within the classifications' do
      ...
      it 'by initial ordering' do
        expect(Viauco::Product.in_taxon(@child_taxon)).to eq([product, product_2])
        expect(Viauco::Product.in_taxon(other_taxon)).to eq([product, product_2])
      end

      it 'after ordering changed' do
        [@child_taxon, other_taxon].each do |taxon|
          Viauco::Classification.find_by(taxon: taxon, product: product).insert_at(2)
          expect(Viauco::Product.in_taxon(taxon)).to eq([product_2, product])
        end
      end
    end
  end
  ...
  context 'property scopes' do
    ...
    context 'with_property' do
      let(:with_property) { Viauco::Product.method(:with_property) }
    ...
    context 'with_property_value' do
      let(:with_property_value) { Viauco::Product.method(:with_property_value) }
  ...
  context '#add_simple_scopes' do
    ...
    before do
      Viauco::Product.add_simple_scopes(simple_scopes)
    end

    context 'define scope' do
      context 'ascend_by_updated_at' do
        context 'on class' do
          it { expect(Viauco::Product.ascend_by_updated_at.to_sql).to eq Viauco::Product.order(Arel.sql("#{Viauco::Product.quoted_table_name}.updated_at ASC")).to_sql }
        end

        context 'on ActiveRecord::Relation' do
          it { expect(Viauco::Product.limit(2).ascend_by_updated_at.to_sql).to eq Viauco::Product.limit(2).order(Arel.sql("#{Viauco::Product.quoted_table_name}.updated_at ASC")).to_sql }
          it { expect(Viauco::Product.limit(2).ascend_by_updated_at.to_sql).to eq Viauco::Product.ascend_by_updated_at.limit(2).to_sql }
        end
      end

      context 'descend_by_name' do
        context 'on class' do
          it { expect(Viauco::Product.descend_by_name.to_sql).to eq Viauco::Product.order(Arel.sql("#{Viauco::Product.quoted_table_name}.name DESC")).to_sql }
        end

        context 'on ActiveRecord::Relation' do
          it { expect(Viauco::Product.limit(2).descend_by_name.to_sql).to eq Viauco::Product.limit(2).order(Arel.sql("#{Viauco::Product.quoted_table_name}.name DESC")).to_sql }
          it { expect(Viauco::Product.limit(2).descend_by_name.to_sql).to eq Viauco::Product.descend_by_name.limit(2).to_sql }
        end
      end
    end
  end
