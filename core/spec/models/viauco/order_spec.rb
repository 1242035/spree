...
class FakeCalculator < Viauco::Calculator
...
describe Viauco::Order, type: :model do
  let(:user) { stub_model(Viauco::LegacyUser, email: 'viauco@example.com') }
  let(:order) { stub_model(Viauco::Order, user: user) }

  before do
    create(:store)
    allow(Viauco::LegacyUser).to receive_messages(current: mock_model(Viauco::LegacyUser, id: 123))
  end
  ...
  describe '.scopes' do
    ...
    describe '.complete' do
      it { expect(Viauco::Order.complete).to include completed_order }
      it { expect(Viauco::Order.complete).not_to include incompleted_order }
    end

    describe '.incomplete' do
      it { expect(Viauco::Order.incomplete).to include incompleted_order }
      it { expect(Viauco::Order.incomplete).not_to include completed_order }
    end
  end

  describe '#update_with_updater!' do
    let(:updater) { Viauco::OrderUpdater.new(order) }
  ...
  context '#cancel' do
    ...
    it 'marks the payments as void' do
      allow_any_instance_of(Viauco::Shipment).to receive(:refresh_rates).and_return(true)
      order.cancel
      order.reload
  ...
  context '#finalize!' do
    let(:order) { Viauco::Order.create(email: 'test@example.com') }

    before do
      order.update_column :state, 'complete'
    end

    after { Viauco::Config.set track_inventory_levels: true }
    ...
    it 'changes the shipment state to ready if order is paid' do
      Viauco::Shipment.create(order: order, stock_location: create(:stock_location))
    ...
    it 'does not sell inventory units if track_inventory_levels is false' do
      Viauco::Config.set track_inventory_levels: false
      expect(Viauco::InventoryUnit).not_to receive(:sell_units)
      order.finalize!
    end

    it 'sends an order confirmation email' do
      mail_message = double 'Mail::Message'
      expect(Viauco::OrderMailer).to receive(:confirm_email).with(order.id).and_return mail_message
    ...
    it 'does not send duplicate confirmation emails' do
      ...
      expect(Viauco::OrderMailer).not_to receive(:confirm_email)
      order.finalize!
    end

    it 'freezes all adjustments' do
      allow(Viauco::OrderMailer).to receive_message_chain :confirm_email, :deliver_later
      adjustments = [double]
  ...
  context 'insufficient_stock_lines' do
    let(:line_item) { mock_model Viauco::LineItem, insufficient_stock?: true }
  ...
  describe '#ensure_line_item_variants_are_not_discontinued' do
    ...
    context 'when variant is destroyed' do
      ...
      it 'has error message' do
        subject
        expect(order.errors[:base]).to include(Viauco.t(:discontinued_variants_present))
      end
  ...
  describe '#ensure_line_items_are_in_stock' do
    ...
    it 'has error message' do
      subject
      expect(order.errors[:base]).to include(Viauco.t(:insufficient_stock_lines_present))
    end
  ...
  context 'empty!' do
    ...
    context 'completed order' do
      ...
      it 'raises an exception' do
        expect { order.empty! }.to raise_error(RuntimeError, Viauco.t(:cannot_empty_completed_order))
      end
    end
  ...
  context '#display_outstanding_balance' do
    it 'returns the value as a viauco money' do
      allow(order).to receive(:outstanding_balance).and_return(10.55)
      expect(order.display_outstanding_balance).to eq(Viauco::Money.new(10.55))
    end
  end
  ...
  context '#display_item_total' do
    it 'returns the value as a viauco money' do
      ...
      expect(order.display_item_total).to eq(Viauco::Money.new(10.55))
    end
  end
  ...
  context '#display_adjustment_total' do
    it 'returns the value as a viauco money' do
      ...
      expect(order.display_adjustment_total).to eq(Viauco::Money.new(10.55))
    end
  end
  ...
  context '#display_promo_total' do
    it 'returns the value as a viauco money' do
      ...
      expect(order.display_promo_total).to eq(Viauco::Money.new(10.55))
    end
  end
  ...
  context '#display_total' do
    it 'returns the value as a viauco money' do
      order.total = 10.55
      expect(order.display_total).to eq(Viauco::Money.new(10.55))
    end
  end
  ...
  context '#confirmation_required?' do
    ...
    context 'Viauco::Config[:always_include_confirm_step] == true' do
      before do
        Viauco::Config[:always_include_confirm_step] = true
      end
    ...
    context 'Viauco::Config[:always_include_confirm_step] == false' do
      it 'returns false if payments empty and Viauco::Config[:always_include_confirm_step] == false' do
        order = Viauco::Order.new
        assert !order.confirmation_required?
      end
  ...
  context 'add_update_hook' do
    ...
    after do
      Viauco::Order.update_hooks = Set.new
    end
  ...
  describe '#tax_address' do
    ...
    before { Viauco::Config[:tax_using_ship_address] = tax_using_ship_address }
  ...
  context '#available_payment_methods' do
    ...
    it 'includes frontend payment methods' do
      payment_method = Viauco::PaymentMethod.create!(name: 'Fake',
                                                    active: true,
    ...
    it 'does not include a payment method that is not suitable for this order' do
      allow(Viauco::PaymentMethod).to receive(:available_on_front_end).and_return(methods)
  ...
  context '#apply_free_shipping_promotions' do
    it 'calls out to the FreeShipping promotion handler' do
      ...
      expect(Viauco::PromotionHandler::FreeShipping).to receive(:new).and_return(handler = double)
      expect(handler).to receive(:activate)

      expect(Viauco::Adjustable::AdjustmentsUpdater).to receive(:update).with(shipment)

      expect(Viauco::TaxRate).to receive(:adjust).with(order, [shipment])
  ...
  context '#products' do
    before do
      @variant1 = mock_model(Viauco::Variant, product: 'product1')
      @variant2 = mock_model(Viauco::Variant, product: 'product2')
      @line_items = [mock_model(Viauco::LineItem, product: 'product1', variant: @variant1, variant_id: @variant1.id, quantity: 1),
                     mock_model(Viauco::LineItem, product: 'product2', variant: @variant2, variant_id: @variant2.id, quantity: 2)]
    ...
    it 'gets the quantity of a given variant' do
      ...
      @variant3 = mock_model(Viauco::Variant, product: 'product3')
      expect(order.quantity_of(@variant3)).to eq(0)
    end

    it 'can find a line item matching a given variant' do
      expect(order.find_line_item_by_variant(@variant1)).not_to be_nil
      expect(order.find_line_item_by_variant(mock_model(Viauco::Variant))).to be_nil
    end

    context 'match line item with options' do
      before do
        Rails.application.config.viauco.line_item_comparison_hooks << :foos_match
      end

      after do
        # reset to avoid test pollution
        Rails.application.config.viauco.line_item_comparison_hooks = Set.new
      end

      it 'matches line item when options match' do
        allow(order).to receive(:foos_match).and_return(true)
        expect(Viauco::Dependencies.cart_compare_line_items_service.constantize.new.call(order: order, line_item: @line_items.first, options: { foos: { bar: :zoo } }).value).to be true

      end

      it 'does not match line item without options' do
        allow(order).to receive(:foos_match).and_return(false)
        expect(Viauco::Dependencies.cart_compare_line_items_service.constantize.new.call(order: order, line_item: @line_items.first, options: {}).value).to be false
      end
    end
  end
  ...
  context '#backordered?' do
    it 'is backordered if one of the shipments is backordered' do
      allow(order).to receive_messages(shipments: [mock_model(Viauco::Shipment, backordered?: false),
                                                   mock_model(Viauco::Shipment, backordered?: true)])
  ...
  context 'locking' do
    let(:order) { Viauco::Order.create } # need a persisted in order to test locking
  ...
  describe '#pre_tax_item_amount' do
    it "sums all of the line items' pre tax amounts" do
      subject.line_items = [
        Viauco::LineItem.new(price: 10, quantity: 2, pre_tax_amount: 5.0),
        Viauco::LineItem.new(price: 30, quantity: 1, pre_tax_amount: 14.0)
      ]
  ...
  describe '#create_proposed_shipments' do
    ...
    it 'assigns the coordinator returned shipments to its shipments' do
      ...
      allow_any_instance_of(Viauco::Stock::Coordinator).to receive(:shipments).and_return([shipment])
      subject.create_proposed_shipments
  ...
  describe '#fully_discounted?' do
    let(:line_item) { Viauco::LineItem.new(price: 10, quantity: 1) }
    let(:shipment) { Viauco::Shipment.new(cost: 10) }
    let(:payment) { Viauco::Payment.new(amount: 10) }
  ...
  describe '#create_shipment_tax_charge!' do
    ...
    context 'when order has shipments' do
      before do
        ...
        allow(Viauco::TaxRate).to receive(:adjust).with(order, order_shipments)
      end

      it { expect(order_shipments).to receive(:any?).and_return(true) }
      it { expect(order).to receive(:shipments).and_return(order_shipments) }
      it { expect(Viauco::TaxRate).to receive(:adjust).with(order, order_shipments) }
    end
  ...
  describe '#shipping_eq_billing_address' do
    ...
    context 'blank addresses' do
      before do
        order.bill_address = Viauco::Address.new
        order.ship_address = Viauco::Address.new
      end
  ...
  describe '#destroying order will trigger ship and bill addresses destroy' do
    ...
    it { expect { order.destroy }.to change { Viauco::Address.count }.by(-2) }
  end
end
