...
module Viauco
  describe Viauco::ProductDuplicator, type: :model do
    ...
    before do
      new_image = Viauco::Image.new(params)
      new_image.attachment.attach(io: file, filename: File.basename(file))
      new_image.save!
    end

    it 'will duplicate the product' do
      expect { duplicator.duplicate }.to change { Viauco::Product.count }.by(1)
    end

    it 'will duplicate already duplicated product' do
      Timecop.scale(3600)
      expect { 3.times { duplicator.duplicate } }.to change { Viauco::Product.count }.by(3)
      Timecop.return
    end

    context 'when image duplication enabled' do
      it 'will duplicate the product images' do
        expect { duplicator.duplicate }.to change { Viauco::Image.count }.by(1)
      end
    end

    context 'when image duplication disabled' do
      let!(:duplicator) { Viauco::ProductDuplicator.new(product, false) }

      it 'will not duplicate the product images' do
        expect { duplicator.duplicate }.to change { Viauco::Image.count }.by(0)
      end
    end
    ...
    context 'image duplication default' do
      ...
      context 'when default is set to false' do
        ...
        it 'does not clone images if no flag passed to initializer' do
          expect { ProductDuplicator.new(product).duplicate }.to change { Viauco::Image.count }.by(0)
        end
      end
    end
    ...
    context 'with variants' do
      ...
      it 'will duplciate the variants' do
        ...
        expect { duplicator.duplicate }.to change { Viauco::Variant.count }.by(3)
      end

      it 'will not duplicate the option values' do
        expect { duplicator.duplicate }.to change { Viauco::OptionValue.count }.by(0)
      end
    end
  end
end
