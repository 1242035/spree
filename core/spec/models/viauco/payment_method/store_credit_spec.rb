...
describe Viauco::PaymentMethod::StoreCredit do
  ...
  context '#authorize' do
    ...
    context 'without an invalid store credit' do
      ...
      it 'declines an unknown store credit' do
        ...
        expect(subject.message).to include Viauco.t('store_credit_payment_method.unable_to_find')
      end
    end
    ...
    context 'with insuffient funds' do
      ...
      it 'declines a store credit' do
        expect(subject.success?).to be false
        expect(subject.message).to include Viauco.t('store_credit_payment_method.insufficient_funds')
      end
    end
    ...
    context 'when the currency does not match the order currency' do
      ...
      it 'declines the credit' do
        expect(subject.success?).to be false
        expect(subject.message).to include Viauco.t('store_credit_payment_method.currency_mismatch')
      end
    end
    ...
    context 'with a valid request' do
      ...
      context 'with an originator' do
        ...
        it 'passes the originator' do
          expect_any_instance_of(Viauco::StoreCredit).to receive(:authorize).
  ...
  context '#capture' do
    ...
    context 'when unable to authorize the amount' do
      ...
      before do
        allow_any_instance_of(Viauco::StoreCredit).to receive_messages(authorize: true)
      end

      it 'declines a store credit' do
        expect(subject.success?).to be false
        expect(subject.message).to include Viauco.t('store_credit_payment_method.insufficient_authorized_amount')
      end
    end
    ...
    context 'with a valid request' do
      it 'captures the store credit' do
        expect(subject.message).to include Viauco.t('store_credit_payment_method.successful_action',
      ...
      context 'with an originator' do
        ...
        it 'passes the originator' do
          expect_any_instance_of(Viauco::StoreCredit).to receive(:capture).
  ...
  context '#void' do
    ...
    context 'when the store credit is not voided successfully' do
      before { allow_any_instance_of(Viauco::StoreCredit).to receive(:void).and_return false }
    ...
    it 'voids a valid store credit void request' do
      expect(subject.success?).to be true
      expect(subject.message).to include Viauco.t('store_credit_payment_method.successful_action',
                                                 action: Viauco::StoreCredit::VOID_ACTION)
    end
    ...
    context 'with an originator' do
      ...
      it 'passes the originator' do
        expect_any_instance_of(Viauco::StoreCredit).to receive(:void).with(anything, action_originator: originator)
        subject
      end
    end
  end
  ...
  context '#purchase' do
    it "declines a purchase if it can't find a pending credit for the correct amount" do
      ...
      store_credit.store_credit_events.create!(action: Viauco::StoreCredit::ELIGIBLE_ACTION,
                                               amount: amount,
                                               authorization_code: auth_code)
      store_credit.store_credit_events.create!(action: Viauco::StoreCredit::CAPTURE_ACTION,
                                               amount: amount, authorization_code: auth_code)

      response = subject.purchase(amount * 100.0, store_credit, gateway_options)
      expect(response.success?).to be false
      expect(response.message).to include Viauco.t('store_credit_payment_method.unable_to_find')
    end
    ...
    it 'captures a purchase if it can find a pending credit for the correct amount' do
      ...
      expect(response.message).to include Viauco.t('store_credit_payment_method.successful_action',
                                                  action: Viauco::StoreCredit::CAPTURE_ACTION)
    end
  end
  ...
  context '#credit' do
    ...
    context "when the store credit isn't credited successfully" do
      before { allow(Viauco::StoreCredit).to receive_messages(credit: false) }
    ...
    context 'with a valid credit request' do
      before { allow_any_instance_of(Viauco::StoreCredit).to receive_messages(credit: true) }

      it 'credits a valid store credit credit request' do
        expect(subject.success?).to be true
        expect(subject.message).to include Viauco.t('store_credit_payment_method.successful_action',
                                                   action: Viauco::StoreCredit::CREDIT_ACTION)
      end
    end
    ...
    context 'with an originator' do
      ...
      it 'passes the originator' do
        allow_any_instance_of(Viauco::StoreCredit).to receive(:credit).
          with(anything, anything, anything, action_originator: originator)
        subject
      end
    end
  ...
  context '#cancel' do
    ...
    context 'store credit event found' do
      ...
      it 'returns a valid store credit cancel request' do
        expect(subject.success?).to be true
        expect(subject.message).to include Viauco.t('store_credit_payment_method.successful_action',
                                                   action: Viauco::StoreCredit::CANCEL_ACTION)
      end
    end
