...
describe Viauco::Calculator::FlexiRate, type: :model do
  let(:calculator) { Viauco::Calculator::FlexiRate.new }

  let(:order) do
    mock_model(
      Viauco::Order, quantity: 10
    )
  end
  ...
  context 'compute' do
    ...
    it 'allows creation of new object with all the attributes' do
      Viauco::Calculator::FlexiRate.new(preferred_first_item: 1, preferred_additional_item: 1, preferred_max_items: 1)
    end
  end
end
