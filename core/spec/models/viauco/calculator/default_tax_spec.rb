...
describe Viauco::Calculator::DefaultTax, type: :model do
  ...
  let!(:calculator) { Viauco::Calculator::DefaultTax.new(calculable: rate) }
      ...
      context 'when the variant matches the tax category' do
        context 'when line item is discounted' do
          before do
            ...
            Viauco::TaxRate.store_pre_tax_amount(line_item, [rate])
          end
  ...
  context 'when given a line_item' do
    subject do
      Viauco::Calculator::DefaultTax.new(calculable: rate).
