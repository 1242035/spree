...
describe Viauco::Variant, type: :model do
  ...
  context 'after create' do
    ...
    it 'propagate to stock items' do
      expect_any_instance_of(Viauco::StockLocation).to receive(:propagate_variant)
      create(:variant, product: product)
    end

    context 'stock location has disable propagate all variants' do
      before { Viauco::StockLocation.update_all propagate_all_variants: false }

      it 'propagate to stock items' do
        expect_any_instance_of(Viauco::StockLocation).not_to receive(:propagate_variant)
  ...
  describe 'scope' do
    describe '.eligible' do
      context 'when only master variants' do
        ...
        it 'returns all of them' do
          expect(Viauco::Variant.eligible).to include(product_1.master)
          expect(Viauco::Variant.eligible).to include(product_2.master)
        end
      end
      ...
      context 'when product has more than 1 variant' do
        ...
        it 'filters master variant out' do
          expect(Viauco::Variant.eligible).to include(variant)
          expect(Viauco::Variant.eligible).not_to include(product.master)
        end
      end
    end
    ...
    describe '.not_discontinued' do
      context 'when discontinued' do
        ...
        it { expect(Viauco::Variant.not_discontinued).not_to include(discontinued_variant) }
      end

      context 'when not discontinued' do
        let!(:variant_2) { create(:variant, discontinue_on: Time.current + 1.day) }

        it { expect(Viauco::Variant.not_discontinued).to include(variant_2) }
      end
    ...
    describe '.not_deleted' do
      context 'when deleted' do
        ...
        it { expect(Viauco::Variant.not_deleted).not_to include(deleted_variant) }
      end

      context 'when not deleted' do
        let!(:variant_2) { create(:variant, deleted_at: nil) }

        it { expect(Viauco::Variant.not_deleted).to include(variant_2) }
      end
    end
    ...
    describe '.for_currency_and_available_price_amount' do
      ...
      context 'when price with currency present' do
        context 'when price has amount' do
          ...
          it { expect(Viauco::Variant.for_currency_and_available_price_amount(currency)).to include(variant) }
        end

        context 'when price do not have amount' do
          let!(:price_1) { create(:price, currency: currency, variant: variant, amount: nil) }

          it { expect(Viauco::Variant.for_currency_and_available_price_amount(currency)).not_to include(variant) }
        end
      end
      ...
      context 'when multiple prices for same currency present' do
        ...
        it 'does not duplicate variant' do
          expect(Viauco::Variant.for_currency_and_available_price_amount(currency)).to eq([variant])
        end
      end
      ...
      context 'when currency parameter is nil' do
        ...
        before { Viauco::Config[:currency] = currency }

        it { expect(Viauco::Variant.for_currency_and_available_price_amount).to include(variant) }
      end
    end
    ...
    describe '.active' do
      ...
      before do
        allow(Viauco::Variant).to receive(:not_discontinued).and_return(variants)
      ...
      it 'finds not_discontinued variants' do
        expect(Viauco::Variant).to receive(:not_discontinued).and_return(variants)
        Viauco::Variant.active(currency)
      end
      ...
      it { expect(Viauco::Variant.active(currency)).to eq(variants) }
    end
  end
  ...
  context '#cost_price=' do
    it 'uses LocalizedNumber.parse' do
      expect(Viauco::LocalizedNumber).to receive(:parse).with('1,599.99')
  ...
  describe '#in_stock?' do
    before do
      Viauco::Config.track_inventory_levels = true
    end

    context 'when stock_items are not backorderable' do
      before do
        allow_any_instance_of(Viauco::StockItem).to receive_messages(backorderable: false)
      end
      ...
      context 'when stock_items out of stock' do
        before do
          allow_any_instance_of(Viauco::StockItem).to receive_messages(backorderable: false)
          allow_any_instance_of(Viauco::StockItem).to receive_messages(count_on_hand: 0)
        end
    ...
    describe '#can_supply?' do
      it 'calls out to quantifier' do
        expect(Viauco::Stock::Quantifier).to receive(:new).and_return(quantifier = double)
    ...
    context 'when stock_items are backorderable' do
      before do
        allow_any_instance_of(Viauco::StockItem).to receive_messages(backorderable: true)
      end
  ...
  describe '#is_backorderable' do
    ...
    it 'invokes Viauco::Stock::Quantifier' do
      expect_any_instance_of(Viauco::Stock::Quantifier).to receive(:backorderable?).and_return(true)
      subject
    end
  end
  ...
  describe '#total_on_hand' do
    ...
    it 'matches quantifier total_on_hand' do
      ...
      expect(variant.total_on_hand).to eq(Viauco::Stock::Quantifier.new(variant).total_on_hand)
    end
  end
  ...
  describe 'in_stock scope' do
    it 'returns all in stock variants' do
      ...
      expect(Viauco::Variant.in_stock).to eq [in_stock_variant]
    end
  end
  ...
  describe '#check_price' do
    ...
    context 'require_master_price set false' do
      before { Viauco::Config.set(require_master_price: false) }
      ...
      context 'price present and currency nil' do
        ...
        it { expect(variant.send(:check_price)).to be(Viauco::Config[:currency]) }
      end
    ...
    context 'require_master_price set true' do
      before { Viauco::Config.set(require_master_price: true) }
      ...
      context 'product and master_variant present and equal' do
        ...
        context 'price nil and currency nil' do
          ...
          it { expect(variant.price).to eq(variant.product.master.price) }
          it { expect(variant.currency).to eq(Viauco::Config[:currency]) }
        end
      end
        ...
        context 'product not present' do
          ...
          context 'price nil and currency present' do
            ...
            it 'adds absence of master error' do
              variant.send(:check_price)
              expect(variant.errors[:base]).to include I18n.t('activerecord.errors.models.viauco/variant.attributes.base.no_master_variant_found_to_infer_price')
            end
          end
  ...
  describe '#ensure_no_line_items' do
    ...
    it 'adds error on product destroy' do
      ...
      expect(variant.errors[:base]).to include I18n.t('activerecord.errors.models.viauco/variant.attributes.base.cannot_destroy_if_attached_to_line_items')
    end
  end
end
