...
describe Viauco::LegacyUser, type: :model do # rubocop:disable RSpec/MultipleDescribes
  ...
  context '#last_incomplete_order' do
    ...
    it 'returns correct order' do
      Timecop.scale(3600) do
        ...
        expect(user.last_incomplete_viauco_order(current_store)).to eq order_3
      end
    end
    ...
    context 'persists order address' do
      it 'copies over order addresses' do
        ...
        end.to change { Viauco::Address.count }.by(2)
      ...
      it 'doesnt create new addresses if user has already' do
        ...
        expect do
          user.persist_order_address(order)
        end.not_to change { Viauco::Address.count }
      end
...
describe Viauco.user_class, type: :model do
  context 'reporting' do
    ...
    describe '#display_lifetime_value' do
      it 'returns a Viauco::Money version of lifetime_value' do
        value = BigDecimal('500.05')
        allow(subject).to receive(:lifetime_value).and_return(value)
        expect(subject.display_lifetime_value).to eq Viauco::Money.new(value)
      end
    end
    ...
    describe '#display_average_order_value' do
      ...
      it 'returns a Viauco::Money version of average_order_value' do
        value = BigDecimal('500.05')
        allow(subject).to receive(:average_order_value).and_return(value)
        expect(subject.display_average_order_value).to eq Viauco::Money.new(value)
      end
    end
  end
