  ...
  describe OrderUpdater, type: :model do
    ...
    context 'order totals' do
      ...
      it 'updates payment totals' do
        ...
        Viauco::OrderUpdater.new(order).update_payment_total
      ...
      context 'with order promotion followed by line item addition' do
        let(:promotion) { Viauco::Promotion.create!(name: '10% off') }
    ...
    context 'completed order' do
      ...
      describe '#update_shipments' do
        it 'updates each shipment' do
          shipment = stub_model(Viauco::Shipment, order: order)
          shipments = [shipment]
    ...
    context 'incomplete order' do
      ...
      describe '#update_shipments' do
        ...
        it 'resets shipping method to frontend-available' do
          ...
          expect(order.shipments.first.shipping_method).to eq Viauco::ShippingMethod.find_by(display_on: 'both')
        end
      end
    end
  end
