...
describe Viauco::Price, type: :model do
  describe '#amount=' do
    let(:price) { Viauco::Price.new }
    let(:amount) { '3,0A0' }
    ...
    it 'is expected to equal to localized number' do
      expect(price.amount).to eq(Viauco::LocalizedNumber.parse(amount))
    end
  end
  ...
  describe 'validations' do
    subject { Viauco::Price.new variant: variant, amount: amount }

    let(:variant) { stub_model Viauco::Variant }
    ...
    context 'when the amount is greater than maximum amount' do
      let(:amount) { Viauco::Price::MAXIMUM_AMOUNT + 1 }
      before { subject.valid? }
      ...
      it 'populates errors' do
        expect(subject.errors.messages[:amount].first).to eq "must be less than or equal to #{Viauco::Price::MAXIMUM_AMOUNT}"
      end
    end

    context 'when the amount is between 0 and the maximum amount' do
      let(:amount) { Viauco::Price::MAXIMUM_AMOUNT }
  ...
  describe '#price_including_vat_for(zone)' do
    ...
    let(:variant) { stub_model Viauco::Variant }
    let(:default_zone) { Viauco::Zone.new }
    let(:zone) { Viauco::Zone.new }
    let(:amount) { 10 }
    let(:tax_category) { Viauco::TaxCategory.new }
    let(:price) { Viauco::Price.new variant: variant, amount: amount }
  ...
  describe '#display_price_including_vat_for(zone)' do
    subject { Viauco::Price.new amount: 10 }
