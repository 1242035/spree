...
describe Viauco::CustomerReturn, type: :model do
  before do
  ...
  describe '.validation' do
    describe '#must_have_return_authorization' do
      ...
      context 'return item does not belong to return authorization' do
        ...
        it 'adds an error message' do
          subject
          expect(customer_return.errors.full_messages).to include(Viauco.t(:missing_return_authorization, item_name: inventory_unit.variant.name))
        end
      end
    ...
    describe '#return_items_belong_to_same_order' do
      ...
      context 'return items are part of different orders' do
        ...
        it 'adds an error message' do
          subject
          expect(customer_return.errors.full_messages).to include(Viauco.t(:return_items_cannot_be_associated_with_multiple_orders))
        end
      end
  ...
  describe 'whitelisted_ransackable_attributes' do
    it { expect(Viauco::CustomerReturn.whitelisted_ransackable_attributes).to eq(%w(number)) }
  end
  ...
  describe '#pre_tax_total' do
    ...
    before do
      Viauco::ReturnItem.where(customer_return_id: customer_return.id).update_all(pre_tax_amount: pre_tax_amount)
    end
  ...
  describe '#display_pre_tax_total' do
    let(:customer_return) { Viauco::CustomerReturn.new }

    it 'returns a Viauco::Money' do
      allow(customer_return).to receive_messages(pre_tax_total: 21.22)
      expect(customer_return.display_pre_tax_total).to eq(Viauco::Money.new(21.22))
    end
  end
    ...
    context 'to the initial stock location' do
      ...
      context 'with Config.track_inventory_levels == false' do
        before do
          ...
          expect(Viauco::StockItem).not_to receive(:find_by)
          expect(Viauco::StockMovement).not_to receive(:create!)
        end
    ...
    context 'to a different stock location' do
      ...
      it 'updates the stock item counts in new stock location' do
        expect do
          ...
          Viauco::StockItem.where(variant_id: inventory_unit.variant_id, stock_location_id: new_stock_location.id).first.count_on_hand
        }.by(1)
      end
