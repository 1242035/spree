...
describe Viauco::ShippingRate, type: :model do
  let(:shipment) { create(:shipment) }
  let(:shipping_method) { create(:shipping_method) }
  let(:shipping_rate) do
    Viauco::ShippingRate.new shipment: shipment,
  ...
  context '#display_price' do
    ...
    context 'when the currency is JPY' do
      let(:shipping_rate) { Viauco::ShippingRate.new(cost: 205) }
  ...
  context '#final_price' do
    ...
    it 'returns 0 if cost is lesser than the discount amount' do
      allow_any_instance_of(Viauco::ShippingRate).to receive_messages(discount_amount: -20.0)
      expect(shipping_rate.final_price).to eq(0.0)
    end

    it 'returns cost minus discount amount' do
      allow_any_instance_of(Viauco::ShippingRate).to receive_messages(discount_amount: -5.0)
