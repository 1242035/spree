...
describe Viauco::ReturnAuthorization, type: :model do
  ...
  let(:return_authorization) do
    Viauco::ReturnAuthorization.new(order: order,
  ...
  context 'save' do
    ...
    context 'expedited exchanges are configured' do
      ...
      before do
        @expediteted_exchanges_config = Viauco::Config[:expedited_exchanges]
        Viauco::Config[:expedited_exchanges] = true
        @pre_exchange_hooks = subject.class.pre_expedited_exchange_hooks
      end

      after do
        Viauco::Config[:expedited_exchanges] = @expediteted_exchanges_config
      ...
      context 'no items to exchange' do
        ...
        it 'does not create a reimbursement' do
          expect { subject.save }.not_to change { Viauco::Reimbursement.count }
        end
      end
      ...
      context 'items to exchange' do
        ...
        it 'performs an exchange reimbursement for the exchange return items' do
          subject.save
          reimbursement = Viauco::Reimbursement.last
        ...
        context 'the reimbursement fails' do
          before do
            allow_any_instance_of(Viauco::Reimbursement).to receive(:save).and_return(false)
            allow_any_instance_of(Viauco::Reimbursement).to receive(:errors) { double(full_messages: 'foo') }
          end
  ...
  describe 'whitelisted_ransackable_attributes' do
    it { expect(Viauco::ReturnAuthorization.whitelisted_ransackable_attributes).to eq(%w(memo number state)) }
  end
  ...
  describe '#display_pre_tax_total' do
    it 'returns a Viauco::Money' do
      ...
      expect(return_authorization.display_pre_tax_total).to eq(Viauco::Money.new(21.22))
    end
  end
