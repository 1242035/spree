...
describe Viauco::Address, type: :model do
  describe 'clone' do
  ...
  context 'aliased attributes' do
    let(:address) { Viauco::Address.new }
  ...
  context 'validation' do
    let(:country) { stub_model(Viauco::Country, states: [state], states_required: true) }
    let(:state) { stub_model(Viauco::State, name: 'maryland', abbr: 'md') }
    let(:address) { build(:address, country: country) }

    before do
      allow(Viauco::State).to receive(:find_all_by_name_or_abbr) { [state] }
    ...
    it 'state is entered but country does not contain that state' do
      address.state = state
      address.country = stub_model(Viauco::Country, states_required: true)
      address.valid?
    ...
    it 'address_requires_state preference is false' do
      Viauco::Config.set address_requires_state: false
      address.state = nil
      address.state_name = nil
      expect(address).to be_valid
    end
  ...
  context '.default' do
    context 'no user given' do
      before do
        @default_country_id = Viauco::Config[:default_country_id]
        new_country = create(:country)
        Viauco::Config[:default_country_id] = new_country.id
      end

      after do
        Viauco::Config[:default_country_id] = @default_country_id
      end

      it 'sets up a new record with Viauco::Config[:default_country_id]' do
        expect(Viauco::Address.default.country).to eq(Viauco::Country.find(Viauco::Config[:default_country_id]))
      end

      # Regression test for #1142
      it 'uses the first available country if :default_country_id is set to an invalid value' do
        Viauco::Config[:default_country_id] = '0'
        expect(Viauco::Address.default.country).to eq(Viauco::Country.first)
      end
    end

    context 'user given' do
      let(:bill_address) { Viauco::Address.new(phone: Time.current.to_i) }
      ...
      it 'returns a copy of that user bill address' do
        expect(Viauco::Address.default(user).phone).to eq bill_address.phone
      end

      it 'falls back to build default when user has no address' do
        allow(user).to receive_messages(bill_address: nil)
        expect(Viauco::Address.default(user)).to eq Viauco::Address.build_default
      end
    end
  end
  ...
  context '#full_name' do
    context 'both first and last names are present' do
      let(:address) { stub_model(Viauco::Address, firstname: 'Michael', lastname: 'Jackson') }

      specify { expect(address.full_name).to eq('Michael Jackson') }
    end

    context 'first name is blank' do
      let(:address) { stub_model(Viauco::Address, firstname: nil, lastname: 'Jackson') }

      specify { expect(address.full_name).to eq('Jackson') }
    end

    context 'last name is blank' do
      let(:address) { stub_model(Viauco::Address, firstname: 'Michael', lastname: nil) }

      specify { expect(address.full_name).to eq('Michael') }
    end

    context 'both first and last names are blank' do
      let(:address) { stub_model(Viauco::Address, firstname: nil, lastname: nil) }
  ...
  context '#state_text' do
    context 'state is blank' do
      let(:address) { stub_model(Viauco::Address, state: nil, state_name: 'virginia') }

      specify { expect(address.state_text).to eq('virginia') }
    end

    context 'both name and abbr is present' do
      let(:state) { stub_model(Viauco::State, name: 'virginia', abbr: 'va') }
      let(:address) { stub_model(Viauco::Address, state: state) }

      specify { expect(address.state_text).to eq('va') }
    end

    context 'only name is present' do
      let(:state) { stub_model(Viauco::State, name: 'virginia', abbr: nil) }
      let(:address) { stub_model(Viauco::Address, state: state) }
  ...
  context 'defines require_phone? helper method' do
    let(:address) { stub_model(Viauco::Address) }
  ...
  describe '.build_default' do
    ...
    it { expect(_address.country).to eq(Viauco::Country.default) }
  end
  ...
  context 'editable & destroy' do
    ...
    it 'has required attributes' do
      expect(Viauco::Address.required_fields).to eq([:firstname, :lastname, :address1, :city, :country, :zipcode, :phone])
    end
    ...
    it 'is destroyed without saving used' do
      address.destroy
      expect(Viauco::Address.where(['id = (?)', address.id])).to be_empty
    end

    it 'is destroyed deleted timestamp' do
      address2.destroy
      expect(Viauco::Address.where(['id = (?)', address2.id])).to_not be_empty
    end
  end
