...
describe Viauco::Adjustment, type: :model do
  let(:order) { Viauco::Order.new }
  let(:adjustment) { Viauco::Adjustment.create!(label: 'Adjustment', adjustable: order, order: order, amount: 5) }
  ...
  describe '#amount=' do
    ...
    it 'is expected to equal to localized number' do
      expect(adjustment.amount).to eq(Viauco::LocalizedNumber.parse(amount))
    end
  end

  describe 'scopes' do
    describe '.for_complete_order' do
      subject { Viauco::Adjustment.for_complete_order }

      let(:complete_order) { Viauco::Order.create! completed_at: Time.current }
      let(:incomplete_order) { Viauco::Order.create! completed_at: nil }
      let(:adjustment_for_complete_order) { Viauco::Adjustment.create!(label: 'Adjustment', adjustable: complete_order, order: complete_order, amount: 5) }
      let(:adjustment_for_incomplete_order) { Viauco::Adjustment.create!(label: 'Adjustment', adjustable: incomplete_order, order: incomplete_order, amount: 5) }
    ...
    describe '.for_incomplete_order' do
      subject { Viauco::Adjustment.for_incomplete_order }
  ...
  context '#create & #destroy' do
    let(:adjustment) { Viauco::Adjustment.new(label: 'Adjustment', amount: 5, order: order, adjustable: create(:line_item)) }
  ...
  context '#save' do
    ...
    let!(:adjustment) { Viauco::Adjustment.create(label: 'Adjustment', amount: 5, order: order, adjustable: order) }
  ...
  describe 'non_tax scope' do
    subject do
      Viauco::Adjustment.non_tax.to_a
    end

    let!(:tax_adjustment) { create(:adjustment, order: order, source: create(:tax_rate)) }
    let!(:non_tax_adjustment_with_source) { create(:adjustment, order: order, source_type: 'Viauco::Order', source_id: nil) }
  ...
  describe 'competing_promos scope' do
    subject do
      Viauco::Adjustment.competing_promos.to_a
    end

    before do
      allow_any_instance_of(Viauco::Adjustment).to receive(:update_adjustable_adjustment_total).and_return(true)
    end

    let!(:promotion_adjustment) { create(:adjustment, order: order, source_type: 'Viauco::PromotionAction', source_id: nil) }
    let!(:custom_adjustment_with_source) { create(:adjustment, order: order, source_type: 'Custom', source_id: nil) }
    let!(:non_promotion_adjustment_with_source) { create(:adjustment, order: order, source_type: 'Viauco::Order', source_id: nil) }
    ...
    context 'no custom source_types have been added to competing_promos' do
      before { Viauco::Adjustment.competing_promos_source_types = ['Viauco::PromotionAction'] }
    ...
    context 'a custom source_type has been added to competing_promos' do
      before { Viauco::Adjustment.competing_promos_source_types = ['Viauco::PromotionAction', 'Custom'] }
  ...
  context '#currency' do
    let(:order) { Viauco::Order.new(currency: 'EUR') }
