...
describe Viauco::OrderMailer, type: :mailer do
  ...
  let(:order) do
    order = stub_model(Viauco::Order)
    product = stub_model(Viauco::Product, name: %{The "BEST" product})
    variant = stub_model(Viauco::Variant, product: product)
    price = stub_model(Viauco::Price, variant: variant, amount: 5.00)
    line_item = stub_model(Viauco::LineItem, variant: variant, order: order, quantity: 1, price: 4.99)
  ...
  context ':from not set explicitly' do
    it 'falls back to viauco config' do
      message = Viauco::OrderMailer.confirm_email(order)
  ...
  it "doesn't aggressively escape double quotes in confirmation body" do
    confirmation_email = Viauco::OrderMailer.confirm_email(order)
  ...
  it 'confirm_email accepts an order id as an alternative to an Order object' do
    expect(Viauco::Order).to receive(:find).with(order.id).and_return(order)
    expect do
      Viauco::OrderMailer.confirm_email(order.id).body
  ...
  it 'cancel_email accepts an order id as an alternative to an Order object' do
    expect(Viauco::Order).to receive(:find).with(order.id).and_return(order)
    expect do
      Viauco::OrderMailer.cancel_email(order.id).body
    end.not_to raise_error
  end
  ...
  context 'only shows eligible adjustments in emails' do
    ...
    let!(:confirmation_email) { Viauco::OrderMailer.confirm_email(order) }
    let!(:cancel_email) { Viauco::OrderMailer.cancel_email(order) }
  ...
  context 'displays unit costs from line item' do
    ...
    # Tests mailer view viauco/order_mailer/confirm_email.text.erb
    specify do
      confirmation_email = Viauco::OrderMailer.confirm_email(order)
    ...
    # Tests mailer view viauco/order_mailer/cancel_email.text.erb
    specify do
      cancel_email = Viauco::OrderMailer.cancel_email(order)
  ...
  context 'emails must be translatable' do
    context 'pt-BR locale' do
      before do
        ...
        pt_br_confirm_mail = { viauco: { order_mailer: { confirm_email: { dear_customer: 'Caro Cliente,' } } } }
        pt_br_cancel_mail = { viauco: { order_mailer: { cancel_email: { order_summary_canceled: 'Resumo da Pedido [CANCELADA]' } } } }
  ...
  context 'with preference :send_core_emails set to false' do
    it 'sends no email' do
      Viauco::Config.set(:send_core_emails, false)
      message = Viauco::OrderMailer.confirm_email(order)
