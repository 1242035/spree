    ...
    module Importer
      class Order
        def self.import(user, params)
          ...
          order = Viauco::Order.create! create_params
          ...
          params.delete(:user_id) unless user.try(:has_viauco_role?, 'admin') && params.key?(:user_id)
        ...
        def self.create_shipments_from_params(shipments_hash, order)
          ...
          shipments_hash.each do |s|
            ...
            shipment.stock_location = Viauco::StockLocation.find_by(admin_name: s[:stock_location]) ||
              Viauco::StockLocation.find_by!(name: s[:stock_location])
            ...
            shipping_method = Viauco::ShippingMethod.find_by(name: s[:shipping_method]) ||
              Viauco::ShippingMethod.find_by!(admin_name: s[:shipping_method])
        ...
        def self.create_line_items_from_params(line_items, order)
          ...
          line_items.each do |line_item|
            ...
            variant = Viauco::Variant.find(line_item[:variant_id])
        ...
        def self.create_payments_from_params(payments_hash, order)
          ...
          payments_hash.each do |p|
            ...
            # viauco_wombat serializes payment state as status so imported orders should fall back to status field.
            payment.state = p[:state] || p[:status] || 'completed'
            payment.created_at = p[:created_at] if p[:created_at]
            payment.payment_method = Viauco::PaymentMethod.find_by!(name: p[:payment_method])
        ...
        def self.ensure_variant_id_from_params(hash)
          ...
          unless hash[:variant_id].present?
            hash[:variant_id] = Viauco::Variant.active.find_by!(sku: sku).id
          end
          hash
        ...
        def self.ensure_country_id_from_params(address)
          ...
          begin
            ...
            address[:country_id] = Viauco::Country.where(search).first!.id
          rescue Exception => e
        ...
        def self.ensure_state_id_from_params(address)
          ...
          begin
            ...
            if state = Viauco::State.where(search).first
