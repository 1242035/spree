module Viauco
  module Core
    ...
    #   1) Make a copy of it in your sites local /lib/viauco folder
    #   2) Add it to the config load path, or require it in an initializer, e.g...
    #
    #      # config/initializers/viauco.rb
    ...
    module ProductFilters
      ...
      #
      Viauco::Product.add_search_scope :price_range_any do |*opts|
        conds = opts.map { |o| Viauco::Core::ProductFilters.price_filter[:conds][o] }.reject(&:nil?)
        scope = conds.shift
        conds.each do |new_scope|
          scope = scope.or(new_scope)
        end
        Viauco::Product.joins(master: :default_price).where(scope)
      end

      def self.format_price(amount)
        Viauco::Money.new(amount)
      end

      def self.price_filter
        v = Viauco::Price.arel_table
        conds = [[Viauco.t(:under_price, price: format_price(10)), v[:amount].lteq(10)],
                 ...
                 [Viauco.t(:or_over_price, price: format_price(20)), v[:amount].gteq(20)]]
        {
          name: Viauco.t(:price_range),
      ...
      Viauco::Product.add_search_scope :brand_any do |*opts|
        conds = opts.map { |o| ProductFilters.brand_filter[:conds][o] }.reject(&:nil?)
        scope = conds.shift
        conds.each do |new_scope|
          scope = scope.or(new_scope)
        end
        Viauco::Product.with_property('brand').where(scope)
      end

      def self.brand_filter
        brand_property = Viauco::Property.find_by(name: 'brand')
        brands = brand_property ? Viauco::ProductProperty.where(property_id: brand_property.id).pluck(:value).uniq.map(&:to_s) : []
        pp = Viauco::ProductProperty.arel_table
      ...
      Viauco::Product.add_search_scope :selective_brand_any do |*opts|
        Viauco::Product.brand_any(*opts)
      end

      def self.selective_brand_filter(taxon = nil)
        taxon ||= Viauco::Taxonomy.first.root
        brand_property = Viauco::Property.find_by(name: 'brand')
        scope = Viauco::ProductProperty.where(property: brand_property).
                joins(product: :taxons).
                where("#{Viauco::Taxon.table_name}.id" => [taxon] + taxon.descendants)
      ...
      def self.taxons_below(taxon)
        return Viauco::Core::ProductFilters.all_taxons if taxon.nil?
      ...
      def self.all_taxons
        taxons = Viauco::Taxonomy.all.map { |t| [t.root] + t.root.descendants }.flatten
        {
          name: 'All taxons',
