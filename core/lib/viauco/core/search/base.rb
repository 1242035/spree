    ...
    module Search
      class Base
        ...
        def initialize(params)
          self.current_currency = Viauco::Config[:currency]
          @properties = {}
          prepare(params)
        end
        ...
        def retrieve_products
          ...
          unless Viauco::Config.show_products_without_price
            @products = @products.where('viauco_prices.amount IS NOT NULL').
                        where('viauco_prices.currency' => current_currency)
          end
        ...
        def get_base_scope
          base_scope = Viauco::Product.viauco_base_scopes.active
          ...
          if search.is_a?(ActionController::Parameters)
            search.each do |name, scope_attribute|
              ...
              base_scope = if base_scope.respond_to?(:search_scopes) && base_scope.search_scopes.include?(scope_name.to_sym)
                             ...
                             base_scope.merge(Viauco::Product.ransack(scope_name => scope_attribute).result)
                           end
            end
          end
          base_scope
        ...
        def prepare(params)
          @properties[:taxon] = params[:taxon].blank? ? nil : Viauco::Taxon.find(params[:taxon])
          @properties[:keywords] = params[:keywords]
          @properties[:search] = params[:search]
          @properties[:include_images] = params[:include_images]

          per_page = params[:per_page].to_i
          @properties[:per_page] = per_page > 0 ? per_page : Viauco::Config[:products_per_page]
