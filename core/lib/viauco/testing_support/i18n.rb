...
# within the code that Viauco's test suite covers.
#
...
module Viauco
  ...
  def self.check_missing_translations
    ...
    used_translations.map { |a| a.split('.') }.each do |translation_keys|
      ...
      translation_keys.each do |key|
        ...
        unless Viauco.missing_translation_messages.include?(error)
          Viauco.missing_translation_messages << error
        end
      end
    end
  end
  ...
  def self.check_unused_translations
    ...
    translation_diff.each do |translation|
      Viauco.unused_translation_messages << "#{translation} (#{I18n.locale})"
    end
  end
  ...
  def self.translations
    @translations ||= I18n.backend.send(:translations)[I18n.locale][:viauco]
  end
end
...
RSpec.configure do |config|
  ...
  if ENV['CHECK_TRANSLATIONS']
    config.after :suite do
      ...
      if Viauco.missing_translation_messages.any?
        puts "\nThere are missing translations within Viauco:"
        puts Viauco.missing_translation_messages.sort
        exit(1)
      end

      Viauco.check_unused_translations
      if false && Viauco.unused_translation_messages.any?
        puts "\nThere are unused translations within Viauco:"
        puts Viauco.unused_translation_messages.sort
        exit(1)
      end
    end
  end
