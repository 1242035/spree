module Viauco
  module Products
    class Find
      ...
      def name_matcher
        Viauco::Product.arel_table[:name].matches("%#{name}%")
      end
      ...
      def by_skus(products)
        ...
        products.joins(:variants_including_master).distinct.where(viauco_variants: { sku: skus })
      end
      ...
      def by_taxons(products)
        ...
        products.joins(:taxons).distinct.where(viauco_taxons: { id: taxons })
      end
