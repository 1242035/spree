  ...
  module Checkout
    class GetShippingRates
      ...
      def ensure_shipping_address(order:)
        return failure([], Viauco.t('errors.services.get_shipping_rates.no_shipping_address')) if order.ship_address.blank?

        success(order: order)
      end

      def ensure_line_items_present(order:)
        return failure([], Viauco.t('errors.services.get_shipping_rates.no_line_items')) if order.line_items.empty?
      ...
      def move_order_to_delivery_state(order:)
        Viauco::Dependencies.checkout_next_service.constantize.call(order: order) until order.state == 'delivery'
