  ...
  module Cart
    class Recalculate
      ...
      def call(order:, line_item:, line_item_created: false, options: {})
        order_updater = ::Viauco::OrderUpdater.new(order)
        ...
        ::Viauco::PromotionHandler::Cart.new(order, line_item).activate
        ::Viauco::Adjustable::AdjustmentsUpdater.update(line_item)
        ::Viauco::TaxRate.adjust(order, [line_item.reload]) if line_item_created
        order_updater.update
        success(line_item)
      end
    end
