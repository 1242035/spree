module Viauco
  module BaseHelper
    def available_countries
      checkout_zone = Viauco::Zone.find_by(name: Viauco::Config[:checkout_zone])
      ...
      countries.collect do |country|
        country.name = Viauco.t(country.iso, scope: 'country_names', default: country.name)
        country
    ...
    def logo(image_path = Viauco::Config[:logo])
      link_to image_tag(image_path), viauco.respond_to?(:root_path) ? viauco.root_path : main_app.root_path
    end
    ...
    def meta_data
      ...
      if meta[:description].blank? && object.is_a?(Viauco::Product)
    ...
    def seo_url(taxon)
      viauco.nested_taxons_path(taxon.permalink)
    end
    ...
    def define_image_method(style)
      self.class.send :define_method, "#{style}_image" do |product, *options|
        ...
        if product.images.empty?
          if !product.is_a?(Viauco::Variant) && !product.variant_images.empty?
    ...
    def image_style_from_method_name(method_name)
      if method_name.to_s.match(/_image$/) && style = method_name.to_s.sub(/_image$/, '')
        style if style.in? Viauco::Image.styles.with_indifferent_access
      end
    end
  end
end
