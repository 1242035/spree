module Viauco
  module ProductsHelper
    ...
    def variant_price(variant)
      if Viauco::Config[:show_variant_full_price]
    ...
    def variant_price_diff(variant)
      ...
      amount = Viauco::Money.new(diff.abs, currency: current_currency).to_html
      label  = diff > 0 ? :add : :subtract
      "(#{Viauco.t(label)}: #{amount})".html_safe
    end
    ...
    def variant_full_price(variant)
      ...
      unless product.variants.active(current_currency).all? { |v| v.price == product.price }
        Viauco::Money.new(variant.price, currency: current_currency).to_html
      end
    end

    # converts line breaks in product description into <p> tags (for html display purposes)
    def product_description(product)
      description = if Viauco::Config[:show_raw_product_description]
                      product.description
                    else
                      product.description.to_s.gsub(/(.*?)\r?\n\r?\n/m, '<p>\1</p>')
                    end
      description.blank? ? Viauco.t(:product_has_no_description) : description
    end
    ...
    def line_item_description_text(description_text)
      if description_text.present?
        ...
        Viauco.t(:product_has_no_description)
      end
    end
    ...
    def cache_key_for_products
      ...
      products_cache_keys = "viauco/products/all-#{params[:page]}-#{max_updated_at}-#{count}"
    ...
    def available_status(product) # will return a human readable string
      return Viauco.t(:discontinued)  if product.discontinued?
      return Viauco.t(:deleted) if product.deleted?

      if product.available?
        Viauco.t(:available)
      elsif product.available_on&.future?
        Viauco.t(:pending_sale)
      else
        Viauco.t(:no_available_date_set)
      end
    end
