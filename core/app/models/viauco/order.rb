require_dependency 'viauco/order/checkout'
require_dependency 'viauco/order/currency_updater'
require_dependency 'viauco/order/payments'
require_dependency 'viauco/order/store_credit'

module Viauco
  class Order < Viauco::Base
    PAYMENT_STATES = %w(balance_due credit_owed failed paid void)
    SHIPMENT_STATES = %w(backorder canceled partial pending ready shipped)

    include Viauco::Order::Checkout
    include Viauco::Order::CurrencyUpdater
    include Viauco::Order::Payments
    include Viauco::Order::StoreCredit
    include Viauco::Order::AddressBook
    include Viauco::Core::NumberGenerator.new(prefix: 'R')
    ...
    if Viauco.user_class
      belongs_to :user, class_name: Viauco.user_class.to_s, optional: true
      belongs_to :created_by, class_name: Viauco.user_class.to_s, optional: true
      belongs_to :approver, class_name: Viauco.user_class.to_s, optional: true
      belongs_to :canceler, class_name: Viauco.user_class.to_s, optional: true
    else
    ...
    belongs_to :bill_address, foreign_key: :bill_address_id, class_name: 'Viauco::Address',
                              optional: true, dependent: :destroy
    alias_attribute :billing_address, :bill_address

    belongs_to :ship_address, foreign_key: :ship_address_id, class_name: 'Viauco::Address',
                              optional: true, dependent: :destroy
    alias_attribute :shipping_address, :ship_address

    belongs_to :store, class_name: 'Viauco::Store'

    with_options dependent: :destroy do
      has_many :state_changes, as: :stateful, class_name: 'Viauco::StateChange'
      has_many :line_items, -> { order(:created_at) }, inverse_of: :order, class_name: 'Viauco::LineItem'
      has_many :payments, class_name: 'Viauco::Payment'
      has_many :return_authorizations, inverse_of: :order, class_name: 'Viauco::ReturnAuthorization'
      has_many :adjustments, -> { order(:created_at) }, as: :adjustable, class_name: 'Viauco::Adjustment'
    end
    has_many :reimbursements, inverse_of: :order, class_name: 'Viauco::Reimbursement'
    has_many :line_item_adjustments, through: :line_items, source: :adjustments
    has_many :inventory_units, inverse_of: :order, class_name: 'Viauco::InventoryUnit'
    has_many :variants, through: :line_items
    has_many :products, through: :variants
    has_many :refunds, through: :payments
    has_many :all_adjustments,
             class_name: 'Viauco::Adjustment',
             foreign_key: :order_id,
             dependent: :destroy,
             inverse_of: :order

    has_many :order_promotions, class_name: 'Viauco::OrderPromotion'
    has_many :promotions, through: :order_promotions, class_name: 'Viauco::Promotion'

    has_many :shipments, class_name: 'Viauco::Shipment', dependent: :destroy, inverse_of: :order do
      def states
        pluck(:state).uniq
      end
    end
    ...
    scope :reverse_chronological, -> { order(Arel.sql('viauco_orders.completed_at IS NULL'), completed_at: :desc, created_at: :desc) }
    ...
    def confirmation_required?
      Viauco::Config[:always_include_confirm_step] ||
    ...
    def tax_address
      Viauco::Config[:tax_using_ship_address] ? ship_address : bill_address
    end
    ...
    def merger
      @merger ||= Viauco::OrderMerger.new(self)
    end

    def ensure_store_presence
      self.store ||= Viauco::Store.default
    end
    ...
    def find_line_item_by_variant(variant, options = {})
      line_items.detect do |line_item|
        line_item.variant_id == variant.id &&
          Viauco::Dependencies.cart_compare_line_items_service.constantize.new.call(order: self, line_item: line_item, options: options).value
      end
    end
    ...
    def create_tax_charge!
      Viauco::TaxRate.adjust(self, line_items)
      Viauco::TaxRate.adjust(self, shipments) if shipments.any?
    end
    ...
    def outstanding_balance
      if canceled?
        ...
        # See: https://github.com/viauco/viauco/issues/6229 & https://github.com/viauco/viauco/issues/8136
    ...
    def ensure_line_item_variants_are_not_discontinued
      if line_items.any? { |li| !li.variant || li.variant.discontinued? }
        ...
        errors.add(:base, Viauco.t(:discontinued_variants_present))
        false
      else
        true
      end
    ...
    def ensure_line_items_are_in_stock
      if insufficient_stock_lines.present?
        restart_checkout_flow
        errors.add(:base, Viauco.t(:insufficient_stock_lines_present))
        false
      else
        true
      end
    ...
    def empty!
      if completed?
        raise Viauco.t(:cannot_empty_completed_order)
      else
    ...
    def can_add_coupon?
      Viauco::Promotion.order_activatable?(self)
    end
    ...
    def create_proposed_shipments
      ...
      StateChange.where(stateful_type: 'Viauco::Shipment', stateful_id: shipment_ids).delete_all
      ...
      self.shipments = Viauco::Stock::Coordinator.new(self).shipments
    end

    def apply_free_shipping_promotions
      Viauco::PromotionHandler::FreeShipping.new(self).activate
      shipments.each { |shipment| Viauco::Adjustable::AdjustmentsUpdater.update(shipment) }
    ...
    def apply_unassigned_promotions
      ::Viauco::PromotionHandler::Cart.new(self).activate
    end
    ...
    def has_non_reimbursement_related_refunds?
      refunds.non_reimbursement.exists? ||
        payments.offset_payment.exists? # how old versions of viauco stored refunds
    end
    ...
    def validate_payments_attributes(attributes)
      ...
      payment_methods = Viauco::PaymentMethod.where(id: available_payment_methods.map(&:id))
    ...
    def ensure_line_items_present
      unless line_items.present?
        errors.add(:base, Viauco.t(:there_are_no_items_for_this_order)) && (return false)
      end
    end
    ...
    def ensure_available_shipping_rates
      if shipments.empty? || shipments.any? { |shipment| shipment.shipping_rates.blank? }
        ...
        shipments.destroy_all
        errors.add(:base, Viauco.t(:items_cannot_be_shipped)) && (return false)
      end
    end
    ...
    def ensure_currency_presence
      self.currency ||= store.default_currency || Viauco::Config[:currency]
    end
