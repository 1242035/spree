module Viauco
  class CreditCard < Viauco::Base
    include ActiveMerchant::Billing::CreditCardMethods

    # to migrate safely from older Viauco versions that din't provide safe deletion for CCs
    # we need to ensure that we have a connection to the DB and that the `deleted_at` column exists
    if !ENV['VIAUCO_DISABLE_DB_CONNECTION'] &&
        connected? &&
        table_exists? &&
        connection.column_exists?(:viauco_credit_cards, :deleted_at)
      acts_as_paranoid
    end
    ...
    # More info: https://github.com/viauco/viauco/issues/6209
    #
    ...
    # ActiveMerchant needs first_name/last_name because we pass it a Viauco::CreditCard and it calls those methods on it.
