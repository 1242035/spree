    ...
    class Coupon
      ...
      def remove_promotion_adjustments(promotion)
        ...
        order.all_adjustments.where(source_id: promotion_actions_ids,
                                    source_type: 'Viauco::PromotionAction').destroy_all
      end

      def remove_promotion_line_items(promotion)
        create_line_item_actions_ids = promotion.actions.where(type: 'Viauco::Promotion::Actions::CreateLineItems').pluck(:id)

        Viauco::PromotionActionLineItem.where(promotion_action: create_line_item_actions_ids).find_each do |item|
          line_item = order.find_line_item_by_variant(item.variant)
          next if line_item.blank?

          Viauco::Dependencies.cart_remove_item_service(order: order, item: item.variant, quantity: item.quantity)
        end
      end
      ...
      def determine_promotion_application_result
        ...
        created_line_items = promotion.actions.detect do |a|
          Object.const_get(a.type).ancestors.include?(
            Viauco::Promotion::Actions::CreateLineItems
          )
        end
