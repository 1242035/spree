require_dependency 'viauco/payment/processing'

module Viauco
  class Payment < Viauco::Base
    include Viauco::Core::NumberGenerator.new(prefix: 'P', letters: true, length: 7)

    include Viauco::Payment::Processing
    ...
    with_options inverse_of: :payments do
      belongs_to :order, class_name: 'Viauco::Order', touch: true
      belongs_to :payment_method, class_name: 'Viauco::PaymentMethod'
    end
    belongs_to :source, polymorphic: true

    has_many :offsets, -> { offset_payment }, class_name: 'Viauco::Payment', foreign_key: :source_id
    has_many :log_entries, as: :source
    has_many :state_changes, as: :stateful
    has_many :capture_events, class_name: 'Viauco::PaymentCaptureEvent'
    ...
    scope :from_credit_card, -> { where(source_type: 'Viauco::CreditCard') }
    scope :with_state, ->(s) { where(state: s.to_s) }
    # "offset" is reserved by activerecord
    scope :offset_payment, -> { where("source_type = 'Viauco::Payment' AND amount < 0 AND state = 'completed'") }
    ...
    scope :store_credits, -> { where(source_type: Viauco::StoreCredit.to_s) }
    scope :not_store_credits, -> { where(arel_table[:source_type].not_eq(Viauco::StoreCredit.to_s).or(arel_table[:source_type].eq(nil))) }
    ...
    # see https://github.com/viauco/viauco/issues/981
    def build_source
    ...
    def validate_source
      if source && !source.valid?
        source.errors.each do |field, error|
          ...
          errors.add(Viauco.t(source.class.to_s.demodulize.underscore), "#{field_name} #{error}")
        end
      end
      !errors.present?
    end
    ...
    def create_eligible_credit_event
      ...
      # payment has a source of itself (Viauco::Payment) no matter the
      # type of payment getting refunded, hence the additional check
      # if the source is a store credit.
      return unless store_credit? && source.is_a?(Viauco::StoreCredit)

      # creates the store credit event
      source.update!(action: Viauco::StoreCredit::ELIGIBLE_ACTION,
