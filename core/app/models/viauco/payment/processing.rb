require_dependency 'viauco/payment/gateway_options'
  ...
  class Payment < Viauco::Base
    module Processing
      ...
      included do
        ...
        self.gateway_options_class = Viauco::Payment::GatewayOptions
      end
          ...
          if source
            unless processing?
              if payment_method.supports?(source) || token_based?
                ...
                invalidate!
                raise Core::GatewayError, Viauco.t(:payment_method_not_supported)
              end
            end
          else
            raise Core::GatewayError, Viauco.t(:payment_processing_failed)
          end
        end
      end
      ...
      def gateway_error(error)
        text = if error.is_a? ActiveMerchant::Billing::Response
                 ...
                 Viauco.t(:unable_to_connect_to_gateway)
               else
                 error.to_s
               end
        logger.error(Viauco.t(:gateway_error))
