module Viauco
  class StockTransfer < Viauco::Base
    include Viauco::Core::NumberGenerator.new(prefix: 'T')
    ...
    def find_stock_location_with_location_id(location_id)
      stock_movements.joins(:stock_item).where('viauco_stock_items.stock_location_id' => location_id)
    end
  end
end
