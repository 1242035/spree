...
module Viauco
  class Adjustment < Viauco::Base
    with_options polymorphic: true do
      belongs_to :adjustable, touch: true
      belongs_to :source
    end
    belongs_to :order, class_name: 'Viauco::Order', inverse_of: :all_adjustments
    ...
    self.competing_promos_source_types = ['Viauco::PromotionAction']

    scope :not_finalized, -> { where(state: 'open') }
    scope :finalized, -> { where(state: 'closed') }
    scope :tax, -> { where(source_type: 'Viauco::TaxRate') }
    scope :non_tax, -> do
      source_type = arel_table[:source_type]
      where(source_type.not_eq('Viauco::TaxRate').or(source_type.eq(nil)))
    end
    scope :price, -> { where(adjustable_type: 'Viauco::LineItem') }
    ...
    scope :promotion, -> { where(source_type: 'Viauco::PromotionAction') }
    scope :return_authorization, -> { where(source_type: 'Viauco::ReturnAuthorization') }
    scope :is_included, -> { where(included: true) }
    scope :additional, -> { where(included: false) }
    scope :competing_promos, -> { where(source_type: competing_promos_source_types) }
    scope :for_complete_order, -> { joins(:order).merge(Viauco::Order.complete) }
    scope :for_incomplete_order, -> { joins(:order).merge(Viauco::Order.incomplete) }

    extend DisplayMoney
    money_methods :amount

    def amount=(amount)
      self[:amount] = Viauco::LocalizedNumber.parse(amount)
    end
