  ...
  class Ability
    ...
    def initialize(user)
      ...
      if user.respond_to?(:has_viauco_role?) && user.has_viauco_role?('admin')
        ...
        can :manage, Viauco::Address do |address|
          address.user == user
        end
        can :create, Viauco::Address do |_address|
          user.id.present?
        end
        ...
        can [:show, :update, :destroy], Viauco.user_class, id: user.id
        can :read, State
        can :read, Taxon
        can :read, Taxonomy
        can :read, Variant
