  ...
  class Promotion
    module Rules
      class Taxon < PromotionRule
        has_many :promotion_rule_taxons, class_name: 'Viauco::PromotionRuleTaxon',
                                         foreign_key: 'promotion_rule_id',
                                         dependent: :destroy
        has_many :taxons, through: :promotion_rule_taxons, class_name: 'Viauco::Taxon'
        ...
        def taxon_ids_string=(s)
          ...
          self.taxons = Viauco::Taxon.find(ids)
        end
        ...
        def order_taxons(order)
          Viauco::Taxon.joins(products: { variants_including_master: :line_items }).where(viauco_line_items: { order_id: order.id }).distinct
        end
        ...
        def taxon_product_ids
          Viauco::Product.joins(:taxons).where(viauco_taxons: { id: taxons.pluck(:id) }).pluck(:id).uniq
        end
      end
    end
  end
