module Viauco
  class ReturnItem < Viauco::Base
    ...
    has_many :exchange_inventory_units, class_name: 'Viauco::InventoryUnit',
                                        foreign_key: :original_return_item_id,
                                        inverse_of: :original_return_item
    belongs_to :exchange_variant, class_name: 'Viauco::Variant'
    belongs_to :preferred_reimbursement_type, class_name: 'Viauco::ReimbursementType'
    belongs_to :override_reimbursement_type, class_name: 'Viauco::ReimbursementType'
    ...
    scope :exchange_required, -> { eager_load(:exchange_inventory_units).where(viauco_inventory_units: { original_return_item_id: nil }).distinct }
    ...
    def currency
      return_authorization.try(:currency) || Viauco::Config[:currency]
    end
    ...
    def stock_item
      ...
      Viauco::StockItem.find_by(variant_id: inventory_unit.variant_id,
    ...
    def belongs_to_same_customer_order
      ...
      if customer_return.order_id != inventory_unit.order_id
        errors.add(:base, Viauco.t(:return_items_cannot_be_associated_with_multiple_orders))
      end
    end
    ...
    def eligible_exchange_variant
      ...
      unless eligible_exchange_variants.include?(exchange_variant)
        errors.add(:base, Viauco.t(:invalid_exchange_variant))
      end
    end
    ...
    def sufficient_quantity_for_return
      ...
      errors.add(:return_quantity, Viauco.t(:cannot_return_more_than_bought_quantity))
    end
    ...
    def validate_no_other_completed_return_items
      other_return_item = Viauco::ReturnItem.where(inventory_unit_id: inventory_unit_id,
    ...
    def cancel_others
      Viauco::ReturnItem.where(inventory_unit_id: inventory_unit_id).where.not(id: id).
        not_cancelled.each(&:cancel!)
    end

    def should_restock?
      resellable? && variant.should_track_inventory? && stock_item && Viauco::Config[:restock_inventory]
    end
  end
end
