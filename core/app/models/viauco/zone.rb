module Viauco
  class Zone < Viauco::Base
    with_options dependent: :destroy, inverse_of: :zone do
      has_many :zone_members, class_name: 'Viauco::ZoneMember'
      has_many :tax_rates
    end
    with_options through: :zone_members, source: :zoneable do
      has_many :countries, source_type: 'Viauco::Country'
      has_many :states, source_type: 'Viauco::State'
    end

    has_many :shipping_method_zones, class_name: 'Viauco::ShippingMethodZone'
    has_many :shipping_methods, through: :shipping_method_zones, class_name: 'Viauco::ShippingMethod'
    ...
    def self.potential_matching_zones(zone)
      if zone.country?
        ...
        joins(countries: :zones).
          where('zone_members_viauco_countries_join.zone_id = ?', zone.id).
          distinct
      else
        ...
        joins(:zone_members).where(
          "(viauco_zone_members.zoneable_type = 'Viauco::State' AND
            viauco_zone_members.zoneable_id IN (?))
           OR (viauco_zone_members.zoneable_type = 'Viauco::Country' AND
            viauco_zone_members.zoneable_id IN (?))",
          zone.state_ids,
    ...
    def self.match(address)
      return unless address &&
        matches = includes(:zone_members).
                    order('viauco_zones.zone_members_count', 'viauco_zones.created_at').
                    where("(viauco_zone_members.zoneable_type = 'Viauco::Country' AND " \
                          'viauco_zone_members.zoneable_id = ?) OR ' \
                          "(viauco_zone_members.zoneable_type = 'Viauco::State' AND " \
                          'viauco_zone_members.zoneable_id = ?)', address.country_id, address.state_id).
                    references(:zones)
    ...
    def country_ids=(ids)
      set_zone_members(ids, 'Viauco::Country')
    end

    def state_ids=(ids)
      set_zone_members(ids, 'Viauco::State')
    end
    ...
    def remove_previous_default
      Viauco::Zone.with_default_tax.where.not(id: id).update_all(default_tax: false)
    end
