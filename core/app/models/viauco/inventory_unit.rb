module Viauco
  class InventoryUnit < Viauco::Base
    with_options inverse_of: :inventory_units do
      ...
      belongs_to :shipment, class_name: 'Viauco::Shipment', touch: true, optional: true
      belongs_to :return_authorization, class_name: 'Viauco::ReturnAuthorization'
      belongs_to :line_item, class_name: 'Viauco::LineItem'
    end

    has_many :return_items, inverse_of: :inventory_unit
    belongs_to :original_return_item, class_name: 'Viauco::ReturnItem'
    ...
    scope :backordered_per_variant, ->(stock_item) do
      includes(:shipment, :order).
        where.not(viauco_shipments: { state: 'canceled' }).
        where(variant_id: stock_item.variant_id).
        where.not(viauco_orders: { completed_at: nil }).
        backordered.order('viauco_orders.completed_at ASC')
    end
    ...
    def find_stock_item
      Viauco::StockItem.where(stock_location_id: shipment.stock_location_id,
    ...
    def variant
      Viauco::Variant.unscoped { super }
    end

    def current_or_new_return_item
      Viauco::ReturnItem.from_inventory_unit(self)
    end
