module Viauco
  class CustomerReturn < Viauco::Base
    include Viauco::Core::NumberGenerator.new(prefix: 'CR', length: 9)
    ...
    extend DisplayMoney
    money_methods pre_tax_total: { currency: Viauco::Config[:currency] }
    ...
    def must_have_return_authorization
      if item = return_items.find { |ri| ri.return_authorization.blank? }
        errors.add(:base, Viauco.t(:missing_return_authorization, item_name: item.inventory_unit.variant.name))
      end
    end
