module Viauco
  class Variant < Viauco::Base
    acts_as_paranoid
    acts_as_list scope: :product

    belongs_to :product, touch: true, class_name: 'Viauco::Product', inverse_of: :variants
    belongs_to :tax_category, class_name: 'Viauco::TaxCategory', optional: true
    ...
    has_many :option_value_variants, class_name: 'Viauco::OptionValueVariant'
    has_many :option_values, through: :option_value_variants, class_name: 'Viauco::OptionValue'

    has_many :images, -> { order(:position) }, as: :viewable, dependent: :destroy, class_name: 'Viauco::Image'

    has_many :prices,
             class_name: 'Viauco::Price',
             dependent: :destroy,
             inverse_of: :variant
    ...
    scope :backorderable, -> { joins(:stock_items).where(viauco_stock_items: { backorderable: true }) }
    ...
    scope :not_deleted, -> { where("#{Viauco::Variant.quoted_table_name}.deleted_at IS NULL") }

    scope :for_currency_and_available_price_amount, ->(currency = nil) do
      currency ||= Viauco::Config[:currency]
      joins(:prices).where('viauco_prices.currency = ?', currency).where('viauco_prices.amount IS NOT NULL').distinct
    end
    ...
    LOCALIZED_NUMBERS.each do |m|
      define_method("#{m}=") do |argument|
        self[m] = Viauco::LocalizedNumber.parse(argument) if argument.present?
      end
    end
    ...
    def tax_category
      if self[:tax_category_id].nil?
        ...
        Viauco::TaxCategory.find(self[:tax_category_id])
      end
    end
    ...
    def set_option_value(opt_name, opt_value)
      ...
      option_type = Viauco::OptionType.where(name: opt_name).first_or_initialize do |o|
      ...
      option_value = Viauco::OptionValue.where(option_type_id: option_type.id, name: opt_value).first_or_initialize do |o|
    ...
    def should_track_inventory?
      track_inventory? && Viauco::Config.track_inventory_levels
    end
    ...
    def quantifier
      Viauco::Stock::Quantifier.new(self)
    end
    ...
    def check_price
      if price.nil? && Viauco::Config[:require_master_price]
      ...
      if price.present? && currency.nil?
        self.currency = Viauco::Config[:currency]
      end
    end

    def set_cost_currency
      self.cost_currency = Viauco::Config[:currency] if cost_currency.blank?
    end
