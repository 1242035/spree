module Viauco
  class StoreCreditCategory < Viauco::Base
    ...
    def non_expiring_category_types
      DEFAULT_NON_EXPIRING_TYPES | Viauco::Config[:non_expiring_credit_types]
    end

    def store_credit_category_used?
      Viauco::StoreCredit.exists?(category_id: id)
    end
    ...
    class << self
      def default_reimbursement_category(_options = {})
        Viauco::StoreCreditCategory.first
      end
    end
  end
end
