module Viauco
  class PaymentMethod < Viauco::Base
    acts_as_paranoid
    acts_as_list
    ...
    with_options dependent: :restrict_with_error do
      has_many :payments, class_name: 'Viauco::Payment', inverse_of: :payment_method
      has_many :credit_cards, class_name: 'Viauco::CreditCard'
    end

    def self.providers
      Rails.application.config.viauco.payment_methods
    end
    ...
    def auto_capture?
      auto_capture.nil? ? Viauco::Config[:auto_capture] : auto_capture
    end
    ...
    def store_credit?
      self.class == Viauco::PaymentMethod::StoreCredit
    end
