module Viauco
  class ReturnAuthorization < Viauco::Base
    include Viauco::Core::NumberGenerator.new(prefix: 'RA', length: 9)

    belongs_to :order, class_name: 'Viauco::Order', inverse_of: :return_authorizations
    ...
    belongs_to :reason, class_name: 'Viauco::ReturnAuthorizationReason', foreign_key: :return_authorization_reason_id
    ...
    def currency
      order.nil? ? Viauco::Config[:currency] : order.currency
    end
    ...
    def must_have_shipped_units
      if order.nil? || order.inventory_units.shipped.none?
        errors.add(:order, Viauco.t(:has_no_shipped_units))
      end
    end
    ...
    def generate_expedited_exchange_reimbursements
      return unless Viauco::Config[:expedited_exchanges]
