...
module Viauco
  class Product < Viauco::Base
    extend FriendlyId
    include Viauco::ProductScopes
    ...
    has_many :product_promotion_rules, class_name: 'Viauco::ProductPromotionRule'
    has_many :promotion_rules, through: :product_promotion_rules, class_name: 'Viauco::PromotionRule'

    has_many :promotions, through: :promotion_rules, class_name: 'Viauco::Promotion'

    has_many :possible_promotions, -> { advertised.active }, through: :promotion_rules,
                                                             class_name: 'Viauco::Promotion',
                                                             source: :promotion

    belongs_to :tax_category, class_name: 'Viauco::TaxCategory'
    belongs_to :shipping_category, class_name: 'Viauco::ShippingCategory', inverse_of: :products

    has_one :master,
            -> { where is_master: true },
            inverse_of: :product,
            class_name: 'Viauco::Variant'
    ...
    has_many :prices, -> { order('viauco_variants.position, viauco_variants.id, currency') }, through: :variants
    ...
    with_options presence: true do
      ...
      validates :price, if: proc { Viauco::Config[:require_master_price] }
    end
    ...
    def property(property_name)
      product_properties.joins(:property).find_by(viauco_properties: { name: property_name }).try(:value)
    end

    def set_property(property_name, property_value, property_presentation = property_name)
      ApplicationRecord.transaction do
        # Works around viauco_i18n #301
    ...
    def brand
      taxons.joins(:taxonomy).find_by(viauco_taxonomies: { name: Viauco.t(:taxonomy_brands_name) })
    end

    def category
      taxons.joins(:taxonomy).find_by(viauco_taxonomies: { name: Viauco.t(:taxonomy_categories_name) })
    end

    private

    def add_associations_from_prototype
      if prototype_id && prototype = Viauco::Prototype.find_by(id: prototype_id)
    ...
    def any_variants_not_track_inventory?
      return true unless Viauco::Config.track_inventory_levels
    ...
    def touch_taxons
      Viauco::Taxon.where(id: taxon_and_ancestors.map(&:id)).update_all(updated_at: Time.current)
      Viauco::Taxonomy.where(id: taxonomy_ids).update_all(updated_at: Time.current)
    end
