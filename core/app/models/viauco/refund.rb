module Viauco
  class Refund < Viauco::Base
    with_options inverse_of: :refunds do
      belongs_to :payment
      belongs_to :reimbursement, optional: true
    end
    belongs_to :reason, class_name: 'Viauco::RefundReason', foreign_key: :refund_reason_id
    ...
    def perform!
      ...
      credit_cents = Viauco::Money.new(amount.to_f, currency: payment.currency).amount_in_cents
    ...
    def process!(credit_cents)
      ...
      unless response.success?
        logger.error(Viauco.t(:gateway_error) + "  #{response.to_yaml}")
      ...
      logger.error(Viauco.t(:gateway_error) + "  #{e.inspect}")
      raise Core::GatewayError, Viauco.t(:unable_to_connect_to_gateway)
    end
