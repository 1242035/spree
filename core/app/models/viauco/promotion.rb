module Viauco
  class Promotion < Viauco::Base
    ...
    has_many :orders, through: :order_promotions, class_name: 'Viauco::Order'
    ...
    scope :applied, lambda {
      joins(<<-SQL).distinct
        INNER JOIN viauco_order_promotions
        ON viauco_order_promotions.promotion_id = #{table_name}.id
      SQL
    }
    ...
    def self.with_coupon_code(coupon_code)
      where("lower(#{table_name}.code) = ?", coupon_code.strip.downcase).
        includes(:promotion_actions).where.not(viauco_promotion_actions: { id: nil }).
        first
    end

    def self.active
      where('viauco_promotions.starts_at IS NULL OR viauco_promotions.starts_at < ?', Time.current).
        where('viauco_promotions.expires_at IS NULL OR viauco_promotions.expires_at > ?', Time.current)
    end
    ...
    def products
      rules.where(type: 'Viauco::Promotion::Rules::Product').map(&:products).flatten.uniq
    end
    ...
    def used_by?(user, excluded_orders = [])
      [
        ...
        user.orders.complete.joins(adjustment_type).where(
          viauco_adjustments: {
            source_type: 'Viauco::PromotionAction',
    ...
    def blacklisted?(promotable)
      case promotable
      when Viauco::LineItem
        !promotable.product.promotionable?
      when Viauco::Order
        promotable.line_items.any? &&
          promotable.line_items.joins(:product).where(viauco_products: { promotionable: true }).none?
      end
    end
