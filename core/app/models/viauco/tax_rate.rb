module Viauco
  class TaxRate < Viauco::Base
    acts_as_paranoid

    include Viauco::CalculatedAdjustments
    include Viauco::AdjustmentSource

    with_options inverse_of: :tax_rates do
      belongs_to :zone, class_name: 'Viauco::Zone', optional: true
      belongs_to :tax_category,
                 class_name: 'Viauco::TaxCategory'
    end
    ...
    scope :potential_rates_for_zone,
          ->(zone) do
            where(zone_id: Viauco::Zone.potential_matching_zones(zone).pluck(:id))
          end
    scope :for_default_zone,
          -> { potential_rates_for_zone(Viauco::Zone.default_tax) }
    ...
    # https://github.com/viauco/viauco/issues/4318#issuecomment-34723428
    def self.store_pre_tax_amount(item, rates)
      pre_tax_amount = case item.class.to_s
                       when 'Viauco::LineItem' then item.discounted_amount
                       when 'Viauco::Shipment' then item.discounted_cost
                       end
    ...
    def self.adjust(order, items)
      ...
      Viauco::Adjustment.where(adjustable: items).tax.destroy_all
    ...
    def label
      Viauco.t included_in_price? ? :including_tax : :excluding_tax,
