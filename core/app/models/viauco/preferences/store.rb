# Use singleton class Viauco::Preferences::Store.instance to access
#
...
module Viauco::Preferences
  class StoreInstance
    ...
    def exist?(key)
      @cache.exist?(key) ||
        should_persist? && Viauco::Preference.where(key: key).exists?
    end
    ...
    def get(key)
      ...
      if should_persist?
        ...
        val = if preference = Viauco::Preference.find_by(key: key)
                # it does exist
                preference.value
              else
    ...
    def persist(cache_key, value)
      ...
      preference = Viauco::Preference.where(key: cache_key).first_or_initialize
    ...
    def destroy(cache_key)
      ...
      preference = Viauco::Preference.find_by(key: cache_key)
      preference&.destroy
    end

    def should_persist?
      @persistence && Viauco::Preference.table_exists?
