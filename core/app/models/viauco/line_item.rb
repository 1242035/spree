module Viauco
  class LineItem < Viauco::Base
    ...
    validates_with Viauco::Stock::AvailabilityValidator
    ...
    def sufficient_stock?
      Viauco::Stock::Quantifier.new(variant).can_supply? quantity
    end
    ...
    def verify_order_inventory
      Viauco::OrderInventory.new(order, self).verify(target_shipment, is_updated: true)
    end

    def verify_order_inventory_before_destroy
      Viauco::OrderInventory.new(order, self).verify(target_shipment)
    end
    ...
    def recalculate_adjustments
      Viauco::Adjustable::AdjustmentsUpdater.update(self)
    end

    def update_tax_charge
      Viauco::TaxRate.adjust(order, [self])
    end
