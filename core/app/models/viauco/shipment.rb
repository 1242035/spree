...
module Viauco
  class Shipment < Viauco::Base
    include Viauco::Core::NumberGenerator.new(prefix: 'H', length: 11)

    include NumberAsParam

    with_options inverse_of: :shipments do
      belongs_to :address, class_name: 'Viauco::Address'
      belongs_to :order, class_name: 'Viauco::Order', touch: true
    end
    belongs_to :stock_location, class_name: 'Viauco::StockLocation'
    ...
    has_one :selected_shipping_rate, -> { where(selected: true).order(:cost) }, class_name: Viauco::ShippingRate.to_s
    ...
    scope :reverse_chronological, -> { order(Arel.sql('coalesce(viauco_shipments.shipped_at, viauco_shipments.created_at) desc'), id: :desc) }
    ...
    def currency
      order ? order.currency : Viauco::Config[:currency]
    end
    ...
    def determine_state(order)
      ...
      order.paid? || Viauco::Config[:auto_capture_on_dispatch] ? 'ready' : 'pending'
    end
    ...
    def free?
      ...
      adjustments.promotion.any? { |p| p.source.type == 'Viauco::Promotion::Actions::FreeShipping' }
    end
