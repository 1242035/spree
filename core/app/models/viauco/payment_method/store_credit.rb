module Viauco
  class PaymentMethod::StoreCredit < ::Viauco::PaymentMethod
    ...
    def authorize(amount_in_cents, store_credit, gateway_options = {})
      if store_credit.nil?
        ActiveMerchant::Billing::Response.new(false, Viauco.t('store_credit_payment_method.unable_to_find'), {}, {})
      else
    ...
    def purchase(amount_in_cents, store_credit, gateway_options = {})
      eligible_events = store_credit.store_credit_events.where(
        ...
        action: Viauco::StoreCredit::ELIGIBLE_ACTION
      )
      event = eligible_events.detect do |eligible_event|
        store_credit.store_credit_events.where(authorization_code: eligible_event.authorization_code).
          where.not(action: Viauco::StoreCredit::ELIGIBLE_ACTION).empty?
      end
      ...
      store_credit.with_lock do
        if response = action.call(store_credit)
          ...
          ActiveMerchant::Billing::Response.new(
            true,
            Viauco.t('store_credit_payment_method.successful_action', action: action_name),
            {},
    ...
    def handle_action(action, action_name, auth_code)
      ...
      if store_credit.nil?
        ActiveMerchant::Billing::Response.new(
          false,
          Viauco.t('store_credit_payment_method.unable_to_find_for_action', auth_code: auth_code, action: action_name),
          {},
          {}
        )
      else
