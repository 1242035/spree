module Viauco
  module ProductScopes
    ...
    included do
      ...
      #
      #   Viauco::Product.joins(:taxons).where(Taxon.table_name => { id: taxon.id })
      #
      # If you're using count on the result of this scope, you must use the
      # `:distinct` option as well:
      #
      #   Viauco::Product.in_taxon(taxon).count(distinct: true)
      #
      # This is so that the count query is distinct'd:
      #
      #   SELECT COUNT(DISTINCT "viauco_products"."id") ...
      #
      #   vs.
      #
      #   SELECT COUNT(*) ...
      add_search_scope :in_taxon do |taxon|
        includes(:classifications).
          where('viauco_products_taxons.taxon_id' => taxon.self_and_descendants.pluck(:id)).
          order('viauco_products_taxons.position ASC')
      end

      # This scope selects products in all taxons AND all its descendants
      # If you need products only within one taxon use
      #
      #   Viauco::Product.taxons_id_eq([x,y])
      ...
      add_search_scope :with_option_value do |option, value|
        ...
        group('viauco_products.id').joins(variants_including_master: :option_values).where(conditions)
      end
      ...
      add_search_scope :taxons_name_eq do |name|
        group('viauco_products.id').joins(:taxons).where(Taxon.arel_table[:name].eq(name))
      end
