    ...
    class Sort
      ...
      def price(products)
        ...
        products.joins(master: :prices).
          select("#{Viauco::Product.table_name}.*, #{Viauco::Price.table_name}.amount").
          distinct.
          where(viauco_prices: { currency: currency }).
          order("#{Viauco::Price.table_name}.amount #{order_direction}")
      end
    end
  end
end
