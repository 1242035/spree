...
Viauco 0.40.0 represents another step forward towards the eventual 1.0.0
...
Viauco extensions that run 0.40.x but its a fairly safe bet that any
...
Viauco has replaced the authlogic gem in favor of
...
Some of the biggest changes to the Viauco authentication process is
consolidation of most all the user management functions into the
*viauco_auth* gem. Prior versions of Viauco still had small bits of code
floating around in *viauco_core* etc. So some files have simply been
...
Viauco 0.40.x introduces several minor but important changes to the REST
...
Guide](http://guides.viaucocommerce.org/legacy/0-40-x//rest.html) for more details.
...
Prior to Viauco 0.40.x the old method of authentication was to pass an
authentication token in the header. This involved using the specially
designated *X-ViaucoAPIKey* header and passing a corresponding token
...
have that access. Viauco allows so-called "guest checkouts" where users
...
Viauco provides a _TokenizedPermission_ model used to grant access to
various resources through a secure token. This model works in
conjunction with the _Viauco::TokenResource_ module which can be used to
add tokenized access functionality to any Viauco resource.

The _Order_ model is one such model in Viauco where this interface is
...
[security guide](http://guides.viaucocommerce.org/legacy/0-40-x/security.html#tokenized-permissions).
