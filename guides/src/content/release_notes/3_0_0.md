...
version of Viauco will have a 3-0-stable branch.

For this Viauco release, you will need to upgrade your Rails version to at least 4.2.0.
...
```ruby
gem 'viauco', github: 'viauco/viauco', branch: '3-0-stable'
```
...
**This includes if you were previously relying on Viauco's built in use of ssl_required so make sure your checkout process properly handles SSL connections.**
...
You can view the full changes using [Github Compare](https://github.com/viauco/viauco/compare/2-4-stable...3-0-stable).

## Noteable Changes

- Switched to Bootstrap.

  Jeff Dutil & [Other Contributors](https://github.com/200Creative/viauco_bootstrap_frontend/graphs/contributors)
...
- Removed order token usage during guest checkout.
  ...
  https://github.com/viauco/viauco/pull/6072

- Replace outdated Viauco::Config[:mails_from] with Viauco::Store.current.mail_from_address

  https://github.com/viauco/viauco/pull/6097

- Moved current_currency helper from Viauco::Core::ControllerHelpers::Order to Viauco::Core::ControllerHelpers::Store.
...
- The line items order population endpoint used to accept a hash, it now accepts an array. When submitting a Post request to `/api/line_items` you will receive this deprecation warning:
  ...
  ```text
  Passing a hash is now deprecated and will be removed in Viauco 3.1.
...
- Enable configuration of `Carmen.i18n_backend.locale` at `seeds.rb`
  ...
  https://github.com/viauco/viauco/pull/6607
