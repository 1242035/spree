...
This is the official 1.1 Release of Viauco. This is a minor release, and
...
are encouraged to alert the Viauco team about it by filing an issue on
...
bumping Viauco gem by modifying your Gemfile:
...
## viauco_product_groups now as standalone extension

Product Groups component has been extracted to a standalone viauco
extension. It is recommended that if you are using this functionality to
add the new extension to your Gemfile:

gem 'viauco_product_groups', git:
'git@github.com:viauco/viauco_product_groups.git'
...
moved](https://github.com/viauco/viauco/commit/0e684e01b5a15ec21b34263699004ebd78692f0d)
into the `Payment` model. If you have customized the
`Viauco::Admin::PaymentsController` or depend on any of the payment
processing methods inside the `CreditCard` model such as `authorize`,
`credit`, or `void`, this change may affect you.

## API rewrite

The API component of Viauco has undergone a major rewrite in order to
...
access the individual components of Viauco.

This is currently a work-in-progress and we would appreciate feedback on
the API on the [mailing
list](https://groups.google.com/group/viauco-user).

As a part of this, the `viauco/viauco_api` assets are no longer
...
This **may** affect your Viauco application, and if it does we would
advise [filing an issue](https://github.com/viauco/viauco/issues) so that
it can be promptly fixed. While the tests for Viauco itself are
extensive, there may still be edge cases where your application goes
that we do not have covered.

## Removed images association of `Viauco::Product`

`Viauco::Product` association with `viewable` has been moved `master`
variant of
the product. The change is backward compatible and require no upgrade
modification.

A call to the `images` method on a `Viauco::Product` will return all the
...
The version of Paperclip required by Viauco 1.1 is now any version of the
2.7.x branch of Paperclip.

## Clearer separation of Viauco components

It was [brought to our
attention](https://github.com/viauco/viauco/issues/1292) that the Core
component of Viauco depended upon things from other components, primarily
Auth. The purpose of the Core component is that it should be usable in
complete isolation from all other components of Viauco. This feature was
regressed during the 1..0 branch, but has [now been
fixed](https://github.com/viauco/viauco/issues/1296) in the 1.1 branch.

From 1.1 onwards, you will once again be able to use the Core component
of Viauco in isolation from the other components if you choose so.

## Allow for easier Viauco controller spec testing

We have added a `Viauco::Core::TestingSupport::ControllerRequests` module
to aid in the testing of Viauco controllers within not only the Viauco
components, but also within your application. [The documentation at the
top of this
module](https://github.com/viauco/viauco/blob/1-1-stable/core/lib/viauco/core/testing_support/controller_requests.rb)
...
`Viauco::Zone#in_zone?` is retired, please use `Viauco::Zone#include?`
instead.

### Viauco::PaymentMethod

`Viauco::PaymentMethod.current` is retired, please use
`current_order.payment_method` instead.

Additionally, `current_gateway` is also removed.

### Viauco::ProductsHelper
...
`Viauco::Variant.additional_fields` has been deprecated in favour of
using decorators and Deface. Please see
[#1406](https://github.com/viauco/viauco/issues/1406) for more
information.
...
-   Paperclip version has been bumped to 2.7.0.
    [#1148](https://github.com/viauco/viauco/issues/1148) +
    [#1152](https://github.com/viauco/viauco/issues/1152)
...
-   nested_set version has been bumped to 1.7.0.
    [9b7eda3](https://github.com/viauco/viauco/commit/9b7eda361dcb001ffa5ad20cd124428d95da21d6)
-   jquery-rails version has been bumped to \~> 2.0.0.
    [d4b3d7](https://github.com/viauco/viauco/commit/d4b3d76491)
-   deface version has been bumped to 0.8.0.
    [e571ed](https://github.com/viauco/viauco/commit/e571edd86dfadab48e4243caf4fd850a3fd10553)
-   highline version has been bumped to 1.6.11.
    [45f5e2](https://github.com/viauco/viauco/commit/45f5e2)

## Other fixes

-   Added `admin/orders/address_states.js` to precompile list.
    [#754](https://github.com/viauco/viauco/issues/754)
-   Added initializer to warn about orphaned preferences. [commit
    4f2669](https://github.com/viauco/viauco/commit/4f2669)
-   Address.default will no longer provide a nil value if the default
    country is deleted.
    [#1142](https://github.com/viauco/viauco/issues/1142) +
    [#997](https://github.com/viauco/viauco/issues/997)
-   Fix undefined_method to_d for PriceSack Shipping.
    [#1156](https://github.com/viauco/viauco/issues/1156)
-   Fixed rounding calculation bug for VAT.
    [#1128](https://github.com/viauco/viauco/issues/1128) +
    [#1172](https://github.com/viauco/viauco/issues/1172)
-   Allow `:error` key to be passed to `link_to_delete` .[#1169](https://github.com/viauco/viauco/issues/1169)
-   Fix issue where assigning a price such as"$5" to a variant
    caused it to set the price to 0.
    [#1173](https://github.com/viauco/viauco/issues/1173)
-   Product names longer than 50 characters are now truncated.
    [#1171](https://github.com/viauco/viauco/issues/1171)
-   Fix issue where preferences set to `false` were not being saved.
    [#1177](https://github.com/viauco/viauco/issues/1177)
-   Fix incorrect variable name in `script/rails` file inside extension
    generator. [#1135](https://github.com/viauco/viauco/issues/1135)
-   Acknowledge Viauco's own locale settings before `Rails.application.config.i18n.default_locale` for Viauco's
    locale details. [#1184](https://github.com/viauco/viauco/issues/1184)
-   Fix issue where preferences set to an empty string were not being
    saved. [#1187](https://github.com/viauco/viauco/issues/1187)
-   Set `default_url_options` in `mail_settings.rb` so that it doesn't
    need to be set manually for each environment or mailer.
    [#1188](https://github.com/viauco/viauco/issues/1188)
-   Correctly fire events for content paths and actions.
    [#1141](https://github.com/viauco/viauco/issues/1141)
-   Allow preferences with a type of `:text`.
-   Image settings (such as width & height) are now configurable via the
    Admin interface.
    [7d987fe](https://github.com/viauco/viauco/commit/7d987fe0e86d799d0896e123e638745201e7adb8)
-   Fix bug where `Payment#build_source` would fail dependent on the
    ordering of the hash passed in.
    [#981](https://github.com/viauco/viauco/issues/981)
-   Fix issue where states javascript include would be prefixed with
    asset host when alternate asset host was configured.
    [#1213](https://github.com/viauco/viauco/issues/1213)
-   Fix issue where `Promotion#products` would return no products due
    to incorrect class specification.
    [#1237](https://github.com/viauco/viauco/issues/1237)
-   Order email addresses are now validated with the Mail gem.
    [#1238](https://github.com/viauco/viauco/issues/1238)
-   Attempt to access `/admin/products/{id}` will now redirect to
    `/admin/products/{id}`.
    [#1239](https://github.com/viauco/viauco/issues/1239)
-   Show 'N/A' for tax category on `/admin/tax_rates` if a tax rate
    doesn't have a tax category.
    [#535](https://github.com/viauco/viauco/issues/#535)
-   Fix issue where incorrect order assignment was breaking return
    authorization creation.
    [#1107](https://github.com/viauco/viauco/issues/1107)
    [#1109](https://github.com/viauco/viauco/issues/1109)
    [#1149](https://github.com/viauco/viauco/issues/1149)
-   Fix issue where under certain circumstances users were able to view
    other people's order information.
    [#1243](https://github.com/viauco/viauco/issues/1243)
-   Fix issue where searching for orders by SKU was broken in admin
    backend. [#1259](https://github.com/viauco/viauco/issues/1259)
-   Logout when "Remember me" was checked for login now will actually
    log a user out. [#1257](https://github.com/viauco/viauco/issues/1257)
-   `Viauco::UrlHelpers` has moved to `Viauco::Core::UrlHelpers`
    [3bf5df](https://github.com/viauco/viauco/commit/3bf5df57e3474322dc484eb57ca5ee9098bd9454)
-   Preview buttons on the admin dashboard are now hidden once the
    dashboard has been configured.
    [#1271](https://github.com/viauco/viauco/issues/1271)
-   Slightly alter permalink code so that it does not conflict on
    similar names. Current permalinks will not be affected.
    [#1254](https://github.com/viauco/viauco/issues/1254)
-   Don't allow payments to be created in admin backend unless payment
    methods have been defined.
    [#1269](https://github.com/viauco/viauco/issues/1269)
-   `Address#full_name` will now return a string with no extra spaces
    around it. [#1298](https://github.com/viauco/viauco/issues/1298)
-   Ensure `StatesController` always returns JS response.
    [#1304](https://github.com/viauco/viauco/issues/1304)
...
-   Viauco::Calculator::PerItem will now calculate per-item, rather than
    ...
    rather than $3.
    [#1414](https://github.com/viauco/viauco/issues/1414)
````
