...
override the default Viauco layout template _application.html.erb_. The
...
and _set_title_ helper methods in Viauco::BaseController.
...
methods in Viauco::BaseController for further control.
...
*Viauco::Config+ is a new config option for customizing the stylesheets
used by the default application layout. The value
of*Viauco::Config[:stylesheets]+ is a comma-separated string of
stylesheet names without the file extensions. See the [customization
guide](http://guides.viaucocommerce.org/legacy/0-11-x/customization_overview.html) for more information.
...
\*Viauco::Config+ is a new config option for customizing the logo image
path.
...
configuration. After upgrading to Viauco 0.9.0 you are likely going to
...
to support Viauco 0.9.0. Please check the [extension
registry](https://github.com/viauco-contrib) to see which versions are
...
Calculators are configured using Viauco's flexible [preference
system](http://guides.viaucocommerce.org/legacy/0-11-x/preferences.html). Default values for the preferences are
...
Viauco now contains a standard mechanism by which calculator preferences
...
write their own [extensions](http://guides.viaucocommerce.org/legacy/0-11-x/extensions.html) to supply additional
functionality or to consider using a [third party
extension](https://github.com/viauco-contrib) written by members of the Viauco
community.

Calculators need to be "registered" with Viauco in order to be made
...
The _CalculatorExtenion_ that is included in the Viauco core is a good
...
register. Viauco provides a mechanism for extension authors to specify
...
***
Viauco automatically configures your calculators for you when using
...
Viauco 0.9.0 provides a new flexible system of adjustments associated
...
generically as an *Adjustment*. This allows a Viauco application to add
...
Viauco now supports a flexible coupon system. Coupons in an online store
...
***
The next version of Viauco will also provide built in filtering for
...
Viauco will not allow the credit amount to exceed the item total. The
...
shouldn't affect you too much unless you have an existing Viauco
...
the [relevant
commit](https://github.com/viauco/viauco/commit/ce1aad7bc25c15a794f8f5689efcdbf8c3311b7b)
in Github.
...
advance of loading into Viauco. Automatic cropping is particularly useful to make best use of screen
area.

# Update to SearchLogic

Viauco now runs with version 2.1.13 of SearchLogic. It has meant some
...
[Viauco
blog](http://viaucocommerce.com/blog/2009/07/30/updating-searchlogic/),
...
Using ideas from SearchLogic version 2, Viauco now contains a basic
...
Viauco now saves the last used bill and ship addresses for logged in
...
If a method *object_missing* for a controller, Viauco will pass all
...
If no method has been defined, Viauco will use its default 404 response.
...
The *rake viauco:upgrade* task has been eliminated. It turns out there
were some crucial flaws that caused issues when the older version of
Viauco used a different version of Rails or a different version of
*upgrade.task* than the newer version of Viauco. The rake task has been
...
After installing a new version of the Viauco gem, simply run either one
...
versions of Viauco. Instead of silently replacing crucial files in your
application, Viauco now checks the content of files it needs to replace,
...
version just loads the core Viauco routes file, so has no need to change.
